...
echo PLUGINDDK_DIR = \$\{BASE_DIR}/ddk/ae_plugin >> path.def
echo PLUGINEXT_DIR = \$\{BASE_DIR}/ddk/ae_plugin_ext >> path.def
...
if [ -d $TDS_PLUGINS_PATH ]; then
  echo "Copy ae_sdk plugins to ddk/ae_plugin..."
  find $TDS_PLUGINS_PATH -name ae_*.o | xargs -i cp {} $ROOTPATH/ddk/ae_plugin/
fi
