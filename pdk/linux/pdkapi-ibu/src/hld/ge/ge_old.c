 ...
 */
 ...
 #define ENABLE_HLD_MUTEX

 #ifdef ENABLE_HLD_MUTEX
static OSAL_ID ge_mutex = OSAL_INVALID_ID;

 #define ge_mutex_lock()	\
	do {						\
		if(ge_mutex==OSAL_INVALID_ID)\
		{										\
			ge_mutex = osal_mutex_create();\
			ASSERT(ge_mutex!=OSAL_INVALID_ID);\
		}												\
		osal_mutex_lock(ge_mutex, OSAL_WAIT_FOREVER_TIME);\
	}while(0)

#define ge_mutex_unlock()	\
    do {                        \
        osal_mutex_unlock(ge_mutex);   \
    }while(0)
 #endif
...
RET_CODE ge_io_ctrl_deo(struct ge_device *dev,UINT32 dwCmd,UINT32 dwParam)
{
	...
	if (dev->ioctl_deo)
	{
		return dev->ioctl_deo(dev,dwCmd,dwParam);
	}
