...
RET_CODE ge_set_dfb_global_color(struct ge_device *dev, UINT32 cmd_pos, UINT32 color)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_dfb_global_color(dev, cmd_pos, color);

	return RET_FAILURE;
}

RET_CODE ge_set_ptn_modulate_mode(struct ge_device *dev, UINT32 cmd_pos, UINT32 mode)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_ptn_modulate_mode(dev, cmd_pos, mode);

	return RET_FAILURE;
}

RET_CODE ge_set_src_premultiply(struct ge_device *dev, UINT32 cmd_pos, enum GE_PREMULTIPLY_SEL sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_src_premultiply(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_ptn_premultiply(struct ge_device *dev, UINT32 cmd_pos, enum GE_PREMULTIPLY_SEL sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_ptn_premultiply(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_ptn_premult_color(struct ge_device *dev, UINT32 cmd_pos, enum GE_PREMULTIPLY_SEL sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_ptn_premult_color(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_ptn_blend_mode(struct ge_device *dev, UINT32 cmd_pos, enum GE_PTN_BLEND_MODE sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_ptn_blend_mode(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_src_blend_mode(struct ge_device *dev, UINT32 cmd_pos, enum GE_SRC_BLEND_MODE sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_src_blend_mode(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_dst_demultiply(struct ge_device *dev, UINT32 cmd_pos, enum GE_PREMULTIPLY_SEL sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_dst_demultiply(dev, cmd_pos, sel);

	return RET_FAILURE;
}

RET_CODE ge_set_dst_xor(struct ge_device *dev, UINT32 cmd_pos, BOOL sel)
{
	/* If device not running, exit */
	if ((dev->flags & HLD_DEV_STATS_UP) == 0)
	{
	...
	if (dev->set_scan_order)
	    return dev->set_dst_xor(dev, cmd_pos, sel);
