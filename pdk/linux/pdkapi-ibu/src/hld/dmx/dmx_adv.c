...
//#define ADV_DMX_DBG
#ifdef ADV_DMX_DBG
#define ADV_DMX_PRINTF			printf//soc_printf
...
void adv_dmx_task(UINT32 param1, UINT32 param2)
{
	...
	{
		UINT32 i;
		osal_mutex_lock(adv_dmx_wait, 200);
...
INT32 adv_dmx_test_cb(UINT32 ch_id, UINT32 flt_id, UINT8 * section, UINT32 length)
{
	...
	printf("ch %d, flt %d submit sec_len %d, sec_buf %08x: ", ch_id, flt_id, length, section);
/*
	if(length>8)
		length = 8;
	for(i=0; i<length; i++)
...
INT32 adv_dmx_test_dump(UINT32 ch_id, UINT32 flt_id, UINT8 * section, UINT32 length)
{
	...
	adv_dmx_test_mutex(1);
#if 1	
	printf("ch %d, flt %d dump sec %d: ", ch_id, flt_id, length);
	if(length>8)
		length = 8;
	for(i=0; i<length; i++)
		printf("0x%02x ", section[i]);
