...
int dib8000_set_diversity_in(struct dibFrontend *fe, int onoff)
{
    ...
    dbgpl(&dib8000_dbg,"fe %d state->revision = 0x%04x",fe->id, state->revision);
...
static void dib8000_set_acquisition_mode(struct dib8000_state *state)
{
    ...
    dbgpl(&dib8000_dbg, "acquisition mode activated");
    if (state->revision >= 0x8002)
        nud |= (1<<5) | (1<<2);
    else
        nud |= (1<<3) | (1<<0);
...
int dib8000_set_output_mode(struct dib8000_state *state, int mode)
{
    ...
    dbgpl(&dib8000_dbg, "-I-  Setting output mode for demod %p to %d", state->fe, mode);
    ...
    default:
        dbgpl(&dib8000_dbg, "Unhandled output_mode passed to be set for demod %p", state->fe);
...
int dib8000_pid_filter_ctrl(struct dibFrontend *fe, uint8_t onoff)
{
    ...
    dbgpl(&dib8000_dbg, "pid filter enabled %d", onoff);
...
{
    struct dib8000_state *state = fe->demod_priv;
    dbgpl(&dib8000_dbg, "Index %x, PID %d, OnOff %d", id, pid, onoff);
