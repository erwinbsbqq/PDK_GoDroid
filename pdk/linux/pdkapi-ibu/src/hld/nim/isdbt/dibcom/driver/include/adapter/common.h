...
#define MAX_NB_OF_VALID_FRONTENDS    4
...
extern struct dibChannelFEInfo * channel_frontend_info(struct dibChannel *, int id);
extern struct dibChannelMonitor * channel_frontend_monitoring(struct dibChannel *, int id);
...
void RtmHal(void);
void RtmHalInit(void);
int  RtmHalGetChipId(void);
void Blink(void);
void cache_flush(void);
int  ProcessHalMsgIntIn(void);
int  ProcessHalMsgIntOut(uint16_t id, uint8_t *tx, uint32_t txlen, uint8_t *rx, uint32_t rxlen);
