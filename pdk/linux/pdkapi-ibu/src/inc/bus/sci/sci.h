...
unsigned char api_uart_read(void);
int api_uart_write(unsigned char ch);

#define sci_mode_set(id, bps, parity, ...)	api_uart_set_mode(bps, parity)
#define sci_clear_buff(...)					do{}while(0)
#define sci_loopback(...)					do{}while(0)
#define sci_read(id)						api_uart_read()
//#define sci_read_tm(...)					ERR_FAILUE
#define sci_read_tm(id, ch, tm)			    api_uart_read_timeout(ch, tm)
#define sci_write(id, ch)					api_uart_write(ch)
#define enable_uart_printf()				api_uart_printf_mode(0)
#define disable_uart_printf()				api_uart_printf_mode(1)
