...
struct net_device
{
	...
	void (*set_enable_status)(struct net_device *, int);
	int (*get_enable_status)(struct net_device *);
	int (*get_ready_status)(struct net_device *);
	int (*get_ap_quality)(struct net_device *); //cpadd_110420_add_quality
	
	int (*plugin_callback)(UINT8);
	int (*rt5370_callback)(UINT32);
	int customer_pid[10];
	int customer_vid[10];
	int wifi_device_enable;
