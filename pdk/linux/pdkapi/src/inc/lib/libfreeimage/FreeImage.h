...
typedef struct {
	BOOL exist;						/* whether EXIF data is exist */
	unsigned int width;				/* the width of image */
	unsigned int height;				/* the height of image */
}FI_EXIF;

typedef struct {
	unsigned int flags;		// flags for load function
	unsigned int page;		// page for load function
	unsigned int page_count;	// page count of image
	unsigned int length;		// the size of buffer for decoded data
	unsigned char* buffer;		// points to the buffer for decoded data
	...
	unsigned int delay_time;		/* the delay time before displaying */
	unsigned int disposal_method;	/* the disposal method for this image */
	FI_COLOR_TYPE color_type;
  	BOOL progressive;		/* TRUE if JPEG specifies progressive mode */
	BOOL preview;			/* TRUE if preview mode */
	FI_EXIF exif;				/* EXIF info */
} FI_ATTR;
...
//set io 
DLL_API void DLL_CALLCONV SetDefaultIO(FreeImageIO *io);
DLL_API void DLL_CALLCONV SetMemoryIO(FreeImageIO *io);
DLL_API int DLL_CALLCONV FreeImage_Stop(void);
DLL_API int DLL_CALLCONV FreeImage_Start(void);
DLL_API int DLL_CALLCONV FreeImage_Free_GIFInfo(fi_handle handle);

extern BOOL s_stop_flag;
