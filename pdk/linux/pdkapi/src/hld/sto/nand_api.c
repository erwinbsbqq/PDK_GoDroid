...
#include <osal/osal.h>
#include <hld/sto/adr_nand_api.h>
...
#define BA_SCAN_START           0
#define BA_SCAN_RANGE           (BA_SCAN_START + 4096)
#define BA_PAGE_SKIP            64

struct BA_NAND_CONFIG
{
	UINT32 wBlksPerChip;       //Blocks per chip
	UINT32 wBytesPerPage;      //Bytes per page (2K,4K,8K etc)  
	UINT32 wPagesPerBlock;     //Pages per Block(64,128, 192,256 etc) 
	UINT32 wReserved0[6];      //Reserved for compatibility, filled with 0xffffffff
	UINT32 wReadClock;         //Read clock setting 
    UINT32 wWriteClock;        //Write clock setting
	UINT32 wReserved1[5];      //Reserved for compatibility, filled with 0xffffffff
};

unsigned char ba_chip_id_pattern[4] = {0x39, 0x21, 0x55, 0xAA};
int word_to_bit(UINT16 word)
{
	if (0x0000 == word)
	{
		return 0;
...
int ba_check_chip_id(UINT8 *buf)
{
	int bit = 0;
	volatile unsigned char i = 0, j = 0; 
	volatile unsigned short *p = (UINT16 *)buf;
	volatile unsigned char data[4];

	memset((UINT8 *)data, 0, 4);
	for (i=0; i<4; i++) 
	{    
		for (j=0; j<8; j++) 
		{
...
INT32 nand_get_bootarea_cfg(struct BA_NAND_CONFIG *ba_nf_cfg)
{
	struct mtd_part_info info = {0};
	unsigned char ba_nand_info_data[2048] = {0};
	int part_idx = 0, page = 0;
	UINT32 start_addr = 0;
	...
	for (part_idx=0; part_idx<2; part_idx++)
	{
		for (page=0; page<=BA_SCAN_RANGE/2; page+=BA_PAGE_SKIP)
		{
			start_addr = page * info.writesize;
			if (NF_read_part_physical(part_idx, start_addr, 2048, ba_nand_info_data) > 0)
			{
				if (ba_check_chip_id(&(ba_nand_info_data[BA_NAND_CHIP_ID_OFFSET])) == 0)
				{
					memcpy(ba_nf_cfg, (ba_nand_info_data+BA_NAND_CONFIG_OFFSET), sizeof(struct BA_NAND_CONFIG));
					NANDAPI_DEBUG("nf_cfg: wRdClk 0x%x wWrClk 0x%x wBlksPerChip 0x%x wBytesPerPage 0x%x wPagesPerBlock 0x%x\n",
							ba_nf_cfg->wReadClock,
							ba_nf_cfg->wWriteClock,
							ba_nf_cfg->wBlksPerChip,
							ba_nf_cfg->wBytesPerPage,
							ba_nf_cfg->wPagesPerBlock);

					NANDAPI_DEBUG("[:)] try page:%d ok\n", page);
					return page;
				}
			}
		}
...
{
	struct BA_NAND_CONFIG ba_nand_cfg = {0};
	unsigned long long chipsize = 0;

	if (nand_get_bootarea_cfg(&ba_nand_cfg) < 0)
	{
		NANDAPI_ERROR("can not find valid PMI\n");
		return 0UL;	
	}

	NANDAPI_DEBUG("wBlksPerChip 0x%x wBytesPerPage 0x%x wPagesPerBlock 0x%x\n",
			ba_nand_cfg.wBlksPerChip,
			ba_nand_cfg.wBytesPerPage,
			ba_nand_cfg.wPagesPerBlock);

	chipsize = (unsigned long long)ba_nand_cfg.wBlksPerChip*
		(unsigned long long)ba_nand_cfg.wPagesPerBlock*(unsigned long long)ba_nand_cfg.wBytesPerPage;

	NANDAPI_DEBUG("[*] Nand Total Size is %llu Bytes\n", chipsize);
	return chipsize;
}
