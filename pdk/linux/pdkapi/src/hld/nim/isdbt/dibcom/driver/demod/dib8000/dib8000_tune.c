...
void dib8000_update_timf(struct dib8000_state *state)
{
	...
	dbgpl(&dib8000_dbg, "FE %d Updated timing frequency: %d (default: %d)",state->fe->id, state->timf, state->timf_default);
}
...
static int dib8000_agc_soft_split(struct dib8000_state *state)
{
	...
	dbgpl(&dib8000_dbg, "FE %d AGC split_offset: %d",state->fe->id, split_offset);
...
static int dib8000_set_agc_config(struct dib8000_state *state, uint8_t band)
{
	...
	if (agc == NULL) {
		dbgpl(&dib8000_dbg, "FE %d No valid AGC configuration found for band 0x%02x",state->fe->id, band);
	...
	dbgpl(&dib8000_dbg, "FE %d WBD: ref: %d, sel: %d, active: %d, alpha: %d",
...
static uint16_t dib8000_get_init_prbs(uint16_t subchannel, struct dibChannel *ch)
{
   ...
   dbgpl(&dib8000_dbg,"sub_channel_prbs_group = %d , subchannel =%d prbs = 0x%04x",sub_channel_prbs_group, subchannel, lut_prbs_8k[sub_channel_prbs_group]);
...
static void dib8000_set_frequency_offset(struct dib8000_state *state, struct dibChannel *ch)
{
    ...
    dbgpl(&dib8000_dbg,"FE %d : %dkhz tuner offset (ch->RF_kHz = %dKHz & state->fe->current_rf = %dKHz) total_dds_offset_khz = %d", state->fe->id, ch->RF_kHz - state->fe->current_rf, ch->RF_kHz, state->fe->current_rf, total_dds_offset_khz);
...
#ifdef CONFIG_DIB8000_NEW_AUTOSEARCH
        dbgpl(&dib8000_dbg, "FE %d IN: context status = %d, TUNE_STATE %d autosearch step = %u systime = %u",fe->id, fe->status, *tune_state, state->autosearch_state, now);
#else
        dbgpl(&dib8000_dbg, "FE %d IN: context status = %d, TUNE_STATE %d systime = %u",fe->id, fe->status, *tune_state, now);
#endif
#endif
    ...
    switch (*tune_state) {
...
#ifdef DEBUG_DIB8000_TUNE
                    dbgpl(&dib8000_dbg, "FE %d corm fft: %u %u %u", fe->id, corm[0], corm[1], corm[2]);
#endif
                    max_value = 0;
...
#ifdef DEBUG_DIB8000_TUNE
                    dbgpl(&dib8000_dbg, "FE %d Autosearch FFT has found Mode %d", fe->id, max_value+1);
#endif
                    *tune_state = CT_DEMOD_SEARCH_NEXT;
...
#ifdef DEBUG_DIB8000_TUNE
                    dbgpl(&dib8000_dbg, "FE %d guard interval found=%i", fe->id, state->found_guard);
#endif
                    *tune_state = CT_DEMOD_STEP_3;
                    break;
                default:
        ...
        case CT_DEMOD_STEP_6: /* (36)  if there is an input (diversity) */
            if(fe->input != NULL && state->output_mode != OUTPUT_MODE_DIVERSITY) {
                dbgpl(&dib8000_dbg,"FE %d outup_mode = %d", fe->id, state->output_mode);
                ...
                if((dib8000_read_word(state, 1291)>>9)&0x1) { /* fe capable of deinterleaving : esram */
                    ...
                    for (i = 0; i < 3; i++) {
                        if(ch->u.isdbt.layer[i].time_intlv >= deeper_interleaver) {
                            dbgpl(&dib8000_dbg,"FE %d ch->u.isdbt.layer[%d].time_intlv = %d ",state->fe->id, i, ch->u.isdbt.layer[i].time_intlv);
                    ...
                    *timeout = now + (2000 * locks); /* give the mpeg lock 800ms if sram is present */
                    dbgpl(&dib8000_dbg,"FE %d Deeper interleaver mode = %d on layer %d : timeout mult factor = %d => will use timeout = %d",state->fe->id, deeper_interleaver, state->longest_intlv_layer, locks, *timeout);
        ...
        case CT_DEMOD_STEP_10: /* 40 */
            if (locks&(1<<(7-state->longest_intlv_layer))) { /* mpeg lock : check the longest one */
                dbgpl(&dib8000_dbg,"FE %d Mpeg locks [ L0 : %d | L1 : %d | L2 : %d ]",state->fe->id, (locks>>7)&0x1, (locks>>6)&0x1, (locks>>5)&0x1);
                if (ch->u.isdbt.sb_mode && ch->u.isdbt.sb_subchannel == -1 && !state->differential_constellation)
...
void dib8000_autosearch_start(struct dibFrontend *fe, struct dibChannel *ch)
{
...
#endif
    {
        ...
        dbgpl(NULL,"FE %d Using list for autosearch : %d", fe->id, slist);
...
static int dib8000_autosearch_irq(struct dibFrontend *fe)
{
    ...
    if ((state->revision >= 0x8002) && (state->autosearch_state == AS_SEARCHING_FFT)) {
        if (b[1] & 0x1) {
            dbgpl(&dib8000_dbg, "FE %d SEARCHING FFT : max correlation result available", fe->id);
            return 3;
        }
    }
    else
    ...
    {
        if (b[1] & 0x1) { // failed
            dbgpl(&dib8000_dbg, "FE %d Autosearch IRQ: failed", fe->id);
            return 1;
        }

        if (b[1] & 0x2) { // succeeded
            dbgpl(&dib8000_dbg, "FE %d Autosearch IRQ: success", fe->id);
            return 2;
        }
    }
