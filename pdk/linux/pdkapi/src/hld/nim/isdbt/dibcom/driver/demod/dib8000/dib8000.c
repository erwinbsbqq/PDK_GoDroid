...
static int dib8000_wakeup(struct dibFrontend *fe)
{
    ...
    if (dib8000_set_adc_state(state, DIBX000_SLOW_ADC_ON) != DIB_RETURN_SUCCESS)
        dbgpl(&dib8000_dbg, "FE %d Can not start Slow ADC", fe->id);
...
void dib8000_set_dds(struct dib8000_state *state, int32_t offset_khz)
{
    ...
    dbgpl(&dib8000_dbg, "FE %d: Setting a DDS frequency offset of %c%dkHz", state->fe->id, invert ? '-' : ' ', dds / unit_khz_dds_val);
...
static int dib8000_release(struct dibFrontend *fe)
{
 ...
 uint16_t dib8000_identify(struct dibDataBusClient *client)
{
    uint16_t value;
    ...
    if ((value = data_bus_client_read16(client, 896)) != 0x01b3) {
        printf( "wrong Vendor ID (read=0x%x)\n", value);
        return 0;
    }

    value = data_bus_client_read16(client, 897);
    if (value != 0x8000 && value != 0x8001 && value != 0x8002) {
        libc_printf(&dib8000_dbg, "wrong Device ID (%x)\n", value);
        return 0;
    }

    switch (value) {
        case 0x8000: printf( "found DiB8000A\n"); break;
        case 0x8001: printf( "found DiB8000B\n"); break;
        case 0x8002: printf( "found DiB8000C\n"); break;
    }
...
int dib8000_set_bandwidth(struct dib8000_state *state, uint32_t bw)
{
    ...
    if (state->timf == 0) {
        dbgpl(&dib8000_dbg, "FE %d is going to use the default TIMF value", state->fe->id);
        timf = state->timf_default;
    } else {
        dbgpl(&dib8000_dbg, "FE %d is going to use the updated TIMF value", state->fe->id);
        timf = state->timf;
    }
...
{
    dbgpl(&dib8000_dbg, "FE %d ifreq: %d %x, inversion: %d", state->fe->id, bw->ifreq, bw->ifreq, bw->ifreq >> 25);
...
static void dib8000_reset_pll(struct dib8000_state *state)
{
    ...
    dbgpl(&dib8000_dbg, "FE %d clk_cfg1 = 0x%04x", state->fe->id, clk_cfg1); /* 0x507 1 0 1 000 0 0 11 1 */
...
static int dib8000_reset(struct dibFrontend *fe)
{
    ...
    if (state->revision == 0x8000)
        dbgpl(&dib8000_dbg, "FE %d Error : dib8000 MA not supported", fe->id);
    ...
    if (state->cfg.drives)
        ...
        dbgpl(&dib8000_dbg, "FE %d : Using standard PAD-drive-settings, please adjust settings in config-struct to be optimal.",fe->id);
    ...
    if (dib8000_reset_gpio(state) != DIB_RETURN_SUCCESS)
        dbgpl(&dib8000_dbg, "FE %d : GPIO reset was not successful.",fe->id);

    if (dib8000_set_output_mode(state, OUTPUT_MODE_OFF) != DIB_RETURN_SUCCESS)
        dbgpl(&dib8000_dbg, "FE %d : OUTPUT_MODE could not be resetted.",fe->id);
...
printf("\sam_2 \n");			
            if (dib8000_identify(&client) == 0) {
                dbgpl(&dib8000_dbg, "#%d: not identified", k);
printf("\sam_3 \n");
                return DIB_RETURN_ERROR;
            }
        }
...
printf("\sam_5 ,new_addr =%x \n",new_addr);
        dbgpl(&dib8000_dbg, "IC %d initialized (to i2c_address 0x%x)", k, new_addr);
    }
...
void dib8000_reset_sync_pha(struct dibFrontend* fe, uint32_t loop_factor)
{
    ...
    if (state->diversity_onoff == 0) {
        dbgpl(&dib8000_dbg, "FE %d : Diversity is not active", fe->id);
        return;
    }
...
void dib8000_tune_reset_fes(struct dibFrontend *fe[] , uint32_t num_fe)
{
    int i;
    if (num_fe > 0) {
        dbgpl(&dib8000_dbg ,"Trying to reset %d",num_fe);
        dib8000_set_low_thres(fe[num_fe]);
        for (i=num_fe-1; i >= 0; i--) {
            if (fe[i]->input != NULL) {
                dbgpl(&dib8000_dbg ,"fe%d",i);
