...
#define VBI_PRINTF(fmt, args...) ADR_DBG_PRINT(VBI, fmt, ##args)
...
INT32 vbi_open(struct vbi_device *dev)
{
	...
	{
		VBI_PRINTF("vbi_open: warning - device openned already!\n");
		return SUCCESS;
	}
	...
	if(vbi_handle < 0)
	{
		VBI_PRINTF("vbi_open: fail to open device %s!\n", dev->name);
		result =  !SUCCESS;
	}
	else
	{
...
INT32 vbi_close(struct vbi_device *dev)
{
	...
	{
		VBI_PRINTF("vbi_close: warning - device %s closed already!\n", dev->name);
		return SUCCESS;
	}
