...
{	
	...
	{
		PAN_ERR_PRINTF("%s : rfk get port fail\n", __FUNCTION__);			
		ret_ir |= ERR_NO_DEV;
	}
	else
	{
		...
		{
			PAN_ERR_PRINTF("[ %s %d ], rfk ioctl set port fail\n", __FUNCTION__, __LINE__);	
	...
	{
		PAN_ERR_PRINTF("[ %s %d ], rfk get port fail\n", __FUNCTION__, __LINE__);			
		ret_panel |= ERR_NO_DEV;
	}
	else
	{
