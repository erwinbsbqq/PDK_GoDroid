...
#define SCART_PRINTF(fmt, args...) ADR_DBG_PRINT(SCART, fmt, ##args)
...
INT32 scart_open(struct scart_device *dev)
{
	...
	{
		SCART_PRINTF("scart_open: warning - device %s openned already!\n", dev->name);
		return SUCCESS;
	}
	...
	if(scart_handle < 0)
	{
		SCART_PRINTF("open Scart handle fail\n");
		return RET_FAILURE;
	}
	else
	{
			...
			SCART_PRINTF("open Scart handle success\n");
	}
	close(scart_handle);
	return result;
}
...
INT32 scart_close(struct scart_device *dev)
{
	...
	{
		SCART_PRINTF("sdec_close: warning - device %s closed already!\n", dev->name);
		return SUCCESS;
	}
...
{	
	...
	if (dev == NULL)
	{
		SCART_PRINTF("Error: Alloc SCART switch device ERROR!\n");
		return ERR_NO_MEM;
	}
	...
	{
		SCART_PRINTF("Error: Register RF modulator device ERROR!\n");
	}
	dev->flags = 0;
