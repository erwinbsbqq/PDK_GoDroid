...
#else
#define PRF(fmt, args...)	ADR_DBG_PRINT(RFK, fmt, ##args)
#endif
...
int rfk_get_port(void)
{
	...
	#ifdef ADR_ALIDROID	
		dev->sock_id = socket(AF_NETLINK, SOCK_RAW | O_CLOEXEC, NETLINK_ALITRANSPORT);
	#else
		dev->sock_id = socket(AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC, NETLINK_ALITRANSPORT);
	#endif
	if(dev->sock_id < 0)
	{
...
void rfk_free_port(int port_id)
{
	...
	if(dev->busy)
	{
		...
		if (dev->nlmsghdr)
		{
			free(dev->nlmsghdr);
			dev->nlmsghdr = NULL;
			m_dev[port_id - 1]->nlmsghdr = NULL;
		}
		if (dev)
		{
			free(dev);
			dev = NULL;
			m_dev[port_id - 1] = NULL;
		}
	}
