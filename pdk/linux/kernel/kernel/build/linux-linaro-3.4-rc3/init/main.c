...
static int __init unknown_bootoption(char *param, char *val)
{
	...
	if (val) {
		...
		for (i = 0; envp_init[i]; i++) {
			if (i == MAX_INIT_ENVS) {
				printk("Too many boot env vars at\n");
		...
		for (i = 0; argv_init[i]; i++) {
			if (i == MAX_INIT_ARGS) {
				printk("Too many boot init vars at\n");
...
debug_step = 17;
	...
	trap_init();
	debug_step = 22;
	printk("%d: %x\n",__LINE__, *(unsigned int*)0xfeff9200);
	mm_init();
debug_step = 23;
	printk("%d: %x\n",__LINE__, jiffies);
	/*
