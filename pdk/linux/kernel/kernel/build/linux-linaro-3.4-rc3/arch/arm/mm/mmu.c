...
/* Added by tony for mark lowmem DRAM as no cacheable HW buffer.
 * Date:2013.07.11 
*/
void arm_remap_lowmem_nocache(unsigned int start, unsigned int len)
{
	struct map_desc map;
	int ret;

    map.pfn = __phys_to_pfn(start);
    map.virtual = phys_to_virt(start);    
    map.length = len;
    map.type = MT_MEMORY_NONCACHED; //MT_UNCACHED;
    create_mapping(&map, false);
	
    printk("=== mem_nocache_reserved finished! === start 0x%08x len 0x%08x\n", start, len);
}
/*Added end
*/
