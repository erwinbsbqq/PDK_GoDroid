...
int lirc_register_driver(struct lirc_driver *d)
{
	...
	devfs_mk_cdev(MKDEV(IRCTL_DEV_MAJOR, ir->d.minor), S_IFCHR|S_IRUSR|S_IWUSR,	DEV_LIRC "/%u", ir->d.minor);
#endif
#if 1
(void)lirc_device_create(lirc_class, ir->d.dev, MKDEV(IRCTL_DEV_MAJOR, ir->d.minor),NULL, "lirc%u", ir->d.minor);
#endif

/*Terry*/
//device_create(lirc_class, NULL, MKDEV(IRCTL_DEV_MAJOR, ir->d.minor),NULL, "lirc%u", ir->d.minor);
//	printk("\n\n\nTerry Lee %s   %d \n\n\n", __func__, __LINE__);
/**/
 ...
 */
	...
	MOD_INC_USE_COUNT;
#endif
	printk("lirc_dev: driver %s registered at minor number = %d\n",
		ir->d.name, ir->d.minor);
	d->minor = minor;
	return minor;
 ...
 */
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 35))
static int irctl_ioctl(struct file *file,
...
/* Added by Terry 2010-08-11 */
static int lirc_dev_init(void)
{  	
	if(register_chrdev(IRCTL_DEV_MAJOR, IRCTL_DEV_NAME, &fops)) {
		printk("lirc_dev: register_chrdev failed\n");
		goto out;
	}

	lirc_class = class_create(THIS_MODULE, "lirc");
	if(IS_ERR(lirc_class)) {
		printk("lirc_dev: class_create failed\n");
		goto err;
	}
	return 0;
