				...
				bool en, u32 mask)
{
	...
	if (BIT_PWR_GYRO_STBY_MASK == mask) {
		i2c_smbus_read_byte_data(sensor->client,
			reg->pwr_mgmt_1);
		/*ret = i2c_smbus_write_byte_data(sensor->client,
...
static int mpu6050_pinctrl_init(struct mpu6050_sensor *sensor)
{
	struct i2c_client *client = sensor->client;
	return 0;//francis
	sensor->pinctrl =pinctrl_get(&client->dev);// devm_pinctrl_get(&client->dev);
			...
			struct mpu6050_platform_data *pdata)
{
	...
	pdata->gpio_en = of_get_named_gpio_flags(dev->of_node,
				"invn,gpio-en", 0, NULL);
	printk("mpu6050_parse_dt  gpio_en=%d\n",pdata->gpio_en);
...
{
	int ret;
	//ret = platform_driver_register(&mpu6050_i2c_driver);
...
{
	//platform_driver_unregister(&mpu6050_i2c_driver);
