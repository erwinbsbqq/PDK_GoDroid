 ...
 static unsigned long irc_decode(struct irc_decoder *ird, unsigned long pulse_width, unsigned long pulse_polarity)
{
	...
	else
	{
		...
		if (ird->attr->normal)
		{
			...
			{
				unsigned long accum_pulse_1 = PULSE_VALUE(ird->attr->pulse[IR_DATA + 1].fst_half) + PULSE_VALUE(ird->attr->pulse[IR_DATA + 1].scd_half);
...
unsigned long irc_pulse_to_code(unsigned long pulse_width, unsigned long pulse_polarity)
{
	...
	for (i = 0; i< IRP_CNT; i++)
	{
		...
		//if (key_code != IR_KEY_INVALID && key_code != 0)
