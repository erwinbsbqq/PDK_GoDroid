...
qh_completions (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
 ...
 retry_xacterr:
	...
	qtd->length - QTD_LENGTH(token), qtd->length, qh->xacterrs);
				 ...
				 /*retry for babble, David */
				else if ((token & QTD_STS_BABBLE) && ++qh->xacterrs < QH_XACTERR_MAX && !urb->unlinked) {
					//GPIO only for debug
					//(*(volatile unsigned int *)0xB80000F4) |= (1<<24);			
					printk(ehci, "detected Babble error retry %d port status 0:0x%08x, 1:0x%08x\n", qh->xacterrs, 
							ehci_readl(ehci, &ehci->regs->port_status[0]), 
							ehci_readl(ehci, &ehci->regs->port_status[1]));
                    #if 0
                    libc_printf("detected Babble error retry %d port status 0:0x%08x, 1:0x%08x\n", qh->xacterrs, 
							ehci_readl(ehci, &ehci->regs->port_status[0]), 
							ehci_readl(ehci, &ehci->regs->port_status[1]));
                    #endif
