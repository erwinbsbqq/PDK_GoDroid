...
#ifdef DMA_LOG
	printk("%s, dma_handl=%08x\n",__func__, *dma_handle);
#endif

	if (patch_s3921)
	{
		//#ifdef CONFIG_PATCH_ALI_S3921_EHCI
		*dma_handle &= 0x7fffffff;
		//#endif
	}
			    ...
			    gfp_t mem_flags)
{
	...
	if (urb->transfer_buffer_length != 0
		...
		if (hcd->self.uses_dma) {
			if (urb->num_sgs) {
			...
			#ifdef DMA_LOG
				printk("%s_6, urb->transfer_buffer = %08x\n",__func__, urb->transfer_buffer);
			#endif
				urb->transfer_dma = dma_map_single(
						...
						dir);
			#ifdef DMA_LOG
				printk("%s_6, urb->transfer_dma = %08x\n",__func__, urb->transfer_dma);
			#endif
				if (patch_s3921)
				{
					//#ifdef CONFIG_PATCH_ALI_S3921_EHCI			
