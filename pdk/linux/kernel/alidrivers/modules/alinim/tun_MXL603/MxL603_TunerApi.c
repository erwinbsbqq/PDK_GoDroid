...
const UINT8 MxLWare603DrvVersion[] = {2, 1, 2, 1, 0}; 
...
// Low power
		...
		status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x18);
		status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_IF_PWR, 0xF1);
#else //20140325, Mxl603, increase driver level, for better cooperated with MN8847x 
		status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_IF_CFG_0, 0x7E); 
		status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_IF_CFG_1, 0x10);
		status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_IF_PWR, 0xB1);
#endif
      }
...
MXL_STATUS MxLWare603_API_CfgTunerSouthAfricaLT(UINT32 devId)
{
  UINT8 status = MXL_SUCCESS;
  ...
  status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_TUNE_RF1_BO_REG, 0x06);
  status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
