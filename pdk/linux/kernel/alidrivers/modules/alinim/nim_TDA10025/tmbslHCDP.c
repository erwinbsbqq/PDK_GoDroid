...
iHCDP_SetMFFilter(pHCDPObject_t pObj);
...
//#define TMBSL_TDA10025_NO_64BITS
    ...
    if(err == TM_OK || err == HCDP_ERR_ERR_NO_INSTANCES)
    {
        ...
        else 
        {
            ...
            if(err == TM_OK)
            {
                ...
                tmDBGPRINTEx(DEBUGLVL_INOUT, "tmbslHCDP__Open(0x%08X)", tUnit);
 ...
 )
{
    ...
    /* Differenciate ES1 and ES2 to support specific settings for both */
    if (err == TM_OK)
    {
    ...
    if (err == TM_OK)
    {
        if( pObj->bReset == True )
        {
            ...
            if (err == TM_OK)
            {
                if (0x187F == uVal || 0x186F == uVal)
                {
                    eDemodLockStatus = tmbslFrontEndStateLocked;
                }
                else
                {
                    eDemodLockStatus = tmbslFrontEndStateNotLocked;
                }
        ...
        {
            /* this path has not be initialised */
            *peLockStatus = tmbslFrontEndStateNotLocked;
        }
    }
...
)
{
    ...
    case tmFrontEndModulationQam64:
        ...
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegEqFfLeakageAlpha, 0xF);
        }
        ...
        /* CtlAcqGain */
        /* overwrites the default value 0xC2 defined by QamMode */
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlAcqDirGain, 0x4);
        }
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlAcqFractGain, 0x0);
        }*/
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlAcqIndGain, 0x2);
        }

        /* CtlTrkGain */
        /* overwrites the default value 0x142 defined by QamMode */
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkDirGain, 0x4);
        }
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkFractGain, 0x0);
        }*/
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkIndGain, 0x5);
        }*/

        /* SctlTrkGain */
        /* overwrites the default value 0x83 defined by QamMode */
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegSctlTrkDirGain, 0x4);
        }
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegSctlTrkFractGain, 0x0);
        }*/
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegSctlTrkIndGain, 0x2);
        }
    ...
    case tmFrontEndModulationQam128:
        ...
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegEqFfLeakageAlpha, 0xE);
        }
        ...
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlAcqFractGain, 0x1);
        }
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlAcqIndGain, 0x2);
        ...
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkDirGain, 0x2);
        }*/
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkFractGain, 0x3);
        }
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkIndGain, 0x5);
        ...
        /* SctlTrkGain */
        /* SctlTrkGain forced to default value 0x83 with QamMode */
    ...
    case tmFrontEndModulationQam256:
        ...
        if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegEqEfLeakageAlpha, 0x9);
        }
        ...
        /* CtlTrkGain */
        /* Annex A/C: overwrites the default value 0x182 defined by QamMode */
        ...
        if (err == TM_OK)
        {
            ...
            {
                err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkDirGain, 0x3);
            }
        }
        /*if (err == TM_OK)
        {
        ...
        /*if (err == TM_OK)
        {
            err = iHCDP_RegWrite(pObj, HCDPRegCtlTrkIndGain, 0x6);
        }*/
    ...
    default:
        ...
        /* CtlAcqGain forced to default 0xC2 with QamMode */

        /* SctlTrkGain forced to default 0x83 with QamMode */
...
{
#ifndef TMBSL_TDA10025_NO_64BITS
...
//        ulHigh /= pObj->sConfig.uSamplingClock;
        ...
        ulHigh = div_u64(ulHigh, pObj->sConfig.uSamplingClock);   //michael chen ; support 64bit divide
    ...
    if (err == TM_OK)
    {
        err = iHCDP_SetExtendSymbolRateRange(pObj, pObj->eExtendedSRMode);
    }
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDP_GetIF(0x%08X)", tUnit);
...
)
{
    ...
    if (err == TM_OK)
    {
        if (sLockInd.bDemod && sLockInd.bFEC)
        {
            /* Get Ber */
...
/*                                                                            */
/* DESCRIPTION: Set the extended timing recovery mode                         */
/*                                                                            */
/* RETURN:      TM_OK if no error                                             */
/*                                                                            */
/*============================================================================*/
...
)
{
    ...
    if (err == TM_OK)
    {
        err = iHCDP_SetExtendSymbolRateRange( pObj, eMode );
    }
...
/*============================================================================*/
/* FUNCTION:    iHCDP_SetMFFilter                                             */
/*                                                                            */
/* DESCRIPTION: Sets the MF Filter.                                           */
/*                                                                            */
/* RETURN:      TM_OK if no error                                             */
/*                                                                            */
/*============================================================================*/
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_SetMFFilter(0x%08X)", pObj->tUnit);
...
#define MAX_BANDWITH 8000000
    if (err == TM_OK)
    {
        ...
        {
            tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_ConfigCTL(0x%08X): LowIF mode detected", pObj);
#ifndef TMBSL_TDA10025_NO_64BITS
        ...
        {
            tmDBGPRINTEx(DEBUGLVL_VERBOSE, "iHCDP_ConfigCTL(0x%08X): DirectIF mode detected", pObj);
#ifndef TMBSL_TDA10025_NO_64BITS
    ...
    if (err == TM_OK)
    {
        if (eLockStatus == tmbslFrontEndStateLocked)
        {
            ...
            if (err == TM_OK)
            {
                ...
                /* does not mean that the measurement is frozen */
...
)
{
    ...
    /* StlSweep (applicable to ES2 only) */

    if( (pObj->eHwCdpSample == HCDP_SAMPLE_ES2) && (err == TM_OK) ) /* temporary: to be adapted when ES1 is removed */
    {
        if( (eMode != HCDP_ExtendSymbolRateModeDisable) &&
            (pObj->sCurrentChannel.uSR >= 4000000)
          )
        {
        ...
        {
            /* extended mode is disable */
            /* only when the mode change */
            /* write default values */

            /* Do not program registers if extend SR is already disabled */

            if( !((eMode == HCDP_ExtendSymbolRateModeDisable) && (pObj->eExtendedSRMode == eMode)) )
            {
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_BLAB, "HCDPWrite(0x%08X)", pObj->tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_BLAB, "HCDPRead(0x%08X)", pObj->tUnit);
...
)
{
    ...
    tmDBGPRINTEx(DEBUGLVL_BLAB, "HCDPWriteBit(0x%08X)", pObj->tUnit);
...
)
{
    ...
    //tmDBGPRINTEx(DEBUGLVL_VERBOSE, "tmbslHCDPDeInit(0x%08X)", tUnit);
