...
/*============================================================================*/
/* FUNCTION:    tmbslTDA18250GetSWSettingsVersion:                           */
/*                                                                            */
...
/*============================================================================*/
tmErrorCode_t
tmbslTDA18250GetSWSettingsVersion
(
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        /* handle tmTDA18250_PowerStandbyWithLoopThroughOnAndWithXtalOn */
        switch (powerState)
        {
        case tmTDA18250_PowerStandby:
        case tmTDA18250_PowerStandbyWithXtalOn:
            /* LT set to ON <=> LT output switch opened */
            if (err == TM_OK)
            {
                err = tmddTDA18250SetpdLT(tUnit, tmddTDA18250_ON);
                tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetpdLT(0x%08X) failed.", tUnit));
            }
            break;

        case tmTDA18250_PowerNormalMode:
        case tmTDA18250_PowerStandbyWithLoopThroughOnAndWithXtalOn:
            /* LT set to OFF <=> LT output switch closed */
            if (err == TM_OK)
            {
                err = tmddTDA18250SetpdLT(tUnit, tmddTDA18250_OFF);
                tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetpdLT(0x%08X) failed.", tUnit));
            }
            break;
        ...
        if (err == TM_OK)
        {
            ...
            if (err == TM_OK)
            {
                ...
                if (pObj->BandParam[uNewBandIndex].ChannelType == tmTDA18250DigitalChannel)
                {
					...
					if (err == TM_OK)
					{
						/* Set the new Digital Path systematicaly */
...
(
    tmUnitSelect_t         tUnit,     /* I: Unit number */
    tmTDA18250IF_Level_t uIF_Level  /* I: IF Level    */
)
{
    ptmTDA18250Object_t pObj = Null;
    tmErrorCode_t       err = TM_OK;
...
/*============================================================================*/
/* FUNCTION:    tmbslTDA18250SetLT:                                           */
/*                                                                            */
/* DESCRIPTION: Sets the Loop Through to ON or OFF.                           */
/*                                                                            */
/* RETURN:      TM_OK if no error                                             */
/*                                                                            */
/* NOTES:                                                                     */
...
)
{
	...
	if (err == TM_OK)
	{
		err = tmddTDA18250SetpdLT(tUnit, (UInt8)uState);
		tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetLT(0x%08X) failed.", tUnit));
	}

	/* set RFSW_MTO_LT_RFin to OFF if we are in single master */
	if (err == TM_OK)
	{
		/* MasterSingle detection  */
		if ( (pObj->deviceType < tmTDA18250_DeviceTypeSlave) && (pObj->deviceType > tmTDA18250_DeviceTypeMasterMultiWoXTout) )
		{
			/* Master Single/Multi configuration detection */
			if ( uState == tmTDA18250_OFF )
			{
				err = tmddTDA18250SetRFSW_MTO_LT_RFin(tUnit, 0x0);
				tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetRFSW_MTO_LT_RFin(0x%08X) failed.", tUnit));
			}
			else
			{
				err = tmddTDA18250SetRFSW_MTO_LT_RFin(tUnit, 0x1);
				tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "tmddTDA18250SetRFSW_MTO_LT_RFin(0x%08X) failed.", tUnit));
			}
		}
	}
...
{   
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            pObj->BandIndex = tmTDA18250_IND_DIG_Band_1;
        }
...
{   
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            /* continuous AGC is always set in power down */
            err = tmddTDA18250SetpdAGC2a(tUnit, tmddTDA18250_PD_Activated);
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        while ( (uNewBandIndex < tmTDA18250_IND_Band_Max) && (uRF > pObj->BandParam[uNewBandIndex].RfMax) )
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            err = tmddTDA18250SetRC_GD_offset(tUnit, pObj->Std_Array[pObj->StandardMode].RC_GD_Offset);
        }
        if (err == TM_OK)
        {
            err = tmddTDA18250SetRC_GD_sign(tUnit, pObj->Std_Array[pObj->StandardMode].RC_GD_Sign);
        }        
    }
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        uRFAGC_TopIndex = 0;
        while ( (uRFAGC_TopIndex < tmTDA18250_IND_RFAGC_Top_Max) && (pObj->uRF > pObj->RFAGC_Top[uRFAGC_TopIndex].RfMax) )
...
)
{
    ...
    while ( (uIndex < tmTDA18250_Lvl_AGC2b_Index_Max) && (uAGC2b > Lvl_AGC2b[uIndex][tmTDA18250_AGC2b_Code]) )
        uIndex = uIndex + 1;
...
)
{
    ...
    while ( (uIndex < tmTDA18250_Lvl_AGC1_Index_Max) && ((Int32)uAGC1 > Lvl_AGC1[uIndex][tmTDA18250_AGC1_Code]) )
        uIndex = uIndex + 1;
...
)
{
    ...
    while ( (uIndex < tmTDA18250_Lvl_AGC1_Index_Max) && ((Int32)uAGC1 > Lvl_AGC1_MasterMulti[uIndex][tmTDA18250_AGC1_Code]) )
        uIndex = uIndex + 1;
    ...
    if (err == TM_OK)
    {
        ...
        */
        ...
        if (err == TM_OK)
        {
            ...
            {
                /* Master Single/Multi configuration detection */
                /* warning : we always consider we are in Master Single */
