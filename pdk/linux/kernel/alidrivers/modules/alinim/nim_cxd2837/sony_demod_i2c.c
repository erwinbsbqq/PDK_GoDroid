...
sony_result_t cxd2837_i2c_CommonReadRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, uint8_t* pData, uint32_t size)
{
...
//    }
    ...
    CXD2837_LOG_I2C(pDemod, result, 1, deviceAddress, &subAddress, 1);
    CXD2837_LOG_I2C(pDemod, result, 0, deviceAddress, pData, size);
...
sony_result_t cxd2837_i2c_CommonWriteRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, const uint8_t* pData, uint32_t size)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t cxd2837_i2c_CommonWriteOneRegister(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, uint8_t data)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t cxd2837_i2c_SetRegisterBits(sony_i2c_t* pI2c, uint8_t deviceAddress, uint8_t subAddress, uint8_t data, uint8_t mask)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t cxd2837_i2c_TunerGateway(void* nim_dev_priv, UINT8	tuner_address , UINT8* wdata , int wlen , UINT8* rdata , int rlen)
{
    ...
    if( (0 == wlen) && (0 == rlen) )
    {
        CXD2837_LOG(pDemod, "s%(): No data for read/write.\r\n", __FUNCTION__);
        SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
    }
	...
	if ( wlen != 0 ) //write to tuner
	{
		...
		{
			CXD2837_LOG(pDemod, "s%(): buffer[%d] is too small: Write wdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, wlen);
			SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
		}
	...
	if ( rlen != 0 ) //read from tuner
	{
		...
		{
			CXD2837_LOG(pDemod, "s%(): buffer[%d] is too small: Read rdata[%d]\r\n", __FUNCTION__, BURST_WRITE_MAX, rlen);
			SONY_TRACE_I2C_RETURN(SONY_RESULT_ERROR_ARG);
		}
