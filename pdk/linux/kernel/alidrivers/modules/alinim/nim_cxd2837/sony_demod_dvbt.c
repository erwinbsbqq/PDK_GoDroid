/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2013-11-29 09:45:32 #$
  File Revision : $Revision:: 7820 $
------------------------------------------------------------------------------*/
...
static sony_result_t SLTtoAT(sony_demod_t * pDemod)
{
     ...
     * slave     Bank    Addr   Bit     default    Value      Name
     * --------------------------------------------------------------------------------------------------------
     * <SLV-T>   18h     30h    [0]     8'h00      8'h01      OREG_FEC_RST_ENABLE1
     * <SLV-T>   18h     31h    [0]     8'h00      8'h01      OREG_FEC_RST_ENABLE2
     */
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x30, 0x01, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x31, 0x01, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
