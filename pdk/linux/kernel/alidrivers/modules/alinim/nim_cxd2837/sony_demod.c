/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2012-12-11 09:13:33 #$
  File Revision : $Revision:: 6439 $
------------------------------------------------------------------------------*/
...
#define sony_demod_ChipID        			    cxd2837_demod_ChipID
#define sony_demod_SetAndSaveRegisterBits 		cxd2837_demod_SetAndSaveRegisterBits
...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
sony_result_t cxd2837_demod_Create (sony_demod_t * pDemod,
...
#ifdef SONY_DEMOD_SUPPORT_DVBS_S2
    pDemod->serialTsClkFreqSat = SONY_DEMOD_SERIAL_TS_CLK_MID_FULL;
    pDemod->satTunerIqSense = SONY_DEMOD_SAT_IQ_SENSE_NORMAL;
...
sony_result_t cxd2837_demod_Shutdown (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t cxd2837_demod_SoftReset (sony_demod_t * pDemod)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t cxd2837_demod_SetConfig (sony_demod_t * pDemod, sony_demod_config_id_t configId, int32_t value)
{
    sony_result_t result = SONY_RESULT_OK;
    ...
    case SONY_DEMOD_CONFIG_DVBC_PER_MES:
         ...
         */
        result = sony_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x40, 0x5C, (uint8_t) (value & 0x0F), 0x0F);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_SAT_TSDATA6_DISEQC:
        {
            uint8_t tsData6Sel;
             ...
             */
            result = sony_demod_SetAndSaveRegisterBits(pDemod, pDemod->i2cAddressSLVT, 0x00, 0xA8, tsData6Sel, 0x07);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
            }
        }
    ...
    case SONY_DEMOD_CONFIG_SAT_MEAS_PERIOD1:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_RANGE);
        }
    ...
    case SONY_DEMOD_CONFIG_SAT_MEAS_PERIOD2:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_RANGE);
        }
    ...
    case SONY_DEMOD_CONFIG_SAT_MEAS_PERIOD3:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_RANGE);
        }
    ...
    case SONY_DEMOD_CONFIG_SAT_MEAS_PERIOD4:
        /* Verify range of value */
        if ((value < 0) || (value > 15)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_RANGE);
        }
...
sony_result_t cxd2837_demod_I2cRepeaterEnable (sony_demod_t * pDemod, uint8_t enable)
{
...
sony_result_t cxd2837_demod_GPIOSetConfig (sony_demod_t * pDemod, 
                                        uint8_t id, 
                                        uint8_t enable, 
...
sony_result_t cxd2837_demod_GPIORead (sony_demod_t * pDemod, 
                                   uint8_t id, 
                                   uint8_t * pValue)
{
    uint8_t rdata = 0x00;
...
sony_result_t cxd2837_demod_GPIOWrite (sony_demod_t * pDemod, 
                                    uint8_t id, 
                                    uint8_t value)
{
...
sony_result_t cxd2837_demod_ChipID (sony_demod_t * pDemod, 
...
sony_result_t cxd2837_demod_SetAndSaveRegisterBits (sony_demod_t * pDemod, 
                                                 uint8_t slaveAddress, 
                                                 uint8_t bank, 
                                                 uint8_t registerAddress,
                                                 uint8_t value,
...
sony_result_t cxd2837_demod_SetTsClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
    sony_result_t result = SONY_RESULT_OK;
    uint8_t serialTs;
...

    {
        ...
        if (system == SONY_DTV_SYSTEM_DVBT) {
            ...
            /* slave    Bank    Addr    Bit    default    Name
             * ---------------------------------------------------------------
             * <SLV-T>  10h     66h     [0]    1'b0       OREG_TSIF_PCK_LENGTH
             */
        ...
        if (system == SONY_DTV_SYSTEM_DVBC) {
            ...
            /* slave    Bank    Addr    Bit    default    Name
             * ---------------------------------------------------------------
             * <SLV-T>  40h     66h     [0]    1'b0       OREG_TSIF_PCK_LENGTH
             */
            result = sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x66, data, 0x01);
            if (result != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (result);
...
static sony_result_t SLStoSLT(sony_demod_t * pDemod)
{
    ...
    /* Select ADC clock mode and X'tal clock mode*/
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVX, 0x13, 0x00) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
