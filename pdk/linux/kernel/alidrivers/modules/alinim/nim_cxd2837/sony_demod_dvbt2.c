/*------------------------------------------------------------------------------
  ...
  Last Updated  : $Date:: 2013-10-09 13:36:16 #$
  File Revision : $Revision:: 7382 $
------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 Functions
------------------------------------------------------------------------------*/
sony_result_t sony_demod_dvbt2_Tune (sony_demod_t * pDemod,sony_dvbt2_tune_param_t * pTuneParam)
{
...
sony_result_t sony_demod_dvbt2_CheckDemodLock (sony_demod_t * pDemod,  sony_demod_lock_result_t * pLock)
{
    sony_result_t result = SONY_RESULT_OK;
...
sony_result_t sony_demod_dvbt2_CheckTSLock (sony_demod_t * pDemod, sony_demod_lock_result_t * pLock)
{
    sony_result_t result = SONY_RESULT_OK;
    ...
    *pLock = SONY_DEMOD_LOCK_RESULT_NOTDETECT;
	...
	if (unlockDetected) //unlockDetected can only remember last tuning status
	{
		if ((sync >= 6) && tslock)
		{ 
			*pLock = SONY_DEMOD_LOCK_RESULT_LOCKED; 
