...
#define ALI_DVFS_VOLT_IDX       (1.466 * 1000)
				   ...
				   int max_uV, unsigned *selector)
{
    ...
    if (min_mV < 800 || min_mV > 1600)
    {
        return -EINVAL;
    }
    
    if (max_mV < 800 || max_mV > 1600)
    {
        return -EINVAL;
    }
...
static int eta1459_regulator_probe(struct platform_device *pdev)
{
	...
	rdev = regulator_register(&eta1459_reg[pdev->id], &config);
#else
