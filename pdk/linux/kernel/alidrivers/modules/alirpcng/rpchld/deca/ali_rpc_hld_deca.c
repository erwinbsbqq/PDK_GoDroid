...
RET_CODE deca_decore_ioctl(struct deca_device *dev, UINT32 cmd, void *param1, void *param2)
{
    ...
    RPC_PARAM_CREATE(p4, PARAM_IN, PARAM_VOID, 4, NULL);

    switch(cmd)
    {
        case DECA_DECORE_INIT:
            {
                Audio_config_rpc a_config;
                struct audio_config *p = (struct audio_config *)param1;
                UINT8 buff[512];

                a_config.decode_mode = p->decode_mode;
                a_config.sync_mode   = p->sync_mode;
                a_config.sync_unit   = p->sync_unit;
                a_config.deca_input_sbm = p->deca_input_sbm;

                a_config.deca_output_sbm = p->deca_output_sbm;
                a_config.snd_input_sbm   = p->snd_input_sbm;
                a_config.pcm_sbm         = p->pcm_sbm;
                a_config.codec_id        = p->codec_id;

                a_config.bits_per_coded_sample = p->bits_per_coded_sample;
                a_config.sample_rate           = p->sample_rate;
                a_config.channels              = p->channels;
                a_config.bit_rate              = p->bit_rate;

                a_config.pcm_buf               = p->pcm_buf;
                a_config.pcm_buf_size          = p->pcm_buf_size;
                a_config.block_align           = p->block_align;

                a_config.codec_frame_size      = p->codec_frame_size;
                a_config.extradata_size        = p->extradata_size;
                a_config.extradata_mode        = p->extradata_mode;
                a_config.cloud_game_prj        = p->cloud_game_prj;

                memcpy(buff, p->extra_data, 512);
                //RPC_PARAM_UPDATE(p2, PARAM_IN, PARAM_Audio_config_rpc, sizeof(Audio_config_rpc), param1);
                RPC_PARAM_UPDATE(p2, PARAM_IN, PARAM_Audio_config_rpc, sizeof(Audio_config_rpc), &a_config);
                RPC_PARAM_UPDATE(p3, PARAM_IN, PARAM_UINT32, 4, param2);
                RPC_PARAM_UPDATE(p4, PARAM_IN, PARAM_OPAQUE, 512, buff);
                
                ret = RpcCallCompletion(RPC_deca_decore_ioctl, &p1, &p2, &p3, &p4,NULL);
                break;
            }
        ...
        case DECA_DECORE_GET_STATUS:
            RPC_PARAM_UPDATE(p2, PARAM_OUT, PARAM_Audio_decore_status_rpc, sizeof(Audio_decore_status_rpc), param1);
            RPC_PARAM_UPDATE(p3, PARAM_OUT, PARAM_UINT32, 4, param2);
            ret = RpcCallCompletion(RPC_deca_decore_ioctl, &p1, &p2, &p3, &p4, NULL); 
            break;
        default:
...
RET_CODE deca_io_control(struct deca_device * dev, UINT32 cmd, UINT32 param)
{
    ...
    switch(cmd)
    {
        ...
        case DECA_STR_PLAY:
        {
            struct ase_str_play_param *str_play_param = (struct ase_str_play_param *)param;
			
			Ase_str_play_param_rpc *rpc_str_play_param_tmp = NULL;
			if(!rpc_str_play_param_tmp)
			{
			   rpc_str_play_param_tmp = kmalloc(sizeof(Ase_str_play_param_rpc), GFP_KERNEL);
			   if(!rpc_str_play_param_tmp)
			   {
			       Log(LOG_ERR," !!! %s kmalloc error!! \n", __FUNCTION__);
				   return -1;
			   }
			   
			}
			
            rpc_str_play_param_tmp->src = str_play_param->src;
            rpc_str_play_param_tmp->len = str_play_param->len;
            rpc_str_play_param_tmp->loop_cnt = str_play_param->loop_cnt;
            rpc_str_play_param_tmp->loop_interval = str_play_param->loop_interval;
            rpc_str_play_param_tmp->async_play = str_play_param->async_play; 
            rpc_str_play_param_tmp->reserved = str_play_param->reserved;
            rpc_str_play_param_tmp->need_notify = str_play_param->need_notify;
            rpc_str_play_param_tmp->registered_cb = str_play_param->registered_cb;

            //printk("*** %s,%d, DECA_STR_PLAY src = 0x%08x, len = %d, loop_cnt = 0x%08x\n\n", __func__, __LINE__, \
                //rpc_str_play_param_tmp->src, rpc_str_play_param_tmp->len, rpc_str_play_param_tmp->loop_cnt);
            
            RPC_PARAM_UPDATE(p3, PARAM_IN, PARAM_Ase_str_play_param_rpc, \
                sizeof(Ase_str_play_param_rpc), (UINT32)rpc_str_play_param_tmp);

			ret = RpcCallCompletion(RPC_deca_io_control, &p1, &p2, &p3, NULL);

			if(rpc_str_play_param_tmp)
			{
			   kfree((void*)rpc_str_play_param_tmp);	   
			   rpc_str_play_param_tmp = NULL;
			}
...
void deca_process_pcm_samples(UINT32 pcm_bytes_len, UINT8 *pcm_raw_buf, UINT32 sample_rate, UINT32 channel_num, UINT32 sample_precision)
{
    ...
    RPC_PARAM_CREATE(p2, PARAM_IN, PARAM_UINT32, sizeof(UINT32), &pcm_raw_buf);
    ...
    UINT32 bs_length, UINT8 *un_processed_bs, UINT32 sample_rate, UINT32 channel_num, UINT32 sample_precision)
{
    ...
    RPC_PARAM_CREATE(p4, PARAM_IN, PARAM_UINT32, sizeof(UINT32), &un_processed_bs);
