...
static void vpo_hdmi_cb(UINT32 type, UINT32 uParam)
{
	if(m_vpo_cb.phdmi_callback)
		((OSAL_T_HSR_PROC_FUNC_PTR)(m_vpo_cb.phdmi_callback))(uParam);
}
...
{
    RPC_PARAM_CREATE(p1, PARAM_IN, PARAM_OPAQUE, strlen(name), name);
    RPC_PARAM_CREATE(p2, PARAM_IN, PARAM_Osd_driver_config_rpc, sizeof(Osd_driver_config_rpc), attach_config);
    RpcCallCompletion(RPC_osd_m36f_attach, &p1, &p2, NULL);
}
...
RET_CODE vpo_ioctl(struct vpo_device *dev, UINT32 dwCmd, UINT32 dwParam)
{
	...
	case VPO_IO_SET_3D_ENH:
              RPC_PARAM_UPDATE(p3, PARAM_IN, PARAM_Alifbio_set_3d_enh_rpc, sizeof(Alifbio_3d_enhance_rpc), dwParam);
    	break;
