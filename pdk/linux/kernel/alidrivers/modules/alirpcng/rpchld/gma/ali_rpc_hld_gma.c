...
void osddrv_attach(void)
{
    RpcCallCompletion(RPC_osddrv_attach,NULL);
}
...
RET_CODE gma_io_control(struct gma_device *dev, UINT32 cmd, UINT32 arg)
{
	...
	switch(cmd)
	{
		...
		case GMA_IO_SET_DBG_FLAG:
			break;
		case GMA_IO_SET_DMEM_PAR:
			RPC_PARAM_UPDATE(p3, PARAM_IN, PARAM_GMA_DMEM_PARS, sizeof(GMA_DMEM_PARS), arg);
			break;		
		default:
			return RET_FAILURE;			
	}
