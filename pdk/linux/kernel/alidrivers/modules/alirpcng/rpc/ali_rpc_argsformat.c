...
XdrOpFunc FindMatchXDRProFunc(Long paramId)
{
    XdrOpFunc pFunc = NULL;
    Int32 i=0;

    if (paramId < 0 || paramId >= PARAM_ID_MAX)
    {
        Log(LOG_ERR, "FindMatchXDRProFunc invalid paramId:%d\n", paramId);
        return NULL;
    }
    /*check if proc table same index is matched*/
    if (gXdrOpTable[paramId].id == paramId)
    {
        pFunc = gXdrOpTable[paramId].op;
    }
    else
    {
        /*Loop check the pro table*/
        ...
        {
            if (gXdrOpTable[i].id == paramId)
            {
                pFunc = gXdrOpTable[i].op;
                break;
            }
        }
    }
...
#if defined(__ALI_TDS__) && defined(CONFIG_ALI_RPCNG) && defined(CONFIG_ALI_MINIRPC)
	if((g_RPCRunning >= RPCST_MINIRPC_INITIALING)&&(g_RPCRunning <= RPCST_MINIRPC_STOPPING))
	{
...
#endif
	{
	    if ((pFunc = FindMatchXDRProFunc(paramId)) == NULL)
	    {
	        Log(LOG_ERR, "XdrProcWrap can not findout the matched XDR func, paramId:%d\n", paramId);
	        return False;
	    }
	    switch (paramId)
	    {
	        ...
	        case PARAM_ARRAY:
	            ...
	            break;
	        case PARAM_BYTES:
	            bResult = pFunc(pxdr, &pelement, &elementsize, elementsize);
	            break;
	        case PARAM_OPAQUE:
	            bResult = pFunc(pxdr, pelement, elementsize);
	            break;
	        case PARAM_STRING:
	            bResult = pFunc(pxdr, &pelement, elementsize);
	            break;
...
Int32  FormatCallerArgsEncode(Uint8 *buf, Int32 bufsize, Uint32 funcid, Int32 rid, Int32 xflag, Param *args[])
{
    ...
    //Log(LOG_DEBUG,"FormatCallerArgsEncode funcid:0x%x\n",funcid);
...
Int32  FormatCheckPacketType(Uint8 *buf, Int32 buflen, Uint32 *rid)
{
    ...
    //Log(LOG_DEBUG,"FormatCheckPacketType, before XDR_DECODE, buf[]:0x%x,0x%x,0x%x,0x%x\n",buf[0],buf[1],buf[2],buf[3]);
    memset(packet_mask, 0, 5);

    XDR_Create(pxdr, (Int8 *)buf, buflen, XDR_DECODE);

    //Log(LOG_DEBUG,"FormatCheckPacketType, XDR_DECODE I, buf[]:0x%x,0x%x,0x%x,0x%x\n",buf[0],buf[1],buf[2],buf[3]);

    if (XDR_GETBYTES(pxdr, &packet_mask, 4) == False)
    {
        return RPC_ARGS_ERR;
    }
    //Log(LOG_DEBUG,"FormatCheckPacketType, XDR_DECODE II, buf[]:0x%x,0x%x,0x%x,0x%x\n",buf[0],buf[1],buf[2],buf[3]);
    if (XDR_Uint32(pxdr, &funcid) == False)
    {
        return RPC_ARGS_ERR;
    }
    ...
    *rid = ui >> 16;
    ...
    Log(LOG_DEBUG, "FormatCheckPacketType rid:%d\n", *rid);
