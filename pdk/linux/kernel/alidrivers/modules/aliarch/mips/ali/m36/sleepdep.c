...
static inline void operate_adac(int power_down)
{
    if (power_down)
    {
        ...
        *(unsigned int *)0xb80020f8 |= 0x00073f0d;
    }
    else
...
static inline void operate_usb(int power_down)
{
	if (power_down)
	{
		...
		*(unsigned int *)0xb803D810 |= (1<<16)|(1<<15);
		*(unsigned int *)0xb8000080 |= (1<<28);	// usb reset
	}
	else
	{
		*(unsigned int *)0xb8000080 &= ~(1<<28);	// usb reset
...
static inline void operate_hdmi(int power_down)
{
    if (power_down)
    {
        ...
        *(unsigned int *)0xb800006c |= (1<<16)|(1<<17)|(1<<18);	// bit16-18 hdmi phy
    }
    else
...
static inline void operate_rxadc(int power_down)
{
    if (power_down)
    {
        __pm_reg_value.rxadc_pd_b8028421 = *(unsigned int *)0xb8028421;
        *(unsigned int *)0xb8028421 |= (1<<1)|(1<<2)|(1<<3);	// 
    }
    else
        *(unsigned int *)0xb8028421 = __pm_reg_value.rxadc_pd_b8028421;
}
...
static inline void operate_pmu(int power_down)
{
    if (power_down)
    {
        __pm_reg_value.pmu_pad_pd_b805c0c0 = *(unsigned int *)0xb805c0c0;
        *(unsigned int *)0xb805c0c0 &= ~(7<<0);	// BIT[2:0] set to 3'b000 to reduce pad driving
    }
    else
        *(unsigned int *)0xb805c0c0 = __pm_reg_value.pmu_pad_pd_b805c0c0;
}
