...
static struct i2so_see2main_ring_info *ring;


static __s32 i2so_see2main_ring_init_see
(
    void *ring_info
)
{
	...
	ret = RpcCallCompletion(RPC_i2so_see2main_ring_init_see, &p1, NULL);

	return(ret);    
}


__s32 i2so_see2main_ring_init
(
    __u8 *buf_base,
...
{	
	 ...
	 */
    ring = (struct i2so_see2main_ring_info *)buf_base;

	memset(ring, 0, sizeof(struct i2so_see2main_ring_info));

	ring->len = buf_len - sizeof(struct i2so_see2main_ring_info);
	...
	ring->buf_main = buf_base + sizeof(struct i2so_see2main_ring_info);

    ring->buf_see = MAIN_ADDR_TO_SEE_ADDR(ring->buf_main);

	//i2so_see2main_ring_init_see(MAIN_ADDR_TO_SEE_ADDR(ring));
	i2so_see2main_ring_init_see(ring);
	
	return(0);
}


__s32 i2so_see2main_ring_onoff
(
...
)
{
	...
	ret = RpcCallCompletion(RPC_i2so_see2main_ring_onoff_see, &p1, NULL);
#else
    ring->status = onoff;
#endif
...
__u32 i2so_see2main_ring_avail
(
    void
)
{
