		 ...
		 int len)
{
	...
	if (err) {
       ...
       if(ubi_check_backup_volume(ubi) && !(len % ubi->min_io_size))
        {
              loff_t addr_temp;
              ...
              for (len_temp=0; len_temp<len; len_temp += ubi->min_io_size, addr_temp +=ubi->min_io_size, buf_temp += ubi->min_io_size)
              {
                    if (ubi_backup_data_to_backup_volume(ubi, addr_temp))
                        printk("[UBI DBG] ubi_backup_data_to_backup_volume .... fail\n");                    
                    err = mtd_write(ubi->mtd, addr_temp, ubi->min_io_size, &written, buf_temp);                    
                    if (err)
                    {
                        ubi_err("error %d while writing %d bytes to PEB %d:%d, written "
                                "%zd bytes", err, ubi->min_io_size, pnum, offset, written);
                        dump_stack();
                        ubi_dump_flash(ubi, pnum, offset, ubi->min_io_size);

                    } 
                    else
                         ubi_assert(written == ubi->min_io_size);
                 }
           }
           else
           {                
