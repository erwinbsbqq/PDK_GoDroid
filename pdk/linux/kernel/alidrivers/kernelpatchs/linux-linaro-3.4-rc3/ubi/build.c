...
static int io_init(struct ubi_device *ubi)
{
  ...
  ubi->flash_size = ubi->mtd->size;
        ...
        /* UBI patch */	
	printk("[UBI DEBUG] ubi->peb_size=0x%x, ubi->peb_count=0x%x, ubi->flash_size=0x%llx\n", 
				ubi->peb_size, ubi->peb_count, ubi->flash_size);
...
int ubi_attach_mtd_dev(struct mtd_info *mtd, int ubi_num, int vid_hdr_offset)
{
	...
	ubi_notify_all(ubi, UBI_VOLUME_ADDED, NULL);
        ...
        /* UBI patch */
        ubi_bad_data_recovery(ubi);   //recovery bad data from backup volume
