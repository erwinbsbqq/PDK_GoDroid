	...
	)
{
	...
	case USB_RX_AGG_DMA:
		...
		DBG_8192C("%s: RX Aggregation DMA mode, size=%dKB, timeout=%dus\n",
			__FUNCTION__, pHalData->UsbRxAggPageCount&0xF, pHalData->UsbRxAggPageTimeout*32);
		break;
	...
	case USB_RX_AGG_MIX:
		...
		DBG_8192C("%s: RX Aggregation USB mode, size=%dKB, timeout=%dus\n",
			__FUNCTION__, (pHalData->UsbRxAggBlockCount&0xF)*4, pHalData->UsbRxAggBlockTimeout*32);
		break;
