	...
	IN PMLME_TDLS_CH_SWITCH_STRUCT	pChSwitchReq)
{
	...
	/* Target Channel */
	TDLS_InsertTargetChannel(pAd, (pOutBuffer + FrameLen), &FrameLen, pChSwitchReq->TargetChannel);

	/* Regulatory Class */
	TDLS_InsertRegulatoryClass(pAd, (pOutBuffer + FrameLen), &FrameLen,
								pChSwitchReq->TargetChannel, pChSwitchReq->TargetChannelBW);

	/* Secondary Channel Offset */
	if(pChSwitchReq->TargetChannelBW != EXTCHA_NONE)
	{
		UCHAR Channel = pChSwitchReq->TargetChannel;
		...
		else
		{
			do
			{
				UCHAR ExtCh;
				UCHAR Dir = pAd->CommonCfg.RegTransmitSetting.field.EXTCHA;
				ExtCh = TDLS_GetExtCh(Channel, Dir);
				if (TDLS_IsValidChannel(pAd, ExtCh))
				{
					TDLS_InsertSecondaryChOffsetIE(pAd, (pOutBuffer + FrameLen), &FrameLen, Dir);
					pAd->StaCfg.TdlsCurrentChannelBW = Dir;
					break;
				}
	...
	/* fill link identifier */
	LinkId = TDLS_SearchLinkId(pAd, pChSwitchReq->PeerMacAddr);
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY)
	{
		DBGPRINT(RT_DEBUG_ERROR,("TDLS - TDLS_ChannelSwitchReqAction() can not find the LinkId!\n"));
		return NDIS_STATUS_FAILURE;
	...
	if (pTDLS->bInitiator)
		TDLS_InsertLinkIdentifierIE(pAd, (pOutBuffer + FrameLen), &FrameLen, pTDLS->MacAddr, pAd->CurrentAddress);
	else
		TDLS_InsertLinkIdentifierIE(pAd, (pOutBuffer + FrameLen), &FrameLen, pAd->CurrentAddress, pTDLS->MacAddr);
...
#else
		RTMPusecDelay(pAd->StaCfg.TdlsOffChannelDelay);
#endif
		RTMP_IO_READ32(pAd, TX_REPORT_CNT, &macCfg);
