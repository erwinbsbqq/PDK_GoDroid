	...
	IN OUT	ADD_HT_INFO_IE 		*pAddHtInfo)
{
	...
	*/
	...
	{
		if (pAd->ApCliMlmeAux.NewExtChannelOffset == EXTCHA_ABOVE)
		{
	...
	do
	{
		...
		if (pMacEntry)
		{
			...
			if (pAd->ApCliMlmeAux.HtCapabilityLen != 0)
			{
				...
				{
					pAd->ApCliMlmeAux.CentralChannel = get_cent_ch_by_htinfo(pAd,
															&pAd->ApCliMlmeAux.AddHtInfo,
...
#ifdef RELEASE_EXCLUDE					
	DBGPRINT(RT_DEBUG_INFO, ("IF-apcli%d : B/M-cast frame pRxWI->WirelessCliID=%d, keyidx=%d \n", 
									pEntry->MatchAPCLITabIdx, pRxWI->RxWIWirelessCliID, pRxWI->RxWIKeyIndex));
#endif /* RELEASE_EXCLUDE */
