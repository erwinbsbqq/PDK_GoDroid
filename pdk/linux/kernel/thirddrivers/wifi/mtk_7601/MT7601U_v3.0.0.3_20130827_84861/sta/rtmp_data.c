...
//---Add by shiangf for debug
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
		...
		if (((pEntry = FindMeshEntry(pAd, pRxWI->RxWIWirelessCliID, pHeader->Addr2)) != NULL)
		    && MESH_ON(pAd)) {
	...
	if (pRxWI->RxWIPhyMode == MODE_VHT)
	{
		...
		if (pHeader->FC.Type == BTYPE_DATA)
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Receive receive RxD - BSSIDX0-2 = %d, BSSIDX3 = %d, Decrypted = %d, U2M = %d, MyBss = %d !!!\n", 
										pRxWI->RxWIBSSID, pRxInfo->BssIdx3, pRxInfo->Decrypted, pRxInfo->U2M, pRxInfo->MyBss));
			hex_dump("RxPacket", (UCHAR *)pHeader, pRxWI->RxWIMPDUByteCnt);	
		}
#endif
	...
	IN PNDIS_PACKET pPacket)
{
	...
	DBGPRINT(RT_DEBUG_LOUD,
		 ("RTMPSendPacket --> pSrcBufVA == %p, SrcBufLen=%d\n",
		  pSrcBufVA, SrcBufLen));
#endif /* RELEASE_EXCLUDE */
	...
	if (pAd->StaCfg.PSPXlink) {
		...
		if (pAd->TxSwQueue[QueIdx].Number >= pAd->TxSwQMaxLen) {
			...
			InsertTailQueueAc(&pAd->TxSwQueue[QueIdx],
...
#ifdef RELEASE_EXCLUDE
			DBGPRINT(RT_DEBUG_INFO,
				 ("I/F(mesh%d) STASendPacket --> Drop packet before mesh secured !!!\n",
				  pEntry->MatchMeshTabIdx));
#endif /* RELEASE_EXCLUDE */
			RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
			return (NDIS_STATUS_FAILURE);
		}
...
/*	RTMP_SET_PACKET_RTS(pPacket, 0); */
	...
	if (pAd->StaCfg.bFragFlag == TRUE) {
		...
		NdisGetSystemUpTime(&Now);
#if 0
		/*if ((pAC_Queue2->Number != 0) && (Now > (*pAC_Jiffies2)+1000)) */
#endif
...
#if defined(MESH_SUPPORT) || defined(SOFT_ENCRYPT) || defined(XLINK_SUPPORT) || defined(CLIENT_WDS) || defined(TXBF_SUPPORT)
		if ( bDoHdrTrans == FALSE )
		STA_AMPDU_Frame_Tx(pAd, pTxBlk);
		else
#endif /* defined(MESH_SUPPORT) || defined(SOFT_ENCRYPT) || defined(XLINK_SUPPORT) || defined(CLIENT_WDS) || defined(TXBF_SUPPORT) */
