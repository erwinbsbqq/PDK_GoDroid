...
INT Set_AutoProvision_Enable_Proc(
    IN  PRTMP_ADAPTER   pAd, 
    IN  PSTRING          arg);

INT Set_ChangeMode_Enable_Proc(
    IN  PRTMP_ADAPTER	pAd, 
    IN  PSTRING			arg);

#ifdef WAC_SUPPORT
INT Set_WACEnable_Proc(    
	IN  PRTMP_ADAPTER   pAd,
	IN  PSTRING         arg);

INT Set_DoAutoWAC_Proc(
	IN  PRTMP_ADAPTER   pAd,
	IN  PSTRING         arg);
#endif /* WAC_SUPPORT */
...
INT Set_WscVendorExtension_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_WscPriorityCtrl_Proc(
	IN  PRTMP_ADAPTER   pAd,
	IN  PSTRING         arg);

INT Set_WscPbcExtraScanCount_Proc(
	IN  PRTMP_ADAPTER   pAd,
	IN  PSTRING         arg);
...
static struct {
	...
	{"ATEIQCOMP",				Set_ATE_IQ_Compensation_Proc},
#endif /* IQ_CAL_SUPPORT */
#ifdef RTMP_TEMPERATURE_COMPENSATION
	{"ATETSSICBAEX",			Set_ATE_READ_EXTERNAL_TSSI_Proc},
    ...
    IN	PRTMP_ADAPTER	pAd)
{
	...
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 21, &regBBP);
	DBGPRINT(RT_DEBUG_ERROR, ("\n>>>>> Reset BBP\nRead:\nR21:%02x\n", regBBP));
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 21, 3);
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 21, &regBBP);
	DBGPRINT(RT_DEBUG_ERROR, ("Write:\nR21:%02x\n", regBBP));
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 21, 0);	
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 21, &regBBP);
	DBGPRINT(RT_DEBUG_ERROR, ("R21:%02x\n<<<<< Reset BBP\n", regBBP));
	return TRUE;
}
...
/*							kfree(pMsgElem); */
		...
		case RT_OID_WSC_AUTO_PROVISION_WITH_BSSID:
			if (wrq->u.data.length != ((MAC_ADDR_LENGTH*2)+2))
                Status = -EINVAL;
            else
            {
	...
	IN	ULONG					Data)
{
	...
	else
    {
        ...
        switch (pIoctlSec->Alg) {
			...
			DBGPRINT(RT_DEBUG_ERROR,("KCK Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			pEntry->PTK[0],pEntry->PTK[1],pEntry->PTK[2],pEntry->PTK[3],pEntry->PTK[4],pEntry->PTK[5],pEntry->PTK[6],pEntry->PTK[7],pEntry->PTK[8],pEntry->PTK[9],pEntry->PTK[10],pEntry->PTK[11],pEntry->PTK[12],pEntry->PTK[13],pEntry->PTK[14],pEntry->PTK[15]));
            ...
            case RT_CMD_STA_IOCTL_SECURITY_ALG_KEK:
            	{
            		...
            		RTMPMoveMemory(&pEntry->PTK[LEN_PTK_KCK], pIoctlSec->pData, pIoctlSec->length);
			DBGPRINT(RT_DEBUG_ERROR,("KEK Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			pEntry->PTK[16],pEntry->PTK[17],pEntry->PTK[18],pEntry->PTK[19],pEntry->PTK[20],pEntry->PTK[21],pEntry->PTK[22],pEntry->PTK[23],pEntry->PTK[24],pEntry->PTK[25],pEntry->PTK[26],pEntry->PTK[27],pEntry->PTK[28],pEntry->PTK[29],pEntry->PTK[30],pEntry->PTK[31]));
            	}
                break;                
#endif /* NEW_WOW_SUPPORT*/
#endif /* WPA_SUPPLICANT_SUPPORT */
...
#if 0
	// Alan test for new RSSI formula.
	{
		...
		if (pAd->LatchRfRegs.Channel > 14)
			RssiOffset = pAd->ARssiOffset[0];
		else
			RssiOffset = pAd->BGRssiOffset[0];

		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R55, &r55);
		LNA = r55 >> 6;
		printk("LNA = %d, RSSI=%x\n", LNA, r55 & 0x3F);
		...
		else
		{
			...
			RSSI = 8 - LNA - (r55 & 0x3F) - LNAGain - RssiOffset;
