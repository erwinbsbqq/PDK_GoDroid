...
#ifdef P2P_SUPPORT
	if ((P2P_GO_ON(pAd) || (pApCliEntry->Valid == TRUE)) && ((pAd->CommonCfg.Channel != pAd->MlmeAux.Channel) || (pAd->CommonCfg.Channel != pAd->LatchRfRegs.Channel)))
	{
		bChangeInitBW = TRUE;
	}
#ifdef CONFIG_MULTI_CHANNEL
	if (P2P_CLI_ON(pAd) && ( pAd->ApCliMlmeAux.Channel != pAd->MlmeAux.Channel) && (pAd->Multi_Channel_Enable == TRUE))
	{
...
#if 0  /* Fix getting nothing in scan result of ra0 when p2p0 is in forced GO mode */
#ifdef P2P_SUPPORT
	if (P2P_GO_ON(pAd))
		return;
#endif /* P2P_SUPPORT */
								...
								pVIE)) 
	{
		...
		*/
		...
		DBGPRINT(RT_DEBUG_INFO, ("SYNC - PeerBeacon from %02x:%02x:%02x:%02x:%02x:%02x - Dtim=%d/%d, Rssi=%02x\n", 
			PRINT_MAC(ie_list->Bssid), ie_list->DtimCount, ie_list->DtimPeriod, RealRssi));
#endif /* RELEASE_EXCLUDE */
...
/*			CFG80211_BeaconCountryRegionParse(pAd, pVIE, LenVIE); */
			...
			DBGPRINT(RT_DEBUG_INFO, ("SYNC - my beacon from %02x:%02x:%02x:%02x:%02x:%02x - Dtim=%d/%d, Rssi=%d, Rssi2=%d, Rssi3=%d\n", 
				PRINT_MAC(ie_list->Bssid), ie_list->DtimCount, ie_list->DtimPeriod, 
				pAd->StaCfg.RssiSample.LastRssi0, pAd->StaCfg.RssiSample.LastRssi1, 
...
#ifdef DOT11_VHT_AC
{
			...
			if (INFRA_ON(pAd))
			{
				...
				*/
				...
				ACMP_BandwidthInfoSet(pAd, ie_list->QbssLoad.StaNum,
										ie_list->QbssLoad.ChannelUtilization,
	...
	if (PeerProbeReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, Ssid, &SsidLen, NULL))
	{
		...
		{			
			...
			Privacy = (pAd->StaCfg.WepStatus == Ndis802_11Encryption1Enabled) || 
					  (pAd->StaCfg.WepStatus == Ndis802_11Encryption2Enabled) || 
					  (pAd->StaCfg.WepStatus == Ndis802_11Encryption3Enabled);
			CapabilityInfo = CAP_GENERATE(0, 1, Privacy, (pAd->CommonCfg.TxPreamble == Rt802_11PreambleShort), pAd->CommonCfg.bUseShortSlotTime, 0);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	*/
	...
	else
	{
		...
		{
			count_scan++;
			if ((count_scan > pAd->p2p_l2sd_scan_toggle) && (pAd->MlmeAux.ScanType == SCAN_PASSIVE))
			{
				count_scan = 0;
				pAd->MlmeAux.Channel = 0;
				/*
					Go back to channel 1, set P2P DISC SM to be LISTEN for receiving Probe Req from Peer.
				*/
			...
			{
				if (pAd->MlmeAux.ScanType == SCAN_PASSIVE && pAd->MlmeAux.OldChannel!=0)
				{
				...
				if (pAd->MlmeAux.ScanType == SCAN_PASSIVE)
				{
