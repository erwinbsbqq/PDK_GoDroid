	...
	IN UCHAR			BandWidth)
{
	...
	/* In ATE mode, we always disable BBP tuning */
/*
...
*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RT3593ATERxVGAInit - Ch=%d, BandWidth=%d, LNA_GAIN=0x%x, set R66 as 0x%x \n", pAd->LatchRfRegs.Channel, BandWidth, GET_LNA_GAIN(pAd), R66));
}
#else
VOID RT3593ATERxVGAInit(
	IN PRTMP_ADAPTER		pAd)
