	...
	IN UINT32 TxIdx)
{
	...
	if (pATEInfo->bQATxStart == TRUE) 
	{
		...
		ATEWriteTxWI(pAd, pTxWI, pATEInfo->TxWI.TxWIFRAG, pATEInfo->TxWI.TxWICFACK,
			pATEInfo->TxWI.TxWITS, pATEInfo->TxWI.TxWIAMPDU, pATEInfo->TxWI.TxWIACK,
			pATEInfo->TxWI.TxWINSEQ, pATEInfo->TxWI.TxWIBAWinSize, 0,
			pATEInfo->TxWI.TxWIMPDUByteCnt, pATEInfo->TxWI.TxWIPacketId, 0, 0,
			pATEInfo->TxWI.TxWITXOP/*IFS_HTTXOP*/, pATEInfo->TxWI.TxWICFACK/*FALSE*/,
			&TxHTPhyMode);
