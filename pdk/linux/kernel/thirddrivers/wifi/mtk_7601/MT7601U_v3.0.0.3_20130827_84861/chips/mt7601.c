...
#ifdef RTMP_PCI_SUPPORT
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R4, BBPValue);
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R178, 0xFF);
#else
		AndesBBPRandomWrite(pAd, 2,
			BBP_R4, BBPValue,
			BBP_R178, 0xFF);
	...
	*/
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST) == FALSE)
	{
		if (Level == RTMP_HALT)
		{
			...
			*/
            #if 1
            DBGPRINT(RT_DEBUG_ERROR, ("%s: Disable MAC TX/RX!!!\n", __FUNCTION__));
            #endif
			RTMP_IO_READ32(pAd, MAC_SYS_CTRL, &MacReg);
			MacReg &= ~(0x0000000c);
			RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL, MacReg);
		...
		else
		{
			...
			*/
            #if 1
            DBGPRINT(RT_DEBUG_ERROR, ("%s: Disable MAC RX Only!!!\n", __FUNCTION__));
            #endif
			RTMP_IO_READ32(pAd, MAC_SYS_CTRL, &MacReg);
			MacReg &= ~(0x00000008);
			//MacReg &= ~(0x0000000c);
