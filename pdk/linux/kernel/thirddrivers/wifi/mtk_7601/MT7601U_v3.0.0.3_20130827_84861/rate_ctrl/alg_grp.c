    ...
    IN ULONG idx) 
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,(
			"   QuickDRS:Aid=%d, TxSuccess=%ld, TxRetransmit=%ld, TxFail=%ld, RSSI=%d\n",
			pEntry->Aid, TxSuccess, TxRetransmit, TxFailCount, Rssi));
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA, (
			"DRS:Aid=%d, TxSuccess=%ld, TxRetransmit=%ld, TxFail=%ld, rssi=[%d, %d, %d]\n",
			pEntry->Aid, TxSuccess, TxRetransmit, TxFailCount,
			pEntry->RssiSample.AvgRssi0, pEntry->RssiSample.AvgRssi1, pEntry->RssiSample.AvgRssi2));
