...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,("DRS: Aid=%d, APMlmeSetTxRate - CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, BW=%d \n\n",
			pEntry->Aid,
	...
	IN RTMP_RA_LEGACY_TB *pTxRate)
{
	...
	if (!pAd || !pEntry || !pTxRate || !pEntry->pTable || (pEntry->EntryType==0))
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: Invalid parameters!\n", __FUNCTION__));
		return;
	}
#ifdef DOT11_N_SUPPORT
...
#ifdef RELEASE_EXCLUDE
					DBGPRINT_RAW(RT_DEBUG_ERROR,("DRS: unkown mode,default use 11N 3S AP \n"));	
#endif /* RELEASE_EXCLUDE */
#else
					*ppTable = RateSwitchTable11N2S;
#ifdef RELEASE_EXCLUDE
