...
/*	if (ACMR_IS_ENABLED(pAd))*/
	{
		...
		{
#ifdef DOT11_N_SUPPORT
			FlgIs2040 = pTxWI->TxWIBW;
			FlgIsShortGI = pTxWI->TxWIShortGI;
#else
			FlgIs2040 = 0;
			FlgIsShortGI = 0;
#endif /* DOT11_N_SUPPORT */
			...
			if (ACMP_PacketPhyModeMCSCheck(
							...
							FlgIsShortGI,
							pTxWI->TxWIPHYMODE,
							pTxWI->TxWIMCS) != ACM_RTN_OK)
			{
				pTxWI->TxWIPHYMODE = RTMP_GET_PACKET_ACM_MIN_PHY_MODE(pTxBlk->pPacket);
				pTxWI->TxWIMCS = RTMP_GET_PACKET_ACM_MIN_PHY_MCS(pTxBlk->pPacket);

#ifdef DOT11_N_SUPPORT
				pTxWI->TxWIShortGI = 0; /* force to use regular GI */
#endif /* DOT11_N_SUPPORT */
			}
		}
	}
