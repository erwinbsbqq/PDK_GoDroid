...
#ifdef WFD_SUPPORT
	UCHAR	WFDOUIBYTE[4] = {0x50, 0x6f, 0x9a, 0xa};
	UCHAR *pWFDSubelement = &ie_lists->WFDSubelement[0];
#endif /* WFD_SUPPORT */
#endif /* P2P_SUPPORT */
#ifdef DOT11R_FT_SUPPORT
...
#ifdef WFD_SUPPORT
				if (NdisEqualMemory(eid_ptr->Octet, WFDOUIBYTE, sizeof(WFDOUIBYTE)) && (eid_ptr->Len >= 4))
				{
					if (ie_lists->WFDSubelementLen == 0)
					{
						RTMPMoveMemory(pWFDSubelement, &eid_ptr->Eid, (eid_ptr->Len+2));
						ie_lists->WFDSubelementLen = (eid_ptr->Len+2);
					}
					else if (ie_lists->WFDSubelementLen > 0)
					{
						RTMPMoveMemory(pWFDSubelement + ie_lists->WFDSubelementLen, &eid_ptr->Eid, (eid_ptr->Len+2));
						ie_lists->WFDSubelementLen += (eid_ptr->Len+2);
					}

					DBGPRINT(RT_DEBUG_TRACE, (" ! ===>WFD - PeerAssocReqSanity  WFD IE Len becomes = %d.\n", ie_lists->WFDSubelementLen));
					break;
				}
#endif /* WFD_SUPPORT */
#endif /* P2P_SUPPORT */
