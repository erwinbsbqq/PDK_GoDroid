...
#ifdef HUAWEI_ATE
#ifdef CONFIG_AP_SUPPORT
	{"ATEFILTER",				Set_ATE_MAC_Filter_Proc},
	{"ATEFILTERENABLE",			Set_ATE_MAC_Filter_Enable_Proc},
#endif /* CONFIG_AP_SUPPORT */
#endif /* HUAWEI_ATE */
#ifdef TXBF_SUPPORT
...
/*logMsg("Address=%08x\n", ((UINT32) pAd->CSRBaseAddress + macAddr), 0,0,0,0,0);*/
		/* MAC Registers */
	...
	IN  PSTRING arg)
{
	...
	if(strlen(arg) <= MAX_LEN_OF_SSID)
	{
					...
					pAd->ApCfg.ApCliTab[ifIndex].PSKLen > 0)
		{
...
#if 0
			if ((strlen((PSTRING) PskKey) >= 8) && (strlen((PSTRING) PskKey) < 64))
			{
			...
			else if ((INT) strlen((PSTRING) PskKey) == 64)
			{
				AtoH((PSTRING) PskKey, pAd->ApCfg.ApCliTab[ifIndex].PMK, 32);
			}						
#else
	...
	IN	PSTRING			arg)
{
	...
	if (pMacEntry)
	{
		pMacEntry->AuthMode = Ndis802_11AuthModeWAIPSK;
