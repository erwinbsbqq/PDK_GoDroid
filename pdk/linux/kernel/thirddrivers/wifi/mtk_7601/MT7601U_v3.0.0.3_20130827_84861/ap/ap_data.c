...
NDIS_STATUS APSendPacket(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket)
{
	...
	if (IS_ENTRY_CLIENT(pMacEntry) || (Wcid == MCAST_WCID))
	{
		...
		if (pMbss != NULL)
		{
				...
				)
			{
				...
				{
#ifdef RELEASE_EXCLUDE
					DBGPRINT(RT_DEBUG_INFO, ("I/F(ra%d) APSendPacket --> Drop M/B-cast packet before AP secured !!!\n", apidx));
#endif /* RELEASE_EXCLUDE */
					RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
					return NDIS_STATUS_FAILURE;			
				}
				...
				{
#ifdef RELEASE_EXCLUDE
					DBGPRINT(RT_DEBUG_INFO, ("I/F(ra%d) APSendPacket --> Drop unicast packet before AP secured !!!\n", apidx));
#endif /* RELEASE_EXCLUDE */
					RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
					return NDIS_STATUS_FAILURE;	
				}
	...
	*/
	...
	else if (RTMP_GET_PKT_CSO(pPacket) || 
...
#if 0 /* merge it into RTMPCheckEtherType(). */
	/*
		STEP 3. Traffic classification. outcome = <UserPriority, QueIdx>
	*/
	UserPriority = 0;
	QueIdx		 = QID_AC_BE;
	if (((pMbss->bWmmCapable) || OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_WMM_INUSED)) &&
		...
		do
		{
			...
			*/
			if (
				...
				(pAd->CommonCfg.APEdcaParm.bACM[QueIdx])
				)
			{
				UserPriority = 0;
				QueIdx		 = QID_AC_BE;
...
#endif /* BLOCK_NET_IF */
#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_INFO, ("UCAST SwQ full, drop it!\n"));
#endif /* RELEASE_EXCLUDE */
				RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
				return NDIS_STATUS_FAILURE;			
...
static inline VOID APBuildCommon802_11Header(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	*/
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bMeshEntry))
	{
		...
		COPY_MAC_ADDR(wifi_hdr->Addr2, pAd->MeshTab.CurrentAddress);				/* from AP1 */
		COPY_MAC_ADDR(wifi_hdr->Addr3, pTxBlk->pMeshDA);
		if (RTMP_GET_MESH_SOURCE(pTxBlk->pPacket) == MESH_PROXY)
			COPY_MAC_ADDR(&wifi_hdr->Octet[0], pAd->MeshTab.CurrentAddress);
		else
...
#if defined(MESH_SUPPORT) || defined(SOFT_ENCRYPT) || defined(WDS_SUPPORT) || defined(TXBF_SUPPORT)
				if ( pTxBlk->NeedTrans == FALSE )
					AP_AMPDU_Frame_Tx(pAd, pTxBlk);
				else
#endif /* defined(MESH_SUPPORT) || defined(SOFT_ENCRYPT) || defined(WDS_SUPPORT) || defined(TXBF_SUPPORT) */
				{
					//DEBUG_POINT1
...
//		hex_dump("Class2ErrPkt", (UCHAR *)pHeader, sizeof(HEADER_802_11));
		...
		if (pEntry)
		{
			if ((pEntry->Sst == SST_ASSOC) && IS_ENTRY_CLIENT(pEntry))
			{
				...
				offset = MAC_WCID_BASE + (pEntry->Aid * HW_WCID_ENTRY_SIZE);
				DBGPRINT(RT_DEBUG_OFF, ("Invalid One: Check if ASIC WCID table(0x%x) has this entry (Aid=%d)!\n",
									offset, pEntry->Aid));
				
				for (j=0; j<2; j++)
				{
					RTMP_IO_READ32(pAd, offset + (j * 4), &ent_mac[j]);
				}
				DBGPRINT(RT_DEBUG_OFF, ("Invalid One: SaveMAC in WCID=0x%x - 0x%x\n", ent_mac[0], ent_mac[1]));
#endif
			}
			return FALSE;
		}
...
//+++Add by shiang for debug
		if (pRxWI->RxWIWirelessCliID >= MAX_LEN_OF_MAC_TABLE) {
...
#if 0
				DBGPRINT(RT_DEBUG_OFF, ("Dump Cls2ErrPktMetaData\n"));
				...
				hex_dump("RxD", (UCHAR *)&pRxBlk->hw_rx_info[0], sizeof(RXD_STRUC));
				dump_rxd(pAd, &pRxBlk->hw_rx_info[0]);
				dumpRxFCEInfo(pAd, pRxBlk->pRxFceInfo);
				dump_rxinfo(pAd, pRxBlk->pRxInfo);

				DBGPRINT(RT_DEBUG_TRACE, ("Dump the RxWI and RxPacket(len=%d):\n", pRxWI->RxWIMPDUByteCnt));
				dump_rxwi(pAd, pRxWI);
				hex_dump("RxCls2ErrPacket", (UCHAR *)pHeader, pRxWI->RxWIMPDUByteCnt > 32 ? 32 : pRxWI->RxWIMPDUByteCnt);
		...
		else if (dump_next_valid)
		{
				...
				offset = MAC_WCID_BASE + (pRxWI->RxWIWirelessCliID * HW_WCID_ENTRY_SIZE);
				DBGPRINT(RT_DEBUG_OFF, ("Valid One: Check if ASIC WCID table(0x%x) has this entry (Aid=%d)!\n",
									offset, pRxWI->RxWIWirelessCliID));
				
				for (j=0; j<2; j++)
				{
					RTMP_IO_READ32(pAd, offset + (j * 4), &ent_mac[j]);
				}
				DBGPRINT(RT_DEBUG_OFF, ("Valid One: SaveMAC in WCID=0x%x - 0x%x\n", ent_mac[0], ent_mac[1]));
#endif
#ifdef RLT_MAC
#ifndef MT7601
...
//---Add by shiangf for debug
	...
	*/
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		do
		{
			...
			if (VALID_WCID(pRxWI->RxWIWirelessCliID)
				&& IS_ENTRY_MESH(&pAd->MacTab.Content[pRxWI->RxWIWirelessCliID]))
			{
				bMeshPacket = TRUE;
				pEntry = FindMeshEntry(pAd, pRxWI->RxWIWirelessCliID, pHeader->Addr2);
...
#if 1 // add for beacon lost patch for apcli
                        else //not only beacon but also unicast data
                        {
	...
	if (RTMP_SA_WORK_ON(pAd))
	{
		...
		if (pRxWI->RxWIRSSI0)
		{
			pEntry->curRSSI[0] += (pRxWI->RxWIRSSI0 & 0xff);
			pEntry->cntRSSI[0]++;
		}
		else
		{
			pEntry->rssi_zero_cnt[0]++;
			printk("Err!Receive A frame with RSSI0=%d!TotalZeroCnt=%d!\n", pRxWI->RxWIRSSI0, pEntry->rssi_zero_cnt[0]);
		}
		if (pRxWI->RxWIRSSI1)
		{
			pEntry->curRSSI[1] += (pRxWI->RxWIRSSI1 & 0xff);
			pEntry->cntRSSI[1]++;
		}
		else
		{
			pEntry->rssi_zero_cnt[1]++;
			printk("Err!Receive A frame with RSSI1=%d!TotalZeroCnt=%d!\n", pRxWI->RxWIRSSI1, pEntry->rssi_zero_cnt[1]);
		}

#ifdef DOT11N_SS3_SUPPORT		
		if (pRxWI->RxWIRSSI2)
		{
			pEntry->curRSSI[2] += (pRxWI->RxWIRSSI2 & 0xff);
			pEntry->cntRSSI[2]++;
		}
		...
		if (pRxWI->RxWISNR0)
		{
			pEntry->sumSNR[0] += (pRxWI->RxWISNR0 & 0xff);
			pEntry->cntSNR[0]++;
		}
		if (pRxWI->RxWISNR1)
		{
			pEntry->sumSNR[1] += (pRxWI->RxWISNR1 & 0xff);
			pEntry->cntSNR[1]++;
		}
#ifdef DOT11N_SS3_SUPPORT
		if (pRxWI->RxWISNR2)
		{
			pEntry->sumSNR[2] += (pRxWI->RxWISNR2 & 0xff);
			pEntry->cntSNR[2]++;
		}
		...
		if (pRxWI->BF_SNR0)
		{
			pEntry->sumPreSNR[0] += (pRxWI->BF_SNR0 & 0xff);
			pEntry->cntPreSNR[0]++;
		}
		if (pRxWI->BF_SNR1)
		{
			pEntry->sumPreSNR[1] += (pRxWI->BF_SNR1 & 0xff);
			pEntry->cntPreSNR[1]++;
		}
		if (pRxWI->BF_SNR2)
		{
			pEntry->sumPreSNR[2] += (pRxWI->BF_SNR2 & 0xff);
			pEntry->cntPreSNR[2]++;
		}
#endif /* DOT11N_SS3_SUPPORT */
		pEntry->saRxCnt++;
		...
		{
			RtmpSATrainInfoUpdate(pAd, pTrainEntry, pRxWI, pSAParam->txNss);
		}
...
#endif /* MT7601 */
		RssiSample = ConvertToRssi(pAd, (UCHAR)pRxWI->RxWIRSSI0, RSSI_0, 0, 0);
...
#if 0 //def DOT11_VHT_AC
	if (pRxWI->RxWIPhyMode == MODE_VHT)
	{
		DBGPRINT(RT_DEBUG_TRACE, ("RxPkt with MODE:VHT,MCS:%d,BW:%d,WCID:%d, BA:%d\n",
										pRxWI->RxWIMCS, pRxWI->RxWIBW, pRxWI->RxWIWirelessCliID, pRxInfo->BA));
	}
#endif /* DOT11_VHT_AC */
...
#ifdef ED_MONITOR
		if (pAd->ed_chk)
			pAd->rx_cnt[pAd->ed_stat_lidx]++;
#endif /* ED_MONITOR */

#if 0
		if (pHeader->FC.Type == BTYPE_DATA)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("Receive MAC - %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pHeader->Addr2)));
			DBGPRINT(RT_DEBUG_TRACE, ("Receive receive RxD - BSSIDX0-2 = %d, BSSIDX3 = %d, Decrypted = %d, U2M = %d, MyBss = %d !!!\n", 
										pRxWI->RXWI_N.bss_idx, pRxInfo->BssIdx3, pRxInfo->Decrypted, pRxInfo->U2M, pRxInfo->MyBss));
			hex_dump("RxPacket", (UCHAR *)pHeader, pRxWI->RxWIMPDUByteCnt);	
		}
#endif
	...
	IN UCHAR QueIdx)
{
...
#endif /* UAPSD_SUPPORT */
	{
		...
		DBGPRINT(RT_DEBUG_INFO,
			("STA (AID=%d) in PSM, move to PSQ, Psqueue #=%ld\n",
			pMacEntry->Aid, pMacEntry->PsQueue.Number));
#endif /* RELEASE_EXCLUDE */
	}
	return NDIS_STATUS_SUCCESS;
}
