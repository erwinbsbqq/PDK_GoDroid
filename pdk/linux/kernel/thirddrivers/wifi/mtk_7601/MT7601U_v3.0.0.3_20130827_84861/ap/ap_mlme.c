			...
			(pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode < Ndis802_11AuthModeWPA)))
		{
	            ...
	            (pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscConfMode != WSC_DISABLE))
	        {
	            *Machine = WSC_STATE_MACHINE;
				...
				if ((EAPType == EAPOLStart) &&
					pEntry->EnqueueStartForWSCTimerRunning != WSC_EAPOL_START_DISABLE)
				{	 
					BOOLEAN Cancelled;
					
					DBGPRINT(RT_DEBUG_TRACE, ("Receive WSC EAPOL-Start frame, TYPE = 1 \n"));
					DBGPRINT(RT_DEBUG_TRACE, ("Cancel the EnqueueStartForWSCTimerRunning \n"));
					RTMPCancelTimer(&pEntry->EnqueueStartForWSCTimer, &Cancelled);
					pEntry->EnqueueStartForWSCTimerRunning = WSC_EAPOL_START_DISABLE;			 
				}				
...
/*	DBGPRINT(RT_DEBUG_TRACE, ("TX_LINK_CFG = %08x\n", TxLinkCfg.word)); */
