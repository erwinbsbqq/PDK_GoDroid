...
#define MIN_NET_DEVICE_FOR_DLS			(MIN_NET_DEVICE_FOR_MESH + 0x20)
#define MIN_NET_DEVICE_FOR_TDLS			(MIN_NET_DEVICE_FOR_MESH + 0x50)
#endif /* MESH_SUPPORT */
#endif /* CONFIG_STA_SUPPORT */
...
/* [CB_OFF+15] ~ [CB_OFF+16], 2B, are reserved for ACM use */
/* reference to ACM_PACKET_INFO in acm_extr.h */
#define RTMP_GET_PACKET_ACM_PARAM_INFO(_p)		(&(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+15]))

/* [CB_OFF+17] ~ [CB_OFF+18], 4B, are reserved for minimum tx rate use */
#define RTMP_SET_PACKET_ACM_MIN_PHY_MODE(_p, _mode)	(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+17] = _mode)
#define RTMP_GET_PACKET_ACM_MIN_PHY_MODE(_p)		(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+17])
#define RTMP_SET_PACKET_ACM_MIN_PHY_MCS(_p, _mcs)	(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+18] = _mcs)
#define RTMP_GET_PACKET_ACM_MIN_PHY_MCS(_p)			(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+18])

/* [CB_OFF+23], 1B, are reserved for TSPEC TSID use */
#define RTMP_SET_PACKET_STM_TSID(_p, _tsid)		(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+23] = _tsid)
#define RTMP_GET_PACKET_STM_TSID(_p)			(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+23])

/* [CB_OFF+24] ~ [CB_OFF+27], 4B, are reserved for tx time use */
#define RTMP_SET_PACKET_TX_TIME(_p, _txtime)	(*(UINT32 *)&RTPKT_TO_OSPKT(_p)->cb[CB_OFF+24] = _txtime)
#define RTMP_GET_PACKET_TX_TIME(_p)				(*(UINT32 *)&RTPKT_TO_OSPKT(_p)->cb[CB_OFF+24])
#endif /* WMM_ACM_SUPPORT */

/* use bit0 of cb[CB_OFF+19] so other bits (7~1) can be used for other functions */
#ifdef CONFIG_AP_SUPPORT
#define RTMP_SET_PACKET_MGMT_PKT(_p, _flg)	        \
        RTPKT_TO_OSPKT(_p)->cb[CB_OFF+19] &= 0xFE;  \
        RTPKT_TO_OSPKT(_p)->cb[CB_OFF+19] |= (_flg&0x01);
#define RTMP_GET_PACKET_MGMT_PKT(_p)		(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+19] & 0x01)
