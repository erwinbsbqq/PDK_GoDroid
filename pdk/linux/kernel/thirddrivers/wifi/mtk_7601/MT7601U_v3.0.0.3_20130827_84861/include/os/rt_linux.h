...
#ifdef RTMP_MAC_USB
#ifdef SEPARATE_PROFILE_SUPPORT
#define STA_PROFILE_PATH            "/etc/Wireless/RT2870STA/RT2870STA_MT7601U.dat"
#else
#define STA_PROFILE_PATH			"/etc/Wireless/RT2870STA/RT2870STA.dat"
#endif /*SEPARATE_PROFILE_SUPPORT */
...
#ifdef SINGLE_SKU_V2
#ifdef SEPARATE_PROFILE_SUPPORT
#define SINGLE_SKU_TABLE_FILE_NAME  "/etc/Wireless/RT2870STA/SingleSKU_MT7601U.dat"
#else
...
#define RTMP_SET_WNM_DMS(_p, _flg)   (RTPKT_TO_OSPKT(_p)->cb[CB_OFF+29] = _flg)
#define RTMP_GET_WNM_DMS(_p)         (RTPKT_TO_OSPKT(_p)->cb[CB_OFF+29])
...
#define RTMP_SET_PKT_CSO(_p, _flg)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) |= ((_flg) << 0))
#define RTMP_GET_PKT_CSO(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) & NAF_TYPE_CSO)
#define RTMP_SET_PKT_TSO(_p, _flg)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) |= ((_flg)<<1))
#define RTMP_GET_PKT_TSO(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) & NAF_TYPE_TSO)
#define RTMP_GET_PKT_NAF_TYPE(_p)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) & (NAF_TYPE_CSO | NAF_TYPE_TSO))

#define RTMP_SET_PKT_TCP(_p, _flg)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) |= ((_flg) << 2))
#define RTMP_GET_PKT_TCP(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) & (1<<2))

#define RTMP_SET_PKT_SG(_p, _flg)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) |= ((_flg) << 3))
#define RTMP_GET_PKT_SG(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+31]) & (1<<3))

#define RTMP_SET_PKT_TUPS(_p, _tups)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+32]) = (_tups))
#define RTMP_GET_PKT_TUPS(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+32]))

#define RTMP_SET_PKT_IPS(_p, _ips)	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+33]) = (_ips))
#define RTMP_GET_PKT_IPS(_p)		((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+33]))

#define RTMP_SET_PKT_MSS(_p, _mss)	\
	do {\
		(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+34]) = ((_mss) & 0xff);\
		(RTPKT_TO_OSPKT(_p)->cb[CB_OFF+35]) = ((_mss) >> 8);\
	}while(0)

#define RTMP_GET_PKT_MSS(_p)	\
	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+34]) | \
	((RTPKT_TO_OSPKT(_p)->cb[CB_OFF+35]) << 8))
...
#define BULKAGGRE_SIZE 	        BUF_ALIGMENT_RINGSIZE * 16    /*default packet size = 2k , so * 16 mean 32k*/
#else
...
#else
#define RTUSB_URB_ALLOC_BUFFER(_dev, _size, _dma)	usb_buffer_alloc(_dev, _size, GFP_KERNEL, _dma)
