	...
	IN M_BLK_ID     pMblk)
{
	...
	return MBSS_VirtualIF_PacketSend((PNDIS_PACKET)pNewMblk, (PNET_DEV)pEndObj);
	...
	IN M_BLK_ID     pMblk)
{
	...
	return ApCli_VirtualIF_PacketSend((PNDIS_PACKET)pNewMblk, (PNET_DEV)pEndObj);
 ...
 */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		 	...
		 	&& (!(pPktSrcAddr[0] & 0x1)))
		{
			...
			/*IMPORT LIST endList; */
			long	endIfType;
			PSTRING	pDevNameBuf = NULL, token, pDevNameList = BRIDGE_PORTS, pUnitNum;
			STRING  DevName[END_NAME_MAX] = {0};
			BOOLEAN bOK = TRUE;
			INT		devNameLen;
