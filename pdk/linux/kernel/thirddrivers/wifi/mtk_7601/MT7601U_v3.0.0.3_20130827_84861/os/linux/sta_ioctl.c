...
INT Set_MlmeWNMFMSReq_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_MlmeWNMSleepModeReq_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_MlmeWNMTFSReq_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);
...
#ifdef    CONFIG_ZHA_WPA_PATCH
	iq->level = pBss->ChannelQuality;
#else
...
/*	pObj = (POS_COOKIE) pAd->OS_Cookie; */
    ...
    switch(subcmd)
    {
		...
		case SHOW_CFG_VALUE:
			{
				Status = RTMPShowCfgValue(pAd, (PSTRING) wrq->pointer, extra, IW_PRIV_SIZE_MASK);
				if (Status == 0)
					wrq->length = strlen(extra) + 1; /* 1: size of '\0' */
			}
			break;
