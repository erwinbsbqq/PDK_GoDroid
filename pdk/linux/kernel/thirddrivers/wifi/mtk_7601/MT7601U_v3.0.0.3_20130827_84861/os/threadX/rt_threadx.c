	...
	LIST_HEADER				*pSemList)
{
	...
	*/
	/*pTxTaskInstance = kmalloc(sizeof(RTMP_OS_TASK), GFP_KERNEL); */
	...
	IN	CHAR			Rssi)
{
	...
	case IW_SCAN_COMPLETED_EVENT_FLAG:
		if (pSyControl->SYWiFiAPScanTrigger)
			SYScanCompletedEventHdlr(pAd, BssIdx);
		break;
	case IW_WPS_END_EVENT_FLAG:
		if (pSyControl->SYWiFiWPSTrigger)
			SYWPSEndEventHdlr(pAd, BssIdx);
		break;

	case IW_WSC_NEXT_CANDIDATE:
		if (pSyControl->SYWiFiWPSTrigger)
			SYNextCanidatedEventHdlr(pAd, BssIdx);
		break;
	case IW_WSC_STATUS_FAIL:
		if (pSyControl->SYWiFiWPSTrigger)
			SYWSCStatusFailEventHdlr(pAd, BssIdx);
		break;
	case IW_STA_LINKUP_EVENT_FLAG:
		if (pSyControl->SYWiFiWPSTrigger)
			SYSTALinkUpEventHdlr(pAd, BssIdx);
		break;
