...
#ifdef RLT_MAC
		reg_cidx = TX_RING_CIDX;
		reg_didx = TX_RING_DIDX;
#endif /* RLT_MAC */
#ifdef RTMP_MAC
		reg_cidx = TX_CTX_IDX0;
		reg_didx = TX_DTX_IDX0;
#endif /* RTMP_MAC */
...
#ifdef RLT_MAC
		RTMP_IO_READ32(pAd, RX_RING_DIDX, &reg);
#endif /* RLT_MAC */
#ifdef RTMP_MAC
...
#endif /* RT3290 */
#ifdef RT65xx
	if (device_id == NIC6590_PCIe_DEVICE_ID ||
		device_id == NIC8592_PCIe_DEVICE_ID ||
...
#ifdef RT65xx
	if (IS_RT65XX(pAd)) {
		// TODO: what shall we do for RT6590
	} else
#endif /* RT8592 */
#ifdef MT7601
	if (IS_MT7601(pAd)) {
