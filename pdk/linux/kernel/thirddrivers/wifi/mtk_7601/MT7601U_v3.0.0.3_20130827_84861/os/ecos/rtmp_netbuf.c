	...
	if (pEcosNetPktCb->pNetPktPool[0])
	{
		...
		for (i = 0; i < RTMP_NET_PKT_POOL_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosNetPktFreeList,
				(PLIST_ENTRY)(pEcosNetPktCb->pNetPktPool[0] + i));
		...
		IN PRTMP_ADAPTER pAd)
{
	...
	pEcosNetPktCb->pNetPktPool[CurNetPktPoolIdx] =
			kmalloc(sizeof(ECOS_NET_PKT_POOL_ENTRY) * RTMP_NET_PKT_POOL_SIZE, GFP_ATOMIC);

	if (pEcosNetPktCb->pNetPktPool[CurNetPktPoolIdx])
	{
		NdisZeroMemory(pEcosNetPktCb->pNetPktPool[CurNetPktPoolIdx],
			sizeof(ECOS_NET_PKT_POOL_ENTRY) * RTMP_NET_PKT_POOL_SIZE);
		initList(&pEcosNetPktCb->EcosNetPktFreeList);
		for (i = 0; i < RTMP_NET_PKT_POOL_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosNetPktFreeList,
				(PLIST_ENTRY)(pEcosNetPktCb->pNetPktPool[CurNetPktPoolIdx] + i));
	...
	IN PECOS_NET_PKT_POOL_ENTRY pNetPkEntry)
{
	...
	insertTailList(&pEcosNetPktCb->EcosNetPktFreeList, (PLIST_ENTRY)pNetPkEntry);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pEcosNetPktCb->pNetRxBufPool[0])
	{
		...
		for (i = 0; i < RTMP_NET_BUF_POOL_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosNetRxBufFreeList, (PLIST_ENTRY)(pEcosNetPktCb->pNetRxBufPool[0] + i));
	...
	IN PECOS_NET_PKT_POOL_ENTRY pNetBufEntry)
{
	...
	insertTailList(&pEcosNetPktCb->EcosNetRxBufFreeList, (PLIST_ENTRY)pNetBufEntry);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pEcosNetPktCb->pNetTxBufPool[0])
	{
		...
		for (i = 0; i < RTMP_NET_BUF_POOL_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosNetRxBufFreeList, (PLIST_ENTRY)(pEcosNetPktCb->pNetTxBufPool[0] + i));
	...
	IN PECOS_NET_PKT_POOL_ENTRY pNetBufEntry)
{
	...
	insertTailList(&pEcosNetPktCb->EcosNetTxBufFreeList, (PLIST_ENTRY)pNetBufEntry);
	...
	if (pEcosNetPktCb->pTxDPool[0])
	{
		...
		for (i = 0; i < TX_RING_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosTxDFreeList,
				(PLIST_ENTRY)(pEcosNetPktCb->pTxDPool[0] + i));
	...
	IN PECOS_TXD_POOL_ENTRY pTxDEntry)
{
	...
	insertTailList(&pEcosNetPktCb->EcosTxDFreeList, (PLIST_ENTRY)pTxDEntry);
	...
	if (pEcosNetPktCb->pRxDPool[0])
	{
		...
		for (i = 0; i < RX_RING_SIZE; i++)
			insertTailList(&pEcosNetPktCb->EcosRxDFreeList,
				(PLIST_ENTRY)(pEcosNetPktCb->pRxDPool[0] + i));
	...
	IN PECOS_TXD_POOL_ENTRY pRxDEntry)
{
	...
	insertTailList(&pEcosNetPktCb->EcosRxDFreeList, (PLIST_ENTRY)pRxDEntry);
	...
	if (pEcosNetPktCb->p1stTxBufPool[0])
	{
		...
		for (i = 0; i < TX_RING_SIZE; i++)
			insertTailList(&pEcosNetPktCb->Ecos1stTxBufFreeList,
				(PLIST_ENTRY)(pEcosNetPktCb->p1stTxBufPool[0] + i));		
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s Fail to alloc memory for pAd->eCos_Net_Pkt_Ctrl.pTxDPool",
	...
	IN PECOS_TXD_POOL_ENTRY p1stTxBufEntry)
{
	...
	insertTailList(&pEcosNetPktCb->Ecos1stTxBufFreeList, (PLIST_ENTRY)p1stTxBufEntry);
