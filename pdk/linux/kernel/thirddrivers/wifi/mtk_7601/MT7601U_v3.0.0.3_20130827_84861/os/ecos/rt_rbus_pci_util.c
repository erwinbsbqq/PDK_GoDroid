	...
	OUT	PNDIS_PHYSICAL_ADDRESS PhysicalAddress)
{
	...
	RTMP_SET_PACKET_SOURCE(OSPKT_TO_RTPKT(pPacket), PKTSRC_NDIS);
	...
	IN	UINT			DataLen)
{
	...
	if (pPacket == NULL)
	{
		*ppPacket = NULL;
        ...
        pPacket->pDataMBuf = (struct mbuf *)RTMP_MemPool_Alloc(pAd, (HeaderLen + DataLen), MemPool_TYPE_MBUF);
	if (pPacket->pDataMBuf == NULL)
	{
		DBGPRINT(RT_DEBUG_WARN, ("%s: Can't allocate buffer from memory pool\n"
		, __FUNCTION__));
	...
	IN	UCHAR			*TPID)
{
    ...
    Status = RTMPAllocateNdisPacket(pAd, &pPacket, NULL, 0, NULL, (HdrLen + DataSize + LENGTH_802_1Q + 2));
	...
	IN	ULONG			DataSize)
{
	...
	pClonedMblk = (PECOS_PKT_BUFFER) skb_clone(pPacket, MEM_ALLOC_FLAG);
	if (pClonedMblk != NULL)
	{
        /* set the correct dataptr and data len */
        pClonedMblk->pDataPtr = pClonedMblk->pDataPtr + (pData - GET_OS_PKT_DATAPTR(pPacket));
        pClonedMblk->pktLen = DataSize;
		ASSERT(DataSize < 1530);
	}
