...
/*	MiniportMMRequest(pAd, pkt->data, pkt->len); */
	...
	else if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS) ||
...
#ifdef CONFIG_STA_SUPPORT
		bReschedule = STARxDoneInterruptHandle(pAd);
#endif /* CONFIG_STA_SUPPORT */
...
static void ac3_dma_done_process(unsigned long data)
{
	 ...
	 * double check to avoid lose of interrupts
	 */
	if (pAd->int_pending & INT_AC3_DLY) {
...
static void ac2_dma_done_process(unsigned long data)
{
	 ...
	 * double check to avoid lose of interrupts
	 */
	if (pAd->int_pending & INT_AC0_DLY) {
...
static void ac1_dma_done_process(unsigned long data)
{
	 ...
	 * double check to avoid lose of interrupts
	 */
	if (pAd->int_pending & INT_AC1_DLY) {
...
void APTxDataIntProcess(PRTMP_ADAPTER pAd, INT_SOURCE_CSR_STRUC TxRingBitmap)
{
	unsigned long flags;
