	...
	IN void *_dev_p)
{
	...
	{
		if (dev_p->descriptor.idVendor == rtusb_dev_id[i].idVendor &&
			dev_p->descriptor.idProduct == rtusb_dev_id[i].idProduct)
		{
	...
	pm_message_t state)
{
	...
	UCHAR InfUpFlag;
	if(!RTMP_DRIVER_ADAPTER_START_UP_TEST(pAd, &InfUpFlag))
	{
		DBGPRINT(RT_DEBUG_ERROR, ("[%s]Wireless network interface is not started up!\n",__FUNCTION__));
		return 0;
	}
...
#define ALI_MAC_ADDR    0xa3ffb000
unsigned char *g_macaddr = NULL;
#endif
...
INT __init rtusb_init(void)
{
	printk("rtusb init %s --->\n", RTMP_DRV_NAME);
#ifdef CONFIG_ALI_ZHA_MAC_PATCH
    g_macaddr = (unsigned char*)kmalloc(6, GFP_KERNEL);
    memcpy(g_macaddr, (UINT8 *) ALI_MAC_ADDR, 6);
    printk("g_macaddr: %02x  %02x %02x %02x %02x %02x \n", g_macaddr[0], g_macaddr[1],g_macaddr[2],
...
#if 0 /* os abl move to os/linux/rt_usb.c */
/*
========================================================================
Routine Description:
    MLME kernel thread.
