	...
	OUT	PNDIS_PHYSICAL_ADDRESS PhysicalAddress)
{
	...
	if (Cached)  {
		...
		*VirtualAddress = (PVOID) KSEG1ADDR((u32)p);
	}
	*PhysicalAddress = (NDIS_PHYSICAL_ADDRESS) CPHYSADDR((u32)p);
}
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (skb != NULL)
	{
		...
		net_pkt_put(GET_OS_PKT_TYPE(skb), HdrLen);
	...
	IN  UCHAR			FromWhichBSSID)
{
	...
	pOSPkt->dev = pNetDev; /*get_netdev_from_bssid(pAd, FromWhichBSSID); */
