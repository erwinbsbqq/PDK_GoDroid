								    ...
								    OUT DC_WIFI_AP_ENCR *pSYAPEncr)
{
	...
	{
		if (*pRTWepStatus == Ndis802_11Encryption2Enabled)
			*pSYAPEncr = DC_WIFI_AP_ENCR_WPAPSK_TKIP;
		else if (*pRTWepStatus == Ndis802_11Encryption3Enabled)
								  ...
								  OUT NDIS_802_11_WEP_STATUS *pRTWepStatus)
{
	...
	case DC_WIFI_IFACE_ENCR_WEP128:
		...
		{
			*pRTAuthMode = Ndis802_11AuthModeOpen;
			*pRTWepStatus = Ndis802_11Encryption1Enabled;
		}
	...
	case DC_WIFI_IFACE_ENCR_WPAPSK_TKIP:
		...
		*pRTWepStatus = Ndis802_11Encryption2Enabled;	
		break;
		
	case DC_WIFI_IFACE_ENCR_WPAPSK_AES:
		*pRTAuthMode = Ndis802_11AuthModeWPAPSK;
		*pRTWepStatus = Ndis802_11Encryption3Enabled;
		break;
...
static inline int SYReconnectSSID(IN PRTMP_ADAPTER pAd, 
								  IN PSTRING SSID)
{
	int ret;
...
SY_WPS_PIN_CONTINUE_START:
	...
	while (pSyControl->StopWpsAPSearch != TRUE && APIndex < pSyControl->NumAPsFound)
	{
		...
		/* SSID */
		Set_WscSsid_Proc(pAd, (PSTRING)SYCandidate.SSID);
...
void wifi_getWifiStatus(OUT DC_WIFI_STATUS *structP)
{
	...
	structP->rssi = (INT32)(pAd->StaCfg.RssiSample.LastRssi0 - pAd->BbpRssiToDbmDelta);
