	...
	IN	ULONG	Length)
{
	...
	if (pNetBuf)
	{
		RTMP_SET_PACKET_SOURCE(OSPKT_TO_RTPKT(pNetBuf), PKTSRC_NDIS);
	}
	else
	{
	...
	OUT	PVOID	*VirtualAddress)
{
	...
	RTMP_SET_PACKET_SOURCE(OSPKT_TO_RTPKT(pPkt), PKTSRC_NDIS);
	...
	IN	UINT			DataLen)
{
	...
	RTMP_SET_PACKET_SOURCE(pNetBuf, PKTSRC_NDIS);
...
PNDIS_PACKET duplicate_pkt(
	...
	IN	UCHAR			FromWhichBSSID)
{
	RTMP_RTIP_NET_BUF *pMblk;
	POS_COOKIE		pObj;
	PNDIS_PACKET	pPacket = NULL;
	PIFACE pi;
	
	pMblk = RtmpTxNetBufGet(HdrLen + DataSize, 0, NULL);
	if (pMblk != NULL)
	{
		pi = (struct _iface RTP_FAR *)pNetDev; /*get_netdev_from_bssid(pAd, FromWhichBSSID); */
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (pMblk != NULL)
	{
		...
		pi = (PIFACE)get_netdev_from_bssid(pAd, FromWhichBSSID);
		SET_OS_PKT_NETDEV(pMblk, pi);
	...
	IN	ULONG			DataSize)
{
	...
	pi = GET_OS_PKT_NETDEV(pPacket);
	pClonedNetBuf = skb_clone(pPacket, MEM_ALLOC_FLAG);
	if (pClonedNetBuf != NULL)
	{
    		...
    		/* set the correct dataptr and data len */
	    	pClonedNetBuf->pDataPtr = pClonedNetBuf->pFrameBuf + (pData - GET_OS_PKT_DATAPTR(pPacket));
    		pClonedNetBuf->pktLen = DataSize;
		ASSERT(DataSize < 1530);
	}
...
/* TODO: For ThreadX, shall we add this features?? */
#if 0
#ifdef RT2880
