...
//			SET_ENTRY_NONE(pEntry);
			...
			if (IS_P2P_GO_ENTRY(pEntry))
			{
				...
				if (pEntry->bP2pClient)
				{
					...
					//if(pAd->LinkedP2pOnGo & 0x01)
					//	pAd->LinkedP2pOnGo &= ~0x01;
					//else if(pAd->LinkedP2pOnGo )//&& pAdapter->LinkedP2pOnGo |=0x01;//lw++)
					//pAd->LinkedP2pOnGo -=0x2;//lw++
					DBGPRINT(RT_DEBUG_ERROR, ("Lwadd::RT_P2P_AP_STA_DISCONNECTED!\n"));

				if(pAd->LinkedP2p0DevNumber)
				...
				{
				#ifdef GET_P2PGO_LINKED_DEVICE
				//if(pAd->LinkedLegacyOnGo)
				//pAd->LinkedLegacyOnGo -=1;//lw++
				DBGPRINT(RT_DEBUG_ERROR, ("Lwadd::RT_P2P_LEGACY_DISCONNECTED!\n"));
