...
#ifdef HE_SUPPORT
						P2pSendWirelessEvent(pAd, RT_P2P_STOP_CONNECT, NULL,  pP2pEntry->addr);
#endif /* HE_SUPPORT */
...
#ifdef WIFI_P2P_DEVICENAME_IN_PERSISTAB
	IndexInClient = P2pGroupTabSearch(pAd, pAddr);
	DBGPRINT(RT_DEBUG_ERROR, ("Perst::IndexInClient = %d \n", IndexInClient));
#endif

#ifdef WIFI_P2P_DEVICENAME_IN_PERSISTAB
           UCHAR NewDeviceNameLen = 0;
           
           // check if update on device name only
           if (pProfile == NULL)
           {
                     // check for update after 2nd connection only.
                     if (index < MAX_P2P_TABLE_SIZE)
                     {
                                i = index;
                                NewDeviceNameLen = Tab->Client[IndexInClient].DeviceNameLen;
                                
                                /* update P2P device name */
                                if ((Tab->Client[IndexInClient].DeviceName != NULL) && \
                                           (NewDeviceNameLen != 0 ) && (NewDeviceNameLen <= P2P_DEVICE_NAME_LEN))
                                {
                                          // check if there is update on device name
                                           if (!NdisEqualMemory(Tab->PerstEntry[i].DeviceName, Tab->Client[IndexInClient].DeviceName, NewDeviceNameLen))
                                           {
                                                     //clear device name
                                                RTMPZeroMemory(Tab->PerstEntry[i].DeviceName, P2P_DEVICE_NAME_LEN);
                                                     //update device name
                                                     RTMPMoveMemory(Tab->PerstEntry[i].DeviceName, Tab->Client[IndexInClient].DeviceName, NewDeviceNameLen);
                                                     Tab->PerstEntry[i].DeviceName[NewDeviceNameLen] = 0x0;

                                                     DBGPRINT(RT_DEBUG_ERROR, ("PerstEntry: update DeviceName=%s, len %d\n",Tab->PerstEntry[i].DeviceName,NewDeviceNameLen));
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
#ifdef HE_SUPPORT
                                                     Tab->bInsPerTable = TRUE;
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
		...
		{		
			if (Tab->PerstEntry[i].bValid == FALSE)
			{
				...
				DBGPRINT(RT_DEBUG_ERROR, ("Perst::Registra MacAddr = %x %x %x %x %x %x \n",Tab->PerstEntry[i].Addr[0], Tab->PerstEntry[i].Addr[1], Tab->PerstEntry[i].Addr[2],Tab->PerstEntry[i].Addr[3],Tab->PerstEntry[i].Addr[4],Tab->PerstEntry[i].Addr[5]));
                         	...
                         	if (Tab->Client[IndexInClient].DeviceNameLen < P2P_DEVICE_NAME_LEN)
					RTMPMoveMemory(Tab->PerstEntry[i].DeviceName, Tab->Client[IndexInClient].DeviceName, Tab->Client[IndexInClient].DeviceNameLen);
                            	else
                         		RTMPMoveMemory(Tab->PerstEntry[i].DeviceName, Tab->Client[IndexInClient].DeviceName, P2P_DEVICE_NAME_LEN);
				DBGPRINT(RT_DEBUG_ERROR, ("Tab->PerstEntry[i].DeviceName=%s\n",Tab->PerstEntry[i].DeviceName));
#endif
	...
	IN	PUCHAR Addr)
{
	...
	switch (MsgType)
	{
		...
		name='p2p-TEST1' config_methods=0x188 dev_capab=0x27 group_capab=0x0 */
			...
			if (!NdisEqualMemory(AllZero, pP2pEntry->InterfaceAddr, MAC_ADDR_LEN))
				RTMPMoveMemory(p2p_all_info.peer, pP2pEntry->InterfaceAddr, MAC_ADDR_LEN);
			else
			{
				if (!NdisEqualMemory(AllZero, p2p_all_info.addr, MAC_ADDR_LEN))
					RTMPMoveMemory(p2p_all_info.peer, p2p_all_info.addr, MAC_ADDR_LEN);
				else
					RTMPMoveMemory(p2p_all_info.peer, p2p_all_info.dev_addr, MAC_ADDR_LEN);
			}
			//p2p_all_info.WpaPsk[64];//we cannot decide wpapsk information at this stage.
			p2p_all_info.P2PChannel = pP2pEntry->OpChannel;
			...
			if (pP2pEntry->WfdEntryInfo.bWfdClient)
			{
				...
				p2p_all_info.coupled_sink_info.CoupledStat = pP2pEntry->WfdEntryInfo.coupled_sink_info.CoupledStat;
				RTMPMoveMemory(p2p_all_info.coupled_sink_info.CoupledPeerAddr, pP2pEntry->WfdEntryInfo.coupled_sink_info.CoupledPeerAddr, MAC_ADDR_LEN);
			}
#endif /* WFD_SUPPORT */
				...
				else
				{
					...
					if (p2pIdx < MAX_P2P_GROUP_SIZE) {
						...
						if (pP2pEntry->WfdEntryInfo.bWfdClient)
						{
							...
							p2p_all_info.coupled_sink_info.CoupledStat = pAd->P2pTable.Client[p2pIdx].WfdEntryInfo.coupled_sink_info.CoupledStat;
							RTMPMoveMemory(p2p_all_info.coupled_sink_info.CoupledPeerAddr, pAd->P2pTable.Client[p2pIdx].WfdEntryInfo.coupled_sink_info.CoupledPeerAddr, MAC_ADDR_LEN);
						}
					}
					DBGPRINT(RT_DEBUG_TRACE, ("%s::RTSP Port: %d, device_type: %d\n", __FUNCTION__, p2p_all_info.rtsp_port, p2p_all_info.wfd_devive_type));
#endif /* WFD_SUPPORT */
		...
		case RT_P2P_AP_STA_CONNECTED:
			...
			*/
			...
			if (pP2pEntry != NULL) {
				...
				p2p_ap_sta_connected.bInsPerTable = pAd->P2pTable.bInsPerTable;
				//p2p_ap_sta_connected.bInsPerTable = pP2pEntry->bInsPerTable;
				if (pAd->P2pTable.bInsPerTable == TRUE)
					pAd->P2pTable.bInsPerTable = FALSE;
#ifdef WFD_SUPPORT
