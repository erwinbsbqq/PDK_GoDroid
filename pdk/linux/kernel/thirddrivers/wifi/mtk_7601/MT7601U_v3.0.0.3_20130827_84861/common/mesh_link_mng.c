	...
	do
	{
		...
		else
		{
				...
				WMODE_CAP_N(pAd->CommonCfg.PhyMode))
			{
				...
				/* find max fixed rate */
				pEntry->MaxHTPhyMode.field.MCS = get_ht_max_mcs(pAd, &pAd->MeshTab.DesiredHtPhyInfo.MCSSet[0],
				...
				if (pAd->MeshTab.DesiredTransmitSetting.field.MCS != MCS_AUTO)
				{
					...
					set_ht_fixed_mcs(pAd, pEntry, pAd->MeshTab.DesiredTransmitSetting.field.MCS, pAd->MeshTab.HTPhyMode.field.MCS);
				}
