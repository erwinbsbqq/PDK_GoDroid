                    ...
                    *pChannel = *pEid->Octet;
					...
					IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
					{
						if (ChannelSanity(pAd, *pChannel) == 0)
						{
							...
							DBGPRINT(RT_DEBUG_INFO, ("%s() - wrong IE_DS_PARM (ch=%d)\n",__FUNCTION__,*pChannel));
...
#ifdef RELEASE_EXCLUDE
                DBGPRINT(RT_DEBUG_INFO, ("IE_WAPI length = %d\n", pEid->Len));
#endif /* RELEASE_EXCLUDE */
                break;
#endif /* WAPI_SUPPORT */
...
#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_INFO, ("%s() - IE_EXT_CAPABILITY(%d)!\n", __FUNCTION__, pEid->Len));
#endif /* RELEASE_EXCLUDE */
				break;
			}
#endif
...
SanityCheck:
	...
	{
#ifdef WIDI_SUPPORT
		DBGPRINT(RT_DEBUG_ERROR, ("%s Sanity Failed\n", __FUNCTION__));
#endif /* WIDI_SUPPORT */
    ...
    IN UCHAR channel)
{
    ...
    for (i = 0; i < pP2PCtrl->ChannelListNum; i ++)
    {
	        ...
	        case IE_VENDOR_SPECIFIC:
					...
					)
                {
					...
					*/
					...
					if (pEntry == NULL)
						pEntry = MacTableInsertEntry(pAd, pAddr2, BSS0, OPMODE_STA, FALSE);

					if (pEntry)
					{
						pEntry->bRaAutoWpsCapable = FALSE;
						if (WpsMICIntegrity(pAd, eid_data, (INT)eid_len, pEntry->Addr, pEntry->apidx))
						{
							BOOLEAN Cancel;							
							if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
							{
						...
						DBGPRINT(RT_DEBUG_WARN, ("MacTable Insert Fail!\n"));
#endif /* CONFIG_STA_SUPPORT */
