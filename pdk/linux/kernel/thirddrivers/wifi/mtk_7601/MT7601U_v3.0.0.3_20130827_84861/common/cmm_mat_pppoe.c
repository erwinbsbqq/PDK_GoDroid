	...
	IN UINT16			isServer)
{
	...
	if (hashIdx < MAT_MAX_HASH_ENTRY_SUPPORT)
	{
		...
		while(pEntry)
		{
    		...
    		{
#if 0
				DBGPRINT(RT_DEBUG_TRACE,("%s(): Got the Mac(%02x:%02x:%02x:%02x:%02x:%02x) with tagInfo:\n",
						__FUNCTION__, pEntry->macAddr[0],pEntry->macAddr[1],pEntry->macAddr[2],
						pEntry->macAddr[3],pEntry->macAddr[4],pEntry->macAddr[5])); 
				for(i=0; i< tagLen; i++)
					DBGPRINT(RT_DEBUG_TRACE,("%02x ", pUIDStr[i] & 0xff));
...
#if 0
	DBGPRINT(RT_DEBUG_TRACE,("%s():needUpdateSesTb=%d, payloadLen=%d, findTag=0x%04x\n",
							__FUNCTION__, needUpdateSesTb, payloadLen, findTag));
	DBGPRINT(RT_DEBUG_TRACE,("%s():Dump pppoe payload==>\n", __FUNCTION__));
...
#if 0
	DBGPRINT(RT_DEBUG_TRACE,("%s():isServer=%d, payloadLen=%d, findTag=0x%04x\n", 
