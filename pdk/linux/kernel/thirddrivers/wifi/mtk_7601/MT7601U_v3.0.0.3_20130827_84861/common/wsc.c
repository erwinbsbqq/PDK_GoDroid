    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
    ...
    /*For normaol WSC, we enqueue an EAPOL-Start command to trigger the process.*/
#ifdef P2P_SUPPORT
#ifdef ENQUEUE_WPS_EAPOL_START
    if (Elem->MsgLen == MAC_ADDR_LENGTH)
    {
...
{		
...
#ifdef HE_SUPPORT
				if (pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI)
					P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_PBC_SESSION_OVERLAP, NULL, NULL);
#endif /* HE_SUPPORT */
	...
	else if (MsgType == WSC_MSG_EAP_FAIL)
	{
		...
		if (pWscControl->WscState >= WSC_STATE_WAIT_EAPFAIL)
		{
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("%s : WscStatus=%d \n", __FUNCTION__, pWscControl->WscStatus));
			...
			(WscCheckNonce(pAdapter, Elem, FALSE, pWscControl) || WscCheckNonce(pAdapter, Elem, TRUE, pWscControl)))
		{
...
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
#ifdef HE_SUPPORT
			if ((pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI) && pEntry)
			{
				P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_RECEIVE_NACK, NULL, NULL);
			}
#endif /* HE_SUPPORT */
#endif
			/*
			...
			*/
			...
			else if ((CurOpMode == STA_MODE) && INFRA_ON(pAdapter))
			{
				...
				{
					DBGPRINT(RT_DEBUG_TRACE, ("NOW WSC STATUS = %d\n", pWscControl->WscStatus));
				}
#endif /* DPA_T */
			}
#endif /* CONFIG_STA_SUPPORT */
...
Fail:
    ...
    {          
		...
		if ((pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI) && pEntry)
		{
				...
				{
					PRT_P2P_CLIENT_ENTRY pP2pEntry = &pAdapter->P2pTable.Client[P2pIdx];
					pP2pEntry->StatusCode = rv;
					P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_FAIL, pP2pEntry, pEntry->Addr);
				}
		}
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
...
#ifdef WIDI_SUPPORT
			DBGPRINT(RT_DEBUG_TRACE, ("WscEapEnrolleeAction : WPS Successful; ifIdx = %d\n", pWscControl->EntryIfIdx));
#endif /* WIDI_SUPPORT */
			...
			if (CurOpMode == STA_MODE)
			{
				...
				if (pWscControl->WscDriverAutoConnect != 0)
				{
						...
						(pCredential->EncrType == WSC_ENCRTYPE_NONE)))
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("WscEapEnrolleeAction : Done but AuthType == WSC_AUTHTYPE_OPEN EncrType == WSC_ENCRTYPE_NONE\n"));
						DBGPRINT(RT_DEBUG_TRACE,("pAd->MlmeAux.Ssid=%s\n",pAdapter->MlmeAux.Ssid));
					}
					else
#endif /* DPA_T */
					{
...
/*	if( (WscData = kmalloc(WSC_MAX_DATA_LEN, GFP_ATOMIC)) == NULL) */
	...
	switch (MsgType)
	{
				...
				else
				{
					...
					*/
					...
					if (bSendM2D)
					{
						...
						if ((pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI) && pEntry)
						{
							P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_SEND_M2D, NULL, NULL);
						}
#endif /* HE_SUPPORT */
#endif
					}
		...
		case WSC_MSG_WSC_DONE:
			...
			{
#ifdef P2P_SUPPORT
				MAC_TABLE_ENTRY PersistentEntry;
				RT_P2P_CLIENT_ENTRY P2pClientEntry;
				memset(&PersistentEntry, 0, sizeof(MAC_TABLE_ENTRY));
				memset(&P2pClientEntry, 0, sizeof(RT_P2P_CLIENT_ENTRY));
				UCHAR P2pTabClientIdx = P2P_NOT_FOUND;
				/* 	Backup this pEntry which will be used in later Persistent Table storage.
					Because sometimes after sending EapFail, peer could send de-auth very quickly,
					causing pEntry been cleared before Persistent storing.
				*/
				if (pEntry)
				{            
					memcpy(&PersistentEntry, pEntry, sizeof(MAC_TABLE_ENTRY));
					P2pTabClientIdx = P2pGroupTabSearch(pAdapter, pEntry->Addr);
					if (P2pTabClientIdx != P2P_NOT_FOUND)
						memcpy(&P2pClientEntry, &pAdapter->P2pTable.Client[P2pTabClientIdx], sizeof(RT_P2P_CLIENT_ENTRY));
				}
#endif /* P2P_SUPPORT */
#ifdef CONFIG_AP_SUPPORT
				if (CurOpMode == AP_MODE)
				...
				P2pSendWirelessEvent(pAdapter, RT_P2P_WPS_COMPLETED, NULL, NULL);
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
				if (IS_PERSISTENT_ON(pAdapter) && !isZeroEtherAddr(PersistentEntry.Addr) && (PersistentEntry.bP2pClient == TRUE))
				{
					pEntry = &PersistentEntry;
					...
					if ((P2pTabClientIdx != P2P_NOT_FOUND) && (IS_P2P_GO_ENTRY(pEntry) || IS_P2P_CLI_ENTRY(pEntry)))
					{
						DBGPRINT(RT_DEBUG_ERROR, ("P2pWPSDone- Save to persistent entry. GrpCap= %x \n", P2pClientEntry.GroupCapability));
			...
			)
		{
			...
			if (CurOpMode == STA_MODE)
			{
					...
					(pCredential->EncrType == WSC_ENCRTYPE_NONE)))
				{
					...
					if (INFRA_ON(pAdapter))
						WscLinkDown(pAdapter);
					DBGPRINT(RT_DEBUG_TRACE, ("WscEapRegistrarAction : Done but AuthType == WSC_AUTHTYPE_OPEN EncrType == WSC_ENCRTYPE_NONE\n"));
					DBGPRINT(RT_DEBUG_TRACE,("pAd->MlmeAux.Ssid=%s\n",pAdapter->MlmeAux.Ssid));
				}
				else
#endif /* DPA_T */
...
/*#endif */
	...
	if ((pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI) && pEntry)
	{
		P2pSendWirelessEvent(pAd, RT_P2P_WPS_TIMEOUT, NULL, pEntry->Addr);
	}
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
    ...
    IN PVOID SystemSpecific3)
{
    ...
    else
    {
		...
		{
#ifdef RELEASE_EXCLUDE
        	DBGPRINT(RT_DEBUG_INFO, ("pAd is NULL!!\n"));
        	DBGPRINT(RT_DEBUG_INFO, ("<----- WscEAPOLTimeOutAction\n"));
#endif /* RELEASE_EXCLUDE */
			return;
		}
...
#ifdef HE_SUPPORT
	if (pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI)
		P2pSendWirelessEvent(pAd, RT_P2P_WPS_2MINS_TIMEOUT, NULL, NULL);
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
	...
	OUT	PUCHAR			pIeLen)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("-----> WscMakeProbeReqIEWithVendorExt\n"));
	...
	IN  PWSC_CTRL   	pWscControl)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("WscPbcExtraRemainScanCount = %d\n", pAd->StaCfg.WscPbcExtraRemainScanCount));
#endif /* DPA_T */
	...
	IN  PWSC_CTRL       pWscControl)
{
...
#ifdef HE_SUPPORT
			if (pWscControl->EntryIfIdx >= MIN_NET_DEVICE_FOR_P2P_CLI)
				P2pSendWirelessEvent(pAd, RT_P2P_WPS_PBC_SESSION_OVERLAP, NULL, NULL);
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("Need extra scan to confirm there is only one WPS PBC AP.\n"));
		pAd->StaCfg.WscPbcExtraRemainScanCount--;
			...
			{							
				...
				if (bSameAP)
				{
						...
						(ApUuidBssid[j].Band != TmpInfo.Band))
					{
						...
						RTMPMoveMemory(&(ApUuidBssid[j].MacAddr[0]), &TmpInfo.MacAddr[0], MAC_ADDR_LEN);
...
#ifdef RELEASE_EXCLUDE
		DBGPRINT(RT_DEBUG_ERROR, ("Receive a UPnP WSC Message!\n"));
#endif /* RELEASE_EXCLUDE */		
    ...
    IN  UCHAR			CurOpMode)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("-----> WscWriteConfToDatFile\n"));
	...
	if (CurOpMode == AP_MODE)
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("<----- WscWriteConfToDatFile (wrong apidx = %d)\n", apidx));
			return;
		}
		pWscControl = &pAd->ApCfg.MBSSID[apidx].WscControl;
#ifdef RTMP_RBUS_SUPPORT
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			DBGPRINT(RT_DEBUG_ERROR, ("pAd->ApCfg.MBSSID[%d].DefaultKeyId=%d\n", index, pAd->ApCfg.MBSSID[index].DefaultKeyId+1));
			if (index == 0)
    ...
    IN PVOID SystemSpecific3) 
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("EnqueueStartForWSCExec\n"));
	if ((pEntry))
	{
		PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)pEntry->pAd;

		if (pEntry->EnqueueStartForWSCTimerRunning == WSC_EAPOL_START_ENABLE)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("Enqueue WSC_EAPOL_START_MSG\n"));
			MlmeEnqueue(pAd, WSC_STATE_MACHINE, WSC_EAPOL_START_MSG, MAC_ADDR_LENGTH, &pEntry->Addr, 0);
		}
	}			
	pEntry->EnqueueStartForWSCTimerRunning = WSC_EAPOL_START_DISABLE;
}
#endif /* ENQUEUE_WPS_EAPOL_START */
#endif /* P2P_SUPPORT */
#endif /* WSC_INCLUDED */
