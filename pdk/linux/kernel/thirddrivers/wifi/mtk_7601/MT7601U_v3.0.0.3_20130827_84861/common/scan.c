...
static INT scan_ch_restore(RTMP_ADAPTER *pAd, UCHAR OpMode)
{
	...
	{
		if ((pAd->CommonCfg.DesiredHtPhy.ChannelWidth) && (pAd->StaActive.SupportedHtPhy.ChannelWidth))
			pAd->hw_cfg.bbp_bw = BW_40;
		else
			pAd->hw_cfg.bbp_bw = BW_20;
	}
	else
		pAd->hw_cfg.bbp_bw = BW_20;
#endif /* CONFIG_STA_SUPPORT */
...
//			 && (pAd->StaCfg.WpsProbeReqIeLen != 0)   //Not add WPS_IE in NON WPS period
			bHasWscIe = FALSE;
	...
	IN UCHAR ForceChannel) 
{
	...
	if (ForceChannel !=0) 
	{
			...
			AsicSwitchChannel(pAd, ForceChannel, TRUE);
			AsicLockChannel(pAd, ForceChannel);

			pAd->StaCfg.LastScanChannel = ForceChannel;
	...
	else 
	{
		...
		if ((pAd->MlmeAux.ScanType == SCAN_P2P) &&
			(pAd->P2pCfg.bChannelListSeparate == TRUE))
		{
			if (P2P_CHAN_PropertyCheck(pAd, pAd->MlmeAux.Channel, CHANNEL_PASSIVE_SCAN) == TRUE)
				ScanType = SCAN_PASSIVE;
		}
		else
