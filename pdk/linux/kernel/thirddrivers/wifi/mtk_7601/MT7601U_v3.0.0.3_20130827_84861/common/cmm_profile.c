...
#ifdef CONFIG_ALI_ZHA_MAC_PATCH
static unsigned char * g_cmacaddr = 0xa3ffb000;
#endif
...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
	...
	{
#ifdef CONFIG_ALI_ZHA_MAC_PATCH
        memcpy(pAd->CurrentAddress, g_cmacaddr, 6);
	...
	*/
...
#ifdef DFS_SUPPORT
	{
		...
		{
		    if(simple_strtol(tmpbuf, 0, 10) != 0)
				pRadarDetect->FixDfsLimit = TRUE;
			else
				pRadarDetect->FixDfsLimit = FALSE;

			DBGPRINT(RT_DEBUG_TRACE, ("FixDfsLimit=%d\n", pRadarDetect->FixDfsLimit));
		}
		...
		{
		    if(simple_strtol(tmpbuf, 0, 10) != 0)
				pRadarDetect->LongPulseRadarTh = simple_strtol(tmpbuf, 0, 10);

			DBGPRINT(RT_DEBUG_TRACE, ("LongPulseRadarTh=%d\n", pRadarDetect->LongPulseRadarTh));
		}
		...
		{
			if(simple_strtol(tmpbuf, 0, 10) != 0)
				pRadarDetect->AvgRssiReq = simple_strtol(tmpbuf, 0, 10);

			DBGPRINT(RT_DEBUG_TRACE, ("AvgRssiReq=%d\n", pRadarDetect->AvgRssiReq));
		}
#endif 
#endif /* RTMP_RBUS_SUPPORT */
		}
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			/*p2p AutoChannelSelect*/
			if(RTMPGetKeyParameter("P2pAutoChannelSelect", tmpbuf, 10, pBuffer, TRUE))
			{
				...
				if(simple_strtol(tmpbuf, 0, 10) != 0)  /*Enable*/
				{
					...
					if ((SelAlg & 0x1) == 0)
					{
                                          pAd->P2pCfg.bAutoChannel = FALSE;
						pAd->P2pCfg.bAutoChannelAtBootup = FALSE;
					}
					else /*Enable*/
					{
						//pAd->P2pCfg.AutoChannelAlg = SelAlg;
						pAd->P2pCfg.AutoChannelAlg = 2;   // fix arg to CCA
						
						// check if auto ch on social chs 1, 6, 11 only
						pAd->P2pCfg.bAutoChOnSocial = (SelAlg & 0x2) ? TRUE : FALSE;

						pAd->P2pCfg.bAutoChannel = TRUE;
						pAd->P2pCfg.bAutoChannelAtBootup = TRUE;
					}
				}
				else /*Disable*/
                                {
				...
				DBGPRINT(RT_DEBUG_TRACE, ("p2p AutoChannelAtBootup=%d\n", pAd->P2pCfg.bAutoChannelAtBootup));
                        }
			...
			if(RTMPGetKeyParameter("P2P_GO_Accept_InvitationReq", tmpbuf, 10, pBuffer, TRUE))
			{
				pAd->P2pCfg.bP2pGoAcceptInvitationReq= simple_strtol(tmpbuf, 0, 10);
				DBGPRINT(RT_DEBUG_ERROR, ("P2P_GO_Accept_InvitationReq=%d\n", pAd->P2pCfg.bP2pGoAcceptInvitationReq));
			}
