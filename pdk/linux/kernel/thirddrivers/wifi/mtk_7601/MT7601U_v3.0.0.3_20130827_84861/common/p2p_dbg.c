	...
	if (P2P_INF_ON(pAd))
	{
		...
		if (pAd->StaCfg.WfdCfg.bWfdEnable)
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("WFD_SessionAvail = %d.  WSD = %d.  PC = %d.  CP = %d.  TimeSync = %d.\n", 
					pAd->StaCfg.WfdCfg.SessionAvail, pAd->StaCfg.WfdCfg.WSD, pAd->StaCfg.WfdCfg.nPC, pAd->StaCfg.WfdCfg.CP, pAd->StaCfg.WfdCfg.TimeSync));
		}
#endif /* WFD_SUPPORT */
	}
		...
		IN	PSTRING 		arg)
{
	...
	{
		if (P2PChannelSanity(pAd, channel) == TRUE)
		{
	...
	IN	PSTRING 		arg)
{
	...
	if (IntraBss)
	{
		...
		pAd->P2pCfg.P2pControl.word |= P2P_CONTROL_REG_INTRA_BSS;
		pAd->P2pCfg.P2pCapability[1] |= GRPCAP_INTRA_BSS;
	}
	else
	{
...

{
	...
	pP2PCtrl->CheckPeerChannelPolicy = ((SigmaEnabled == 0) ? CHECK_PEER_CHANNEL_IF_NO_CONNECTION : CHECK_PEER_CHANNEL_DISABLE);
        ...
        pP2PCtrl->bAutoChannel = ((SigmaEnabled == 0) ? (pP2PCtrl->bAutoChOnSocial) : FALSE);
#endif
...
INT Set_P2P_DelPerstEntry_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	POS_COOKIE			pObj;
...
INT Set_P2P_CheckPeerChannel_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	POS_COOKIE			pObj;
	...
	IN	PSTRING			arg)
{
	...
	if (strlen(arg) < 1)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("=====================================================\n"));
 		for (channel_index=0 ; channel_index < MyChannelListNum ; channel_index++)
			/* debug messages */
			DBGPRINT(RT_DEBUG_TRACE, ("Channel %d : Dirty = %ld, ApCnt=%ld, Busy Time = %d, Skip Channel = %s\n", 
			pMyChannelList[channel_index].Channel,
		...
		for (channel_idx = 0; channel_idx < MyChannelListNum; channel_idx++)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("Channel %d : Dirty = %ld, False CCA = %u, Busy Time = %u, Skip Channel = %s\n",
					pMyChannelList[channel_idx].Channel,
...
INT Set_P2P_Go_Accept_Invitation_Request(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING 		arg)
{
	POS_COOKIE			pObj;
...
/* 
    ==========================================================================
    Description:
        Set P2P Country Region.
        This command will not work, if the field of CountryRegion in eeprom is programmed.
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
INT Set_P2P_CountryRegion_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	int retval;
	...
	retval = RT_CfgSetP2PCountryRegion(pAd, arg, BAND_24G);
	if (retval == FALSE)
		return FALSE;
...
/* 
    ==========================================================================
    Description:
        Set P2P Country Region for A band.
        This command will not work, if the field of CountryRegion in eeprom is programmed.
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    ==========================================================================
*/
INT Set_P2P_CountryRegionABand_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	int retval;
	...
	retval = RT_CfgSetP2PCountryRegion(pAd, arg, BAND_5G);
	if (retval == FALSE)
		return FALSE;
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s::(CountryRegionForABand=%d)\n", __FUNCTION__, pP2PCtrl->CountryRegionForABand));
	
	return TRUE;
}

INT Set_P2P_ChannelListSeparate_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING 		arg)
{
	POS_COOKIE			pObj;
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s::P2P channel list separate enable = %d\n", __FUNCTION__, bEnable));
