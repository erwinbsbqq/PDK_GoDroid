...
#ifdef P2P_SUPPORT
		/* Fix P2P Certificated test case 6.1.11 */
	...
	else
	{
...
#endif /* P2P_SUPPORT */
			{
				...
				if (pAd->EthConvert.ECMode & ETH_CONVERT_MODE_DONGLE)
				{
					...
					if(!NdisEqualMemory(pAd->CurrentAddress, (GET_OS_PKT_DATAPTR(pPacket) + 6), ETH_LENGTH_OF_ADDRESS))
					{
						TX_BLK_SET_FLAG(pTxBlk, fTX_bDonglePkt);
#ifdef RELEASE_EXCLUDE
						DBGPRINT(RT_DEBUG_INFO, ("RTMPHardTransmit: Dongle Packet)\n"));
#endif /* RELEASE_EXCLUDE */
					}
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME)
		{
			...
			{	/* Specific packet, i.e., bDHCPFrame, bEAPOLFrame, bWAIFrame, need force low rate.*/
                ...
                if (RTMP_GET_PACKET_LOWRATE(pPacket) && pMacEntry && pMacEntry->bP2pClient)
                {
                    /* Follow Mgmt rate */
...
//---Add by shiang for debug
	...
	*/
	...
	if (pRxBlk->DataSize > MAX_RX_PKT_LEN)
	{
		...
		if (err_size > 20)
		{
			 DBGPRINT(RT_DEBUG_TRACE,("Legacy DataSize = %d\n", pRxBlk->DataSize));
