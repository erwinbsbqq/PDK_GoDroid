	...
	IN PRTMP_ADAPTER pAd) 
{
...
#ifdef HE_BD_GG_SUPPORT
	pP2PCtrl->P2p_OpMode = P2P_ONLY;
#else
	pP2PCtrl->P2p_OpMode = P2P_CONCURRENT;
#endif /* HE_BD_GG_SUPPORT */
	 ...
	 */
#ifdef P2P_CLIENT_DISCOVERY_REMOVE 
	pP2PCtrl->P2pControl.word = 0x80008008;
#else
	...
	pP2PCtrl->bChannelListSeparate = FALSE;
	...
	IN PVOID SystemSpecific3) 
{
	...
	if ((pP2PCtrl->bProvAutoRsp == FALSE) && (pP2PCtrl->P2pProvIndex != P2P_NOT_FOUND) )
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s: NeedResumeNoA is TRUE, Go Start NoA!\n", __FUNCTION__));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))
	{
		...
		ScanNextChannel(pAd, OPMODE_STA, 0);
	}
...
VOID P2pStopScan(
	IN PRTMP_ADAPTER pAd,
	IN UCHAR StopChannel)
{
	BOOLEAN Cancelled;

	DBGPRINT(RT_DEBUG_INFO, ("<---- P2P - P2pStopScan @channel = %d. StopChannel = %d.\n", pAd->MlmeAux.Channel, StopChannel));
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))
	{
		...
		RTMPCancelTimer(&pAd->MlmeAux.ScanTimer, &Cancelled);
		pAd->MlmeAux.Channel = 0;
		ScanNextChannel(pAd, OPMODE_STA, StopChannel);
	}
	...
	else if (P2P_CLI_ON(pAd))
	{
		...
		if (pEntry->P2pInfo.NoADesc[0].bValid == TRUE)
		{
			...
			else
			{
				P2pStopNoA(pAd, pEntry);
#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_INFO,("4  P2pGPTimeOutHandle.!!StopGP.  Error Case. shoudn't happen! \n"));
#endif /* RELEASE_EXCLUDE */
...
{ 
	...
	if ((pP2PCtrl->Rule == P2P_IS_GO) && (P2P_GO_ON(pAd)))
	{
		...
		/* Set Channel. */
		DBGPRINT(RT_DEBUG_ERROR, ("(%d, %d)\n", pAd->LatchRfRegs.Channel, pP2PCtrl->GroupOpChannel));

		if (pAd->LatchRfRegs.Channel != pP2PCtrl->GroupOpChannel)
			bChangeInitBW = TRUE;
		else
		{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("P2P_FIXED_MODE\n"));
			AsicBBPAdjust(pAd);
		}
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_ERROR, ("<---- P2pGoNegoDone: ConnectState[%s] \n", decodeP2PState(pP2PCtrl->P2PConnectState)));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_ERROR, ("----> P2pGoNegoDone: P2PState= %s\n", decodeP2PState(pP2PCtrl->P2PConnectState)));
#endif /* RELEASE_EXCLUDE */
...
*/
	...
	if (pP2PCtrl->bWIDI)
	{
		...
		*/
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("Bssid is zero; Not sending this LINK DOWN\n"));
		}
		else
		{
...
*/
	...
	pP2PCtrl->bConfirmByUI = pP2PCtrl->bP2pAutoAccept;
	if (INFRA_ON(pAd))
		pP2PCtrl->GroupOpChannel = pAd->CommonCfg.Channel;
	else
	{
	       if (!(pAd->flg_p2p_OpStatusFlags & P2P_FIXED_MODE))
	       {
	...
	IN PUCHAR	pChannelList)
{
	...
	for (k = 0;k < MAX_NUM_OF_CHANNELS;k++)
	{
		...
		{
			if (pMyChannelList[i].Channel == *(pChannelList + k))
			{
				break;
			}
		}
...
*/
	...
	else if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
	{
		/* Woody 2: Enter PIN Code */
		{
					...
					union iwreq_data wrqu;
					NdisZeroMemory(&wrqu, sizeof(wrqu));
					DBGPRINT(RT_DEBUG_ERROR, ("Key Pin Key Pin Key Pin Key Pin\n"));
...
/*	BOOLEAN		bDot1xReload = FALSE; */
	...
	if (!INFRA_ON(pAd))
		pAd->CommonCfg.Channel = pAd->P2pCfg.GroupOpChannel;
...
#if 0
	/* todo */
	NdisMoveMemory(pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid, "DIRECT-AA", 9);
	pAd->ApCfg.MBSSID[MAIN_MBSSID].AuthMode = Ndis802_11AuthModeWPA2PSK;
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("---> P2PAPInitialize\n"));
	...
	IN INT				band)
{
	...
	if (band == BAND_24G)
		pCountryRegion = &pP2PCtrl->CountryRegion;
	else
		pCountryRegion = &pP2PCtrl->CountryRegionForABand;
	
	if((region >= 0) && 
	   (((band == BAND_24G) &&((region <= REGION_MAXIMUM_BG_BAND) || 
	   (region == REGION_31_BG_BAND) || (region == REGION_32_BG_BAND) || (region == REGION_33_BG_BAND) )) || 
	    ((band == BAND_5G) && ((region <= REGION_MAXIMUM_A_BAND) || (region == REGION_DISABLE_A_BAND))))
	  )
	{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s::region(%ld) out of range!\n", __FUNCTION__, region));
		return FALSE;
	}
