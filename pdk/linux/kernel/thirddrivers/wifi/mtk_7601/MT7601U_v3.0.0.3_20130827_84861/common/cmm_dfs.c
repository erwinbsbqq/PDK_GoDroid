	...
	IN	PSTRING			arg)
{
	if (simple_strtol(arg, 0, 10) == 1)
		...
		pRadarDetect->McuRadarDebug |= RADAR_SIMULATE2;
	return TRUE;
}
#endif
...
INT	Set_FixDfsLimit_Proc(
	...
	IN	PSTRING			arg)
{
	pRadarDetect->FixDfsLimit = simple_strtol(arg, 0, 10);
	return TRUE;
}
...
INT	Set_AvgRssiReq_Proc(
	...
	IN	PSTRING			arg)
{
	pRadarDetect->AvgRssiReq = simple_strtol(arg, 0, 10);
	return TRUE;
}
#endif
	...
	IN	PSTRING			arg)
{
	PDFS_SW_DETECT_PARAM pDfsSwParam = &pAd->CommonCfg.RadarDetect.DfsSwParam;
#if 0
Cnt=4640, w= 208  Time=142172
Cnt=4640, w= 160  Time=260514
Cnt=4640, w= 208  Time=363873
Cnt=4640, w= 160  Time=482216
Cnt=4640, w= 208  Time=585575
Cnt=4640, w= 164  Time=703918
Cnt=4640, w= 208  Time=807277
Cnt=4640, w= 160  Time=925620
Cnt=4640, w= 208  Time=1028979
Cnt=4640, w= 164  Time=1147321
#endif
int i=0, id=0;
	...
	IN PSTRING arg)
{
	...
	pRadarDetect->LongPulseRadarTh = Value;
...
void NewRadarDetectionStart(PRTMP_ADAPTER pAd)
{
	...
	RTMP_BBP_IO_WRITE32(pAd, DFS_R30, 3);
		...
		IN PRTMP_ADAPTER pAd)
{
	...
	for (k = 0; k < limit; k++)
	{
		...
		{	
			RTMP_BBP_IO_READ32(pAd, DFS_R31, &bbp_val);
			//printk("bbp_val = 0x%08X\n", bbp_val);
	...
	if (pDfsSwParam->dfs_w_counter & 1)
		...
		RTMP_BBP_IO_WRITE32(pAd, DFS_R30, 7);
	...
	RTMP_BBP_IO_WRITE32(pAd, DFS_R30, (dfs_R30 | 0x3));
