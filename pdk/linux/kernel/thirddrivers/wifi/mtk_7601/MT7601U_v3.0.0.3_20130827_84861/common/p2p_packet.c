	...
	OUT PUCHAR 	pDest)
{
	...
	PUCHAR		pLastLenPos;
	UCHAR		LastNum = 0;
	UCHAR MyChannelListNum;
	PCHANNEL_TX_POWER pMyChannelList;
...
#ifdef P2P_CHANNEL_LIST_SEPARATE
	if (pP2PCtrl->bChannelListSeparate == TRUE)
	{
		MyChannelListNum = pP2PCtrl->ChannelListNum;
		pMyChannelList = pP2PCtrl->ChannelList;
	}
	else
#endif /* P2P_CHANNEL_LIST_SEPARATE */
	{
		MyChannelListNum = pAd->ChannelListNum;
		pMyChannelList = pAd->ChannelList;
	}

	for (i = 0; i < MyChannelListNum; i++)
	{
		CurRegClass = ChannelToClass(pMyChannelList[i].Channel, 1);
		...
		/* Insert  */
		*(pDest + pos) = pMyChannelList[i].Channel;
		LastNum++;
		pos++;

		/* Last item. Update Num. */
		if (i == MyChannelListNum -1)
		{
			*pLastLenPos = LastNum;
		}
	}
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	else if ((StatusCode == P2PSTATUS_REJECT_BY_USER) || (StatusCode == P2PSTATUS_PASSED))
	{
		...
		if (index < MAX_P2P_GROUP_SIZE)
		{
			...
			P2pStopScan(pAd, pP2PCtrl->ListenChannel);
	...
	if (index < MAX_P2P_GROUP_SIZE)
	{
		...
		P2pOpChannelSelection(pAd, ChannelList, pP2pEntry);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
		...
		*/
...
#endif
			if (index == P2P_NOT_FOUND) //Nick add
			{
				P2pGroupTabInit(pAd);
				index = P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_DISCOVERY, NULL, 0, DeviceCap, GroupCap);
			}
		...
		if (index < MAX_P2P_GROUP_SIZE)
		{
			...
			pP2pEntry = &pAd->P2pTable.Client[index];
			RTMPMoveMemory(pP2pEntry->DeviceName, DeviceName, 32);
			pP2pEntry->DeviceNameLen = DeviceNameLen;
			pP2pEntry->DeviceName[DeviceNameLen] = 0x0;
			pP2pEntry->ConfigMethod = ConfigMethod;
			pP2pEntry->bSupport5G = FALSE;
			pP2pEntry->bSCCChannel=FALSE;
		...
		P2PMakeGoNegoRsp(pAd, DevAddr, SentDpid, pFrame->Token, TempIntent, pP2PCtrl->GroupOpChannel, RspStatus, pOutBuffer, &TempLen);
		FrameLen = TempLen;
		pDest += TempLen;
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	{
		P2pStopScan(pAd, pAd->P2pCfg.ListenChannel);
//		AsicSwitchChannel(pAd, pAd->P2pCfg.ListenChannel, FALSE);
//		AsicLockChannel(pAd, pAd->P2pCfg.ListenChannel);
	}
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("P2pReceInviteReqAction ==> %s \n", decodeP2PState(pP2PCtrl->P2PConnectState)));
#endif /* RELEASE_EXCLUDE */
	RTMPZeroMemory(ChannelList, MAX_NUM_OF_CHANNELS);
...
#endif
	p2pindex = P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_DISCOVERY, Ssid, SsidLen, DeviceCap, GroupCap); //Nick add
	if (p2pindex == P2P_NOT_FOUND) 
	{
		P2pGroupTabInit(pAd);
		p2pindex = P2pGroupTabInsert(pAd, DevAddr, P2PSTATE_DISCOVERY, Ssid, SsidLen, DeviceCap, GroupCap);
	}
	...
	if (p2pindex == P2P_NOT_FOUND)
		goto CleanUp;
	else if (p2pindex < MAX_P2P_GROUP_SIZE)
	{
		pP2pEntry = &pAd->P2pTable.Client[p2pindex];
		RTMPMoveMemory(pP2pEntry->DeviceName, DeviceName, 32);
		pP2pEntry->DeviceNameLen = DeviceNameLen;
	...
	if (P2P_GO_ON(pAd))
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("Invite: Reject invitation request while being P2P GO\n"));
	...
	if (bReinvoke == TRUE)
	{
		...
		if (RspStatus == P2PSTATUS_SUCCESS)
		{
			...
			else
			{
				...
				/* Copy peer bssid to p2p client entry that we can search the p2p entry by bssid when bssid is different from peer device address */
				COPY_MAC_ADDR(pP2pEntry->bssid, BssidAddr);
				COPY_MAC_ADDR(pP2pEntry->InterfaceAddr, BssidAddr);
				pP2pEntry->OpChannel = OpChannel;
			}
		}
	}
	...
	if (index >= MAX_P2P_TABLE_SIZE)
		RspStatus = P2PSTATUS_UNKNOWN_GROUP;
#endif /* WIDI_SUPPORT */
	P2PMakeInviteRsp(pAd, MyRule, pFrame->Token, pFrame->p80211Header.Addr2, pP2PCtrl->CurrentAddress, &pP2PCtrl->GroupOpChannel, &RspStatus, &FrameLen);
	...
	else if ((bReinvoke == TRUE) && (RspStatus == P2PSTATUS_SUCCESS))
	{
		...
		else
		{
			if (!P2P_GO_ON(pAd) && !P2P_CLI_ON(pAd))
			{
				...
				/*
					Wait here for waiting this invite response frame is sent out;
					otherwise this invite response frame may be sent out in operation channel.
				*/
				OS_WAIT(50);
				P2pDeauthAP(pAd, pP2pEntry);
		...
		|| (WpsConfigMethod == 0x188))
	{
		...
		if (p2pindex >= MAX_P2P_GROUP_SIZE)
		{
			...
			p2pindex = P2pGroupTabInsert(pAd, pFrame->p80211Header.Addr2, P2PSTATE_DISCOVERY, NULL, 0, DeviceCap, GroupCap);
                                if (p2pindex == P2P_NOT_FOUND) //Nick add
                                {
					P2pGroupTabInit(pAd);
					p2pindex = P2pGroupTabInsert(pAd, pFrame->p80211Header.Addr2, P2PSTATE_DISCOVERY, NULL, 0, DeviceCap, GroupCap);
				}
	...
	IN PUCHAR		Addr1)
{
		...
		{
			Channel = pP2PCtrl->GroupOpChannel;
...
*/	
		...
		*/
		...
		if (pP2pEntry->Rule == P2P_IS_CLIENT)
		{
			/* Set my rule to GO and status flag is P2P_GO_UP after send out the GO nego comfirm frame */
			P2pDeauthAP(pAd, pP2pEntry);
			pP2PCtrl->Rule = P2P_IS_GO;
			pAd->flg_p2p_OpStatusFlags = P2P_GO_UP; 
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
			/* Note: Send event will take long time, so we send event only after GO-Neg-Confirm is 	sent out. */
			P2pSendWirelessEvent(pAd, RT_P2P_CONNECTED, pP2pEntry, pP2PCtrl->CurrentAddress);
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
		}
		else
		{
			/* My rule is P2P client */
#ifdef CONFIG_MULTI_CHANNEL
			if (pAd->Multi_Channel_Enable == FALSE)
#endif /*CONFIG_MULTI_CHANNEL*/
				P2pDeauthAP(pAd, pP2pEntry);
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_OP_CHANNEL, &Channel, pDest);
	pDest += Length;
 	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	...
	*pTotalFrameLen += Length;
	...
	Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_CHANNEL_LIST, &Channel, pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/*  attach subelementID= 13. */
	...
	OUT PUCHAR pWpsIE) 
{
		...
		switch(pEid->Eid)
		{
			...
			case IE_VENDOR_SPECIFIC:
				...
				if (NdisEqualMemory(pEid->Octet, WPS_OUI, 4) && (pEid->Len > 4))
				{
					...
					{
						DBGPRINT(RT_DEBUG_TRACE, ("Found WIDI WPS IE in P2P Probe Request\n"));
	...
	*/
	...
	{
		brc = FALSE;
		DBGPRINT(RT_DEBUG_INFO, ("Ignore Probe Request that had invalid SSID Name.\n"));
	}
#endif
	...
	*/
...
#endif /* WFD_SUPPORT */
		{
			...
			if (P2P_CLI_ON(pAd))
			{
				...
				{
					DBGPRINT(RT_DEBUG_ERROR, ("1 From Addr2 = %02x:%02x:%02x:%02x:%02x:%02x  \n",  PRINT_MAC(Addr2)));
					DBGPRINT(RT_DEBUG_ERROR, ("1 pAd->P2pCfg.Bssid = %02x:%02x:%02x:%02x:%02x:%02x	return\n", PRINT_MAC(pAd->P2pCfg.Bssid)));
					goto CleanUp;
				}
			}
#endif
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				if (DeviceNameLen != 0)
				{
					/* If the P2P device name is empty in P2P entry table, send wireless event RT_P2P_DEVICE_FIND to inform upper layer */
					if (pP2pEntry->DeviceNameLen == 0)
						bSendP2pEvent = TRUE;
#ifdef DPA_S
					if (bNewFound)
					{
...
/*	IN BOOLEAN	bSendRsp)  */
{
		...
		UCHAR		GroupCap = 0, DeviceCap = 0, DevAddr[MAC_ADDR_LEN], StatusCode;
		...
		if (Channel != pAd->P2pCfg.ListenChannel)
		{
			...
			if ((pAd->CommonCfg.BBPCurrentBW == BW_40) &&
				(((pAd->CommonCfg.CentralChannel < pAd->CommonCfg.Channel) && ((Channel + 2) == pAd->P2pCfg.ListenChannel)) ||
				((pAd->CommonCfg.CentralChannel > pAd->CommonCfg.Channel) && ((Channel - 2) == pAd->P2pCfg.ListenChannel))))
			{
				bSendRsp = TRUE;
				Channel = pAd->P2pCfg.ListenChannel;
									...
									WpsIE))
		{
			...
			/* p2p need insert p2p entry in probe req. due to p2p0 only in listen state */
			/* This will update the listen channel of p2p entry and send the P2P probe request to the correct channelS */
			if (index == P2P_NOT_FOUND)
			{
				index = P2pGroupTabInsert(pAd, Addr2, P2PSTATE_DISCOVERY, Ssid, SsidLen, DeviceCap, GroupCap);
			}
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("-----> P2pMakeProbeReqIE\n"));
#endif /* RELEASE_EXCLUDE */
	...
	OUT	ULONG *pFrameLen) 
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("%s:: pP2PCtrl.Addr[%02x:%02x:%02x:%02x:%02x:%02x]\n", __FUNCTION__, PRINT_MAC(pP2PCtrl->CurrentAddress)));
#endif /* RELEASE_EXCLUDE */
	...
	CapabilityInfo = CAP_GENERATE(0, 0, 1, TxPreamble, pAd->CommonCfg.bUseShortSlotTime, 0);
	if (P2P_GO_ON(pAd))
		CapabilityInfo = CAP_GENERATE(1, 0, 1, TxPreamble, pAd->CommonCfg.bUseShortSlotTime, 0);
	...
	*/	
...
#ifdef WFD_SUPPORT
	{
		...
		WfdIeBitmap = (0x1 << SUBID_WFD_DEVICE_INFO) | (0x1 << SUBID_WFD_ASSOCIATED_BSSID) |
			(0x1 << SUBID_WFD_COUPLED_SINK_INFO);
		if (SubType == SUBTYPE_PROBE_RSP)
			WfdIeBitmap |= (0x1 << SUBID_WFD_SESSION_INFO);
	...
	OUT	PULONG			pIeLen)
{
	...
	*pIeLen = 0;
#ifdef RELEASE_EXCLUDE
	DBGPRINT(RT_DEBUG_INFO, ("%s: PacketType - %d\n", __FUNCTION__, PacketType));
#endif /* RELEASE_EXCLUDE */
	/* 0. P2P fixed IE */
	...
	if ((pP2PCtrl->P2pManagedParm.TotalNumOfP2pAttribute > 0) &&
		(PacketType == SUBTYPE_ASSOC_REQ) && INFRA_ON(pAd))
	{
		...
		if ((P2P_TEST_BIT(P2pCapability[1], GRPCAP_CROSS_CONNECT))
			&& RTMPEqualMemory(pAd->CommonCfg.Bssid, pAd->P2pCfg.P2pManagedParm.ManageAPBSsid, MAC_ADDR_LEN))
		{
...
#if 0
	DBGPRINT(RT_DEBUG_TRACE, ("P2pStopGoTimer -  13 sec\n"));
	...
	IN  PUCHAR	pIpReqP2ptabindex) 
{
		...
		if (pCapability != NULL)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("P2pUpdateBssBeacon Cap::  = %x  %x. )\n", *(pCapability), *(pCapability+1)));
	...
	IN PRTMP_ADAPTER pAd)
{
		...
		RTMPWriteTxWI(pAd, pPort, pTxWI, FALSE, TRUE, FALSE, FALSE, TRUE, 0, BSS0Mcast_WCID, 
			FrameLen, PID_MGMT, 0, IFS_HTTXOP, FALSE, BeaconTransmit);
		/*
		 * step 6. move BEACON TXD and frame content to on-chip memory
		 */
		ptr = (PUCHAR)&pAd->BeaconTxWI;
	...
	IN UCHAR	*pRspStatus)
{
	...
	*pRspStatus = P2PSTATUS_SUCCESS;
	...
	if (bIAmGO == FALSE)
	{
		...
		for (i = 0;i < MyChannelListNum;i++)
		{
			if (pMyChannelList[i].Channel == OpChannel)
			{
				break;
			}
		}
		if ( i == MyChannelListNum)
		{
			*pRspStatus = P2PSTATUS_NO_CHANNEL;
	...
	IN UCHAR		Country)
{
	...
	if (Country == 1/*COUNTRY_USA*/)
	{
		...
		/* Update global operating classes base on 802.11-2012.pdf. Table E-4 */
		if (Channel <= 13)
		{
			ReturnClass = 81;
		}
		...
		{
			ReturnClass = 82;	/* Atheros suggest to use 1 2010-May */
		}			
		else if (Channel <= 48)
		{
	...
	IN PRT_P2P_CLIENT_ENTRY pP2pEntry)
{
	PRT_P2P_CONFIG	pP2PCtrl = &pAd->P2pCfg;
	int iPeerChannelIndex, i;
	UCHAR MyChannelListNum, bMatchChannel = FALSE;
	PCHANNEL_TX_POWER pMyChannelList;
#ifdef P2P_AUTO_CHANNEL_SUPPORT
...
#endif /* P2P_AUTO_CHANNEL_SUPPORT */
	...
	for (iPeerChannelIndex = 0;iPeerChannelIndex < MAX_NUM_OF_CHANNELS;iPeerChannelIndex++)
	{
		...
		( !(INFRA_ON(pAd) || P2P_GO_ON(pAd) || P2P_CLI_ON(pAd))))
		{
			for (i=0;i<MyChannelListNum;i++)
			{
								...
								if (!((pMyChannelList[i].Channel == 1) || \
									(pMyChannelList[i].Channel == 6) || \
									(pMyChannelList[i].Channel == 11)) &&  \
					(pP2PCtrl->bAutoChOnSocial == TRUE) && \
					(NoSocialChs == FALSE))
						continue;
	
				// search for matching channels
				if ( ( pMyChannelList[i].Channel == ChannelList[iPeerChannelIndex]))
				{				  
					// find a best channel from matching channels
					if ((min_dirty > pChannelInfo->dirtyness[i]))
					{
						min_dirty = pChannelInfo->dirtyness[i];
						pP2PCtrl->GroupOpChannel  = pMyChannelList[i].Channel;
						DBGPRINT(RT_DEBUG_TRACE, ("%s min_dirty = %d, PeerChannelListNum = %d\n", \
									__FUNCTION__, pChannelInfo->dirtyness[i], pMyChannelList[i].Channel));		 
		...
		*/
			...
			(((iPeerChannelIndex == 0) && (ChannelList[1] == 0)) || (pP2pEntry->bSupport5G == TRUE)))
		{
			for (i=0;i<MyChannelListNum;i++)
			{
				if ( ( pMyChannelList[i].Channel == ChannelList[iPeerChannelIndex]))				   
				{
					pP2PCtrl->GroupOpChannel = ChannelList[iPeerChannelIndex];
					bMatchChannel = TRUE;
					break;
				}
			}
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s GroupOpChannel = %d \n", __FUNCTION__, pP2PCtrl->GroupOpChannel));
	DBGPRINT(RT_DEBUG_TRACE, ("bConfirmByUI = %d, CheckPeerChannelPolicy = %d. INFRA_ON = %d. P2P_GO_ON = %d. P2P_CLI_ON = %d. PeerChannelListNum = %d. PeerChannelIs5GBand = %d. bMatchChannel = %d\n", 
		pP2PCtrl->bConfirmByUI, pP2PCtrl->CheckPeerChannelPolicy, INFRA_ON(pAd), P2P_GO_ON(pAd), P2P_CLI_ON(pAd), iPeerChannelIndex, pP2pEntry->bSupport5G, bMatchChannel));

	return;
}

VOID P2pAutoChSelForAutoGo(
	IN PRTMP_ADAPTER pAd)
{
#ifdef P2P_AUTO_CHANNEL_SUPPORT
	PCHANNELINFO pChannelInfo = pAd->pChannelInfo;
	...
	if (pAd->P2pCfg.bAutoChannel == TRUE)
	{
		UCHAR MyChannelListNum;
		PCHANNEL_TX_POWER pMyChannelList;
#ifdef P2P_CHANNEL_LIST_SEPARATE
		...
		// find a best channel from social channels, ch1, ch6, ch11
		// We will not use 5G chs here, since we can not sure if client is capable of 5G capability.
		for (i_ch = 0 ; i_ch <= 14 ; i_ch++)
		{
			if (!((pMyChannelList[i_ch].Channel == 1) ||\
				(pMyChannelList[i_ch].Channel == 6) || \
				(pMyChannelList[i_ch].Channel == 11)))
				continue;
		
			if (min_dirty > pChannelInfo->dirtyness[i_ch])
			{
				min_dirty = pChannelInfo->dirtyness[i_ch];
				pAd->P2pCfg.GroupOpChannel = pMyChannelList[i_ch].Channel;
				DBGPRINT(RT_DEBUG_TRACE, ("%s min_dirty = %d, GroupOpChannel = %d\n", \
							__FUNCTION__, pChannelInfo->dirtyness[i_ch], pMyChannelList[i_ch].Channel));         
			}
		}
	}
#endif /* P2P_AUTO_CHANNEL_SUPPORT */
