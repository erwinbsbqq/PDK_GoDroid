	...
	OUT	USHORT *freeCnt)
{
	...
	if ((QueIdx == QID_HCCA) && (pAd->Multi_Channel_Enable == TRUE))
		...
		if ((pTxBlk->naf_type == NAF_TYPE_TSO) && ((usb_frame_end - usb_frame_start) > 0))
		{
			...
			TSO_INFO *soinfo = (TSO_INFO *)(usb_frame_start + sizeof(TXINFO_STRUC) + sizeof(TXWI_STRUC));
			UCHAR *hdr = (UCHAR *)(usb_frame_start + sizeof(TXINFO_STRUC) + sizeof(TXWI_STRUC) + sizeof(TSO_INFO));
...
#ifdef RELEASE_EXCLUDE
	//DBGPRINT(RT_DEBUG_ERROR, ("<-- AsicSleepThenAutoWakeup, TbttNumToNextWakeUp=%d\n", TbttNumToNextWakeUp));
#endif /* RELEASE_EXCLUDE */
