...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
 ...
 && (pAd->Multi_Channel_Enable == FALSE)
	...
	*/
	...
	//Watchdog for monitoring if wifi malfunctioned by checking the RX count and the status of BBP
	static UINT16 NoRxCnt = 0;	
	#define NO_RX_CNT_THRESHOLD 10 // unit: 100ms
	UCHAR regBBP = 0, regBBP_1 = 0, regBBP_2 = 0, regBBP_3 = 0;
	
	if (P2P_GO_ON(pAd))
	{
		if(!pAd->RalinkCounters.OneSecRxCount)
			NoRxCnt++;
		else
			NoRxCnt = 0;

		if(NoRxCnt > NO_RX_CNT_THRESHOLD)
		{
			NoRxCnt = 0;	
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, 142, 0x21);
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 142, &regBBP);
			DBGPRINT(RT_DEBUG_TRACE, ("Write:\nR142:%02x\n", regBBP));
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 143, &regBBP_1); // 1st query
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 143, &regBBP_2); // 2nd query
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, 143, &regBBP_3); // 3rd query			
			DBGPRINT(RT_DEBUG_TRACE, ("Read:\nR143:%02x\n", regBBP_1));			
			DBGPRINT(RT_DEBUG_TRACE, ("Read:\nR143:%02x\n", regBBP_2));
			DBGPRINT(RT_DEBUG_TRACE, ("Read:\nR143:%02x\n", regBBP_3));

			// Continually read the status of BBP, and if all of the 3 queries shows 0x14, then something wrong with BBP!!!
			if(0x14 == regBBP_1 && 0x14 == regBBP_2 && 0x14 == regBBP_3) 
			{
				Set_Bbp_Reset(pAd);
			}
		}	
...
#ifdef WSC_INCLUDED
    DBGPRINT(RT_DEBUG_TRACE, ("WSC_HDR_BTN_CheckHandler WscHdrPshBtnFlag = 0x%x\n", (pAd)->WscHdrPshBtnFlag ));
    pAd->WscHdrPshBtnFlag = 0;
...
/*		if (ACMP_IsAllACEnabled(pAd) != ACM_RTN_OK)*/
	...
	if (INFRA_ON(pAd) && (P2P_CLI_ON(pAd) | P2P_GO_ON(pAd)) && (pAd->Multi_Channel_Enable == TRUE))
       ...
       if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) )
		...
		if (CQI_IS_DEAD(pAd->Mlme.ChannelQuality) && (pWscControl->bWscTrigger == FALSE)
#ifdef CONFIG_MULTI_CHANNEL
...
#if 0
	/*
		It is useless to set PSM bit 0 or 1 in control frame.
	*/
	...
	IN ADD_HT_INFO_IE *pAddHtInfo)
{
	...
	*/
    ...
    if (pAd->MlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_20)
    {
	 pAd->MlmeAux.HtCapability.HtCapInfo.ShortGIfor40 = 0x0; /*If not support BW40, not support SGIfor40 also*/
    }
  ...
  { 
		...
		if (pAd->Mlme.OneSecPeriodicRound % 4 == 0)
		{
			DBGPRINT(RT_DEBUG_INFO,
				...
				pAd->CommonCfg.BBPCurrentBW, GET_LNA_GAIN(pAd), R66));
		}
#endif /* RELEASE_EXCLUDE */
	}
}
