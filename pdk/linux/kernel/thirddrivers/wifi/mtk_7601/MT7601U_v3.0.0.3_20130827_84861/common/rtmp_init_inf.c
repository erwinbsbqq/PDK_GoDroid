...
//---
	...
	if (IS_RT3290(pAd))
	{
		...
		/*
			Too much time for reading efuse(enter/exit L1), and our device 
				will hang up
			Disable L1
		*/
...
#if 0
	/*
		STA doesn't need to do AsicSwitchChannel here, 
		because STA will do AsicSwitchChannel again when scan, scan completed, join to AP and LinkUp.
	*/
#endif
...
#ifdef P2P_AUTO_CHANNEL_SUPPORT
                if (pAd->P2pCfg.bAutoChannel)
                {
                        if (pAd->P2pCfg.bAutoChannelAtBootup || (pAd->CommonCfg.Channel == 0))
                        {
                                /* Enable Interrupt first due to we need to scan channel to receive beacons.*/
                                RTMP_IRQ_ENABLE(pAd);
#ifdef RTMP_MAC_USB
                                ...
                                //pAd->CommonCfg.Channel = AP_AUTO_CH_SEL(pAd, pAd->P2pCfg.AutoChannelAlg);
                                AP_AUTO_CH_SEL(pAd, pAd->P2pCfg.AutoChannelAlg);
