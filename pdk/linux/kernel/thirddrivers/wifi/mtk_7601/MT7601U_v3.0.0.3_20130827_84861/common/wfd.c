	...
	switch (SubId)
	{
		case SUBID_WFD_DEVICE_INFO:
		{
			...
			if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) && (INFRA_ON(pAd)))
				DevInfo.nPC = pWFDCtrl->nPC;
			else
				DevInfo.nPC = WFD_PC_P2P;
		...
		case SUBID_WFD_COUPLED_SINK_INFO:
		{
			if ((pWFDCtrl->DeviceType != WFD_SOURCE ) && (pWFDCtrl->SinkCoupled == WFD_COUPLED_SUPPORT))
			{
				...
				tmpValue = cpu2be16(EidLen);
				RTMPMoveMemory(pDest, &tmpValue, 2);
				SinkInfo.CoupledStat = pWFDCtrl->CoupledSinkInfo.CoupledStat;
				RTMPMoveMemory(&SinkInfo.CoupledPeerAddr[0], &pWFDCtrl->CoupledSinkInfo.CoupledPeerAddr[0], MAC_ADDR_LEN);
		...
		case SUBID_WFD_SESSION_INFO:
		{
				...
				)
			{
				...
				/*
				  * Modify for Wi-Fi Display spec.:
				  * Note that this subelement shall not include GO itself, 
				  * and this subelement shall not be included in the frame(s) 
				  * when the GO doesn't have associated client that is WFD capable.
				*/
				if (NumOfDev == 0)
				{
					Length = 0;
				...
				for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++)
				{
					...
					if ((P2pIdx < MAX_P2P_GROUP_SIZE) && (Tab->Client[P2pIdx].WfdEntryInfo.bWfdClient == TRUE))
					{
						...
						SessionInfo.WfdDevInfo.nPC = pAd->P2pTable.Client[P2pIdx].WfdEntryInfo.wfd_PC;
						...
						SessionInfo.CoupledSinkInfo.CoupledStat = pAd->P2pTable.Client[P2pIdx].WfdEntryInfo.coupled_sink_info.CoupledStat;
						...
						*/
						RTMPMoveMemory(&SessionInfo.CoupledSinkInfo.CoupledPeerAddr[0], &pAd->P2pTable.Client[P2pIdx].WfdEntryInfo.coupled_sink_info.CoupledPeerAddr[0], MAC_ADDR_LEN);
...
//				printk(">> Eid = %d.\n", pWfdEid->Eid);
				...
				{						
					...
					case SUBID_WFD_COUPLED_SINK_INFO:
					{
						...
						RTMPMoveMemory(&pWfdEntryInfo->coupled_sink_info, pWfdEid->Octet, SUBID_WFD_COUPLED_SINK_INFO_LEN);
