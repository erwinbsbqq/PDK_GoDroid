...
//#endif /* CONFIG_PM */	
       ...
       /* Set some preconfigured value before interface up*/
...
#endif /* CONFIG_STA_SUPPORT */
#else
			pStats->noise = pAd->BbpWriteLatch[66]; /* noise level (dBm)*/
			pStats->noise += 256 - 143;
#endif
		}
			break;
		...
		case CMD_RTPRIV_IOCTL_ADAPTER_START_UP_TEST:
		/* test driver state to fRTMP_ADAPTER_START_UP */
			*(UCHAR *)pData = RTMP_TEST_FLAG(pAd,fRTMP_ADAPTER_START_UP);
			break;
	}
