	...
	OUT PFT_INFO pFtInfoBuf)
{
	...
	do 
	{
		...
		if (pFtInfo->RSNIE_Len != 0)
		{
			...
			if ((pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode == Ndis802_11AuthModeWPA1PSKWPA2PSK) ||
				(pAd->ApCfg.MBSSID[pEntry->apidx].AuthMode == Ndis802_11AuthModeWPA1WPA2)) 
		    {
			...
			NdisMoveMemory(pFtInfoBuf->FtIeInfo.R1khId, 
					   pAd->ApCfg.MBSSID[pEntry->apidx].Bssid, MAC_ADDR_LEN);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	{
		if (RTMPEqualMemory(pHdr->Addr1,
				pAd->ApCfg.MBSSID[apidx].Bssid, MAC_ADDR_LEN))
			break;
	}
...
/*				if ((FtInfoBuf.RicInfo.pRicInfo = kmalloc(512, GFP_ATOMIC)) != NULL) */
				if (FtInfoBuf.RicInfo.pRicInfo != NULL)
				{
					...
					FT_RrbEnqueue(pAd, pHdr->Addr3, (PFT_ACTION)pFtActFrame, 
									FtActFrameLen, pEntry->apidx);
/*					kfree(FtInfoBuf.RicInfo.pRicInfo); */
...
#endif
		RTMP_SET_PACKET_SOURCE(OSPKT_TO_RTPKT(pPktComm), PKTSRC_NDIS);
	...
	OUT	PFT_FTIE_INFO		pFtInfo)
{
	...
	key_idx = pAd->ApCfg.MBSSID[pEntry->apidx].DefaultKeyId;
	...
	OUT PFT_INFO 			pFtInfo_out)
{
	...
	{	
		...
		hex_dump("pAd->ApCfg.MBSSID[pEntry->apidx].Bssid=", pAd->ApCfg.MBSSID[pEntry->apidx].Bssid, 6);
			...
			(RTMPEqualMemory(pEntry->FT_PMK_R1_NAME, pR1hkEntry->PmkR1Name, LEN_PMK_NAME) == FALSE))
		{
			...
			FT_KDP_EVENT_INFORM(pAd, pEntry->apidx, FT_KDP_SIG_KEY_REQ_AUTO,
	...
	GenRandom(pAd, pAd->ApCfg.MBSSID[pEntry->apidx].Bssid, pEntry->ANonce);
...
{																								
	...
	if ((RTMPEqualMemory(pPeerFtIe->R0khId, pR1hkEntry->R0khId, pR1hkEntry->R0khIdLen) == FALSE) ||
		(RTMPEqualMemory(pPeerFtIe->R1khId, pAd->ApCfg.MBSSID[pEntry->apidx].Bssid, MAC_ADDR_LEN) == FALSE) ||
...
VOID FT_TEST(int input)
{
	...
	/*KDF(key_tmp, 3, "KDF test", 8, (PSTRING)data_tmp, 4, hash, sizeof(hash)); */
