...
#ifdef P2P_SUPPORT
#ifdef HE_BD_SUPPORT
			pAd->P2pCfg.P2PSupRate[0] = 0x12;
			pAd->P2pCfg.P2PSupRate[1] = 0x24;
			pAd->P2pCfg.P2PSupRate[2] = 0x48;
			pAd->P2pCfg.P2PSupRate[3] = 0x6c;
			pAd->P2pCfg.P2PSupRateLen = 4;
#endif
#endif
			break;
	...
	INOUT  STRING	tmpSecurityStr[])
{
	...
	if (chiper == 1)
	{
		...
		{
			if (pBss->WPA.PairCipherAux == Ndis802_11Encryption2Enabled)
				sprintf(tmpSecurityStr, "%sTKIP", tmpSecurityStr);
			else if (pBss->WPA.PairCipherAux == Ndis802_11Encryption3Enabled)
				sprintf(tmpSecurityStr, "%sAES", tmpSecurityStr);
		}
		if (pBss->WPA.PairCipher == Ndis802_11Encryption2Enabled)
			sprintf(tmpSecurityStr, "%sTKIP", tmpSecurityStr);
		else if (pBss->WPA.PairCipher == Ndis802_11Encryption3Enabled)
			sprintf(tmpSecurityStr, "%sAES", tmpSecurityStr);
	}
	else
	{
		...
		{
			if (pBss->WPA2.PairCipherAux == Ndis802_11Encryption2Enabled)
				sprintf(tmpSecurityStr, "%sTKIP", tmpSecurityStr);
			else if (pBss->WPA2.PairCipherAux == Ndis802_11Encryption3Enabled)
				sprintf(tmpSecurityStr, "%sAES", tmpSecurityStr);
		}
		if (pBss->WPA2.PairCipher == Ndis802_11Encryption2Enabled)
			sprintf(tmpSecurityStr, "%sTKIP", tmpSecurityStr);
		else if (pBss->WPA2.PairCipher == Ndis802_11Encryption3Enabled)
			sprintf(tmpSecurityStr, "%sAES", tmpSecurityStr);
	}
}
#endif /* DPA_T */
	...
	IN PSTRING		arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("Bulk-In Info:\n"));
	RTMP_IRQ_LOCK(&pAd->BulkInLock, IrqFlags);
	DBGPRINT(RT_DEBUG_OFF,  ("\tPendingRx=%d, BIIdx=%d, BIReadIdx=%d, BIPos=%ld, BIRPos=%ld, BIReq=x%lx, BIComplete=0x%lx, BICFail0x%lx\n", 
	...
	DBGPRINT(RT_DEBUG_OFF, ("\tDetailed RxContext Info:\n"));
	for (i = 0; i < RX_RING_SIZE; i++)
	{
		pRxContext = &pAd->RxContext[i];
 		DBGPRINT(RT_DEBUG_OFF, ("\t\tRxContext[%d]: IRPPending=%d, InUse=%d, Readable=%d, bRxHandling=%d, BIOffset=%ld!\n"
