    ...
    IN	BOOLEAN				bClearFrame)
{
	...
	do {
	        ...
	        DBGPRINT(RT_DEBUG_INFO,("%s - len=%d, AID=%d, DA=%02x:%02x:%02x:%02x:%02x:%02x\n", 
    						        ...
    						        pHeader802_3[0],pHeader802_3[1],pHeader802_3[2],
    						        pHeader802_3[3],pHeader802_3[4],pHeader802_3[5]));
    ...
    IN ULONG			TimeInterval) 
{
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->PMKID_CacheIdx != ENTRY_NOT_FOUND))
	{
        ...
        {    
                ...
                {
                        DBGPRINT(RT_DEBUG_ERROR, ("[PMF]%s: send Msg1 of 4-way using SHA256\n", __FUNCTION__));        
                        RT_HMAC_SHA256(pAd->ApCfg.MBSSID[apidx].PMK, PMK_LEN, PMK_key, 20, digest, LEN_PMKID);
                }
                else {
                        DBGPRINT(RT_DEBUG_ERROR, ("[PMF]%s: send Msg1 of 4-way using SHA1\n", __FUNCTION__));
                        RT_HMAC_SHA1(pAd->ApCfg.MBSSID[apidx].PMK, PMK_LEN, PMK_key, 20, digest, LEN_PMKID);
                }
    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
	...
	if (IS_P2P_CLI_ENTRY(pEntry) && (pEntry->bP2pClient))
	{
		...
		P2pSendWirelessEvent(pAd, RT_P2P_CONNECTED, pP2pEntry, pEntry->HdrAddr3);
    ...
    do
    {
		...
		if (IS_P2P_GO_ENTRY(pEntry))
		{
			if (pEntry->bP2pClient)
			{
				...
				//pAd->LinkedP2pOnGo +=0x1;//lw++
				DBGPRINT(RT_DEBUG_ERROR, ("Lwadd::p2p--RT_P2P_AP_STA_CONNECTED!\n"));
			...
			{
				#ifdef GET_P2PGO_LINKED_DEVICE
				//pAd->LinkedLegacyOnGo +=1;//lw++
				pAd->LinkedP2p0DevNumber++;
				DBGPRINT(RT_DEBUG_ERROR, ("Lwadd::Legacy--RT_P2P_AP_STA_CONNECTED!\n"));
				#endif
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
