...
void main()
{
	...
	{
					yuv.x = texture2D(s_texture_Y, v_texCoord + offset[i]).r;
					yuv.yz = texture2D(s_texture_C, v_texCoord + offset[i]).ra - vec2(0.5,0.5); //vec2(0.0,0.0);//
					vec3 rgb = yuv2rgb * yuv;
					//vec4 tmp = vec4(rgb, 1.0);
					vec4 tmp = vec4(rgb.b,rgb.g,rgb.r,1.0);
