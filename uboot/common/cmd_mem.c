...
int dcrc_chk(unsigned long addr)
{
	image_header_t *hdr = (image_header_t *)addr;
	
	if (!image_check_dcrc(hdr)) {
		printf("Data CRC verify FAIL, addr=0x%08x\n",addr);
		return 1;
	}

	printf("Data CRC verify okay.\n");

	return 0;
}

static int do_chkdcrc(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	unsigned long addr;
	image_header_t *hdr;
	...
	addr = simple_strtoul(argv[1], NULL, 16);
	return dcrc_chk(addr);
}

U_BOOT_CMD(
	chkdcrc,	2,	1,	do_chkdcrc,
