...
void cache_flush(unsigned long buf, unsigned long len);
void cache_invalidate(unsigned long buf, unsigned long len);
...
#define DESC_STATIC_STRU_SET_SIZE(desc, ind, size) ((unsigned int *)desc)[1 + 3*ind]=size 
#define DESC_OUTPUT_STRU_SET_SIZE(desc, ind, size) ((unsigned int *)desc)[1 + 3*ind]=0x10000|size      
