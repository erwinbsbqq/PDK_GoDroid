...
void omap_rev_string(void)
{
	...
	printf("OMAP%x ES%x.%x\n", omap_variant, major_rev,
		minor_rev);
#else
	extern void get_chip_info(u32 *id,u8 *package,u8 *version);

	u32 chip_id,chip_package,chip_ver;
	get_chip_info(&chip_id,&chip_package,&chip_ver);

	//printf("ALi_Chip_Version ID is 0x%x\n", ALi_Chip_Version);
	printf("ALi M%4x %s package\n", chip_id,M3921_QFP256 == chip_package?"QFP":"BGA");
#endif
}
