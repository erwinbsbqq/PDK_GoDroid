...
#ifdef IRC_DEBUG
	#define IRC_DEBUG_PRINTF	printf
#else
...
//#define IRC_TRACE
	...
	#define IRC_TRACE_PRINTF	printf
#else
...
static struct ir_attr ir_attr_rc6 = 
{
	...
	15000,		// pulse max width 
	{
		...
		{IR_END, PULSE_HIGH | RC6_6T_H, 1000}, 
	},
	ir_rc6, //pluse_step
	NULL, 
};
 ...
 static UINT32 rc5_x_decode(struct irc_decoder *ird, UINT32 pulse_width, UINT32 pulse_polarity)
{
  ...
  //  libc_printf("pulse_width %d[%d]\n", pulse_width , pulse_polarity);
	if (ird->first_half_got == 0)  	  /* first_half_got is flag  for dule-pulse senser */
	{
        ...
        else if(INRANGE(pulse_width, 2*PULSE_VALUE(ird->attr->pulse[0].fst_half),2*ird->attr->pulse[0].tolerance )) // 2T
	...
	else
	{
        ...
        else if(INRANGE(pulse_width, 2*PULSE_VALUE(ird->attr->pulse[0].scd_half),2*ird->attr->pulse[0].tolerance )) // 2T
        {
		...
		if (ird->attr->normal) 
		{
				...
				result == IRC_DECODE_DATA1)
			{
				...
				if(ird->key_bit_cnt == 13) 
				{
					...
					{
                         if(get_ticks() - key_got_tick < 200)
                         {
                            key_cnt =  1;  // repeat key ;
                            return  PAN_KEY_INVALID ;
                         }
					}
                    key_cnt = 0 ;
                    last_key = ird->last_key_code[0] ;
                    key_got_tick = get_ticks();
...
UINT32 irc_pulse_to_code(UINT32 pulse_width, UINT32 pulse_polarity)
{
	...
	for (i = 0; i< IRP_CNT; i++)
	{
		...
		{
			//printf("%s: Got key code %08X coded by %s.\n", __FUNCTION__, key_code, irp_strs[irc_decoders[i].attr->type]);
			return key_code;
		}
	}
