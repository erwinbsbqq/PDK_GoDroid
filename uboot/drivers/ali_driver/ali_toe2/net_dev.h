...
*    File:    net_dev.h
*
*    Description:     Header file of the management of net device.
*    History:
...
typedef struct tag_mac_device_stats
{
	...
	UINT32	rx_buffer_empty;			/* there is no packet stored in rx buffer ring */

	UINT32	rx_broadcast;			/* broadcast packets received */
	UINT32	rx_unicast;				/* unicast packets received */

	UINT32	rx_purged_errors;		/* receiver purged packet */
	UINT32 	rx_lost_in_ring;			/* got lost in ring buffer */

	UINT32	rx_buffer_errors;
	UINT32	rx_physical_layer_error;
	UINT32	rx_wd_timeout_errors;
	UINT32	rx_late_collision_seen;
	UINT32	rx_dribble_errors;
	UINT32	rx_long_errors;
	UINT32	rx_runt_errors;

	UINT32	ip_chksum_errors;
	UINT32	tcp_chksum_errors;

	UINT32	tx_collision_counter[16];	/* 0 to 15 collisions */
	UINT32	tx_collision_errors;		/* excessive collision */
} mac_device_stats, *pmac_device_stats;
...
//To calculate CRC-32 value
UINT32 calc_crc32(UINT8 *data, UINT32 len, UINT32 invert)
{
	UINT8 *p;
	UINT32  crc;
	...
	return (invert ? (~crc) : crc);			// transmit complement, per CRC-32 spec
}
 */
