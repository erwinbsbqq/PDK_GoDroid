...
extern struct remote_hld_device *remote_hld_device_base ;
...
//	printf("dev_get_by_id: type%x, id:%x \n",type,id);
...
void remote_hld_dev_memcpy(void *dest, const void *src, unsigned int len)
{
    ...
    MEMCPY(dest, src,len);
	if ((((UINT32)dest)&0x80000000) == 0x80000000)
