...
/*
The function used to get the address and length in flash by chunk ID.
UINT32 id      : The chunk ID number
UINT32 *addr   : The start address of the chunk ID, output 
UINT32 *len : The length of the chunk ID, output 
*/
int sto_get_chunk_len(UINT32 id, UINT32 *addr, UINT32 *len)
{
	RET_CODE ret;
	UINT32 chunk_hdr;
    chunk_hdr =  (UINT32)sto_chunk_goto(&id, 0xFFFF0000, 1); 
    if((ERR_PARA == chunk_hdr)||(ERR_FAILUE== chunk_hdr))
        return 1;
...
UINT32 sto_chunk_check(void)
{
	...
	do
	{
		...
		if (data_crc != NO_CRC)
		{
			...
			crc = data_crc ;//MG_Table_Driven_CRC(0xFFFFFFFF, buffer, data_len);
			FREE(buffer);
			if (data_crc != crc)
			{
 ...
 UINT32 sto_chunk_check_ext(UINT32 image_addr, UINT32 max_len)
{
	...
	do
	{
		...
		data_id  = fetch_long((UINT32)buffer + CHUNK_ID);
		data_len = fetch_long((UINT32)buffer + CHUNK_LENGTH);
		data_off = fetch_long((UINT32)buffer + CHUNK_OFFSET);
		data_crc = fetch_long((UINT32)buffer + CHUNK_CRC);
		...
		if (data_crc != NO_CRC)
		{
			if(buffer + data_len + 16 > image_addr + max_len)
			{
