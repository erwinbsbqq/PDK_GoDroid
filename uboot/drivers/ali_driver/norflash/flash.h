...
static void sflash_get_id(UINT32 *result, UINT32 cmdaddr);
static int sflash_erase_sector(UINT32 sector_addr);
static int sflash_erase_chip(void);
static int sflash_verify(unsigned int flash_addr, unsigned char *data, unsigned int len);
static int sflash_read(void *buffer, void *flash_addr, UINT32 len);
static int sflash_copy(UINT32 flash_addr, UINT8 *data, UINT32 len);
void sflash_set_io(UINT8 io_num, UINT8 board_support_qio);

//#define SYS_FLASH_BASE_ADDR	0xafc00000
#ifdef ALI_ARM_STB
#define SF_BASE_ADDR		0x1802e000
#else
...
#ifdef ALI_ARM_STB
#define FLASH_FAST_READ_EN()	*((volatile UINT8 *)(0x18000099)) = (0x4D)
#define FLASH_FAST_READ_DIS()	*((volatile UINT8 *)(0x18000099)) = (0x0D)
#else
#define FLASH_FAST_READ_EN()	*((volatile UINT8 *)(0xb8000099)) = (0x4D)
#define FLASH_FAST_READ_DIS()	*((volatile UINT8 *)(0xb8000099)) = (0x0D)
#endif
//#define MUTEX_ENTER()       osal_mutex_lock()
//#define MUTEX_LEAVE()       osal_mutex_unlock()
...
extern struct flash_info_st flash_info;
