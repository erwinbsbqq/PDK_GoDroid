...
unsigned short sflash_devtp = 0;
unsigned short sflash_devid = 0;
...
#define sflash_write_soft_protect(...) do{} while(0)
...
void sflash_set_io(UINT8 io_num, UINT8 fls_idx)
{
	...
	{	
		...
		if(ALI_S3602F==sys_ic_get_chip_id()||ALI_S3811==sys_ic_get_chip_id())
			*((volatile UINT8 *)0xb800008d) &= 0xfe;
		sys_ic_revert_boot_type();
		return;
	}	
...
void sflash_get_id(UINT32 *result, UINT32 cmdaddr)
{
  ...
  else if(ALI_S3921<=sys_ic_get_chip_id() )
  	sflash_reg_addr = 0x1802e000;
	...
	{
		tmp = *((volatile UINT32 *)0x18000098);
	...
	printf("nor flash get id: cmd:0x9f,result:0x%08x \n",result[0]);
	...
	printf("nor flash get id: cmd:0xab,result:0x%08x \n",result[1]);
#ifdef SUPPORT_GLOBAL_PROTECT	
