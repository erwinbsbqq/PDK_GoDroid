...
static int ep_link(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_CONTROL:
		ed->hwNextED = 0;
#ifdef CONFIG_USB_OHCI_ALI
		if (ohci->ed_controltail == NULL)
			ohci_writel(ed & ~(0xA0000000), &ohci->regs->ed_controlhead);			
		else
			ohci->ed_controltail->hwNextED = m32_swap((unsigned long)ed) & ~(0xA0000000);					
#else
		if (ohci->ed_controltail == NULL)
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL)
			ohci_writel(ed & ~(0xA0000000), &ohci->regs->ed_bulkhead);				
		else
			ohci->ed_bulktail->hwNextED = m32_swap((unsigned long)ed) & ~(0xA0000000);					
#else
		if (ohci->ed_bulktail == NULL)
	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev(6, interval); i += inter) {
			inter = 1;
#ifdef CONFIG_USB_OHCI_ALI
			for ( ed_p = (__u32) &(ohci->hcca->int_table[ep_rev(5, i) + int_branch]) | 0xA0000000; 
			     (*ed_p != 0) && (((ed_t *)ed_p)->int_interval >= interval);
				 ed_p = (__u32) &(((ed_t *)ed_p)->hwNextED) | 0xA0000000)
				inter = ep_rev(6,((ed_t *)ed_p)->int_interval);
			ed->hwNextED = *ed_p & ~(0xA0000000);
			*ed_p = m32_swap((unsigned long)ed) & ~(0xA0000000);
#else				
...
#ifdef CONFIG_USB_OHCI_ALI
		__u32	*ed_p = (__u32) &ohci->hcca->int_table [index] | 0xA0000000; 

		/* ED might have been unlinked through another path */
		while (*ed_p != 0) {
			if (((struct ed *) m32_swap((unsigned long)ed_p)) == ed) {
				*ed_p = ed->hwNextED | 0xA0000000; 
				break;
			}
			ed_p = (__u32) &(((struct ed *) m32_swap((unsigned long)ed_p))->hwNextED) | 0xA0000000; 
		}
#else		
			...
			int interval, int load)
{
	...
	if (ed->state == ED_NEW) {
		...
		td = td_alloc(usb_dev);
#ifdef CONFIG_USB_OHCI_ALI
		ed->hwTailP = m32_swap((unsigned long)td) & ~(0xA0000000);
#else
...
#ifdef CONFIG_USB_OHCI_ALI
	td = urb_priv->td [index] = (td_t *)(m32_swap(urb_priv->ed->hwTailP) | (0xA0000000) & ~0xf);
#else
...
#ifdef CONFIG_USB_OHCI_ALI
	td->hwCBP = m32_swap((unsigned long)data) & ~(0xA0000000);
	if (data)
		td->hwBE = m32_swap((unsigned long)(data + len - 1) & ~(0xA0000000));		
	else
		td->hwBE = 0;
	td->hwNextTD = m32_swap((unsigned long)td_pt & ~(0xA0000000));	
#else
...
#ifdef CONFIG_USB_OHCI_ALI
	tdBE   = m32_swap(td->hwBE)  | 0xA0000000;
	tdCBP  = m32_swap(td->hwCBP) | 0xA0000000;
#else
...
#ifdef CONFIG_USB_OHCI_ALI
	__u32      *phwHeadP  = (__u32) (&td_list->ed->hwHeadP) | 0xA0000000;
#else
...
static td_t *dl_reverse_done_list(ohci_t *ohci)
{
...
#ifdef CONFIG_USB_OHCI_ALI	
	td_list_hc = m32_swap(ohci->hcca->done_head) & 0xfffffff0 | 0xA0000000;	
#else	
	...
	while (td_list_hc) {
		...
		td_rev = td_list;
#ifdef CONFIG_USB_OHCI_ALI
		td_list_hc = m32_swap(td_list->hwNextTD) & 0xfffffff0 | 0xA0000000;	
#else
...
#ifdef CONFIG_USB_OHCI_ALI
	ohci_writel((__u32)ohci->hcca & ~(0xA0000000), &ohci->regs->hcca);	/* ALi OHCI HW should set physical address */
#else
