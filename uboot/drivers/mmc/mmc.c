...
int mmc_send_ext_csd(struct mmc *mmc, char *ext_csd)
{
	...
	for (i =0;i<512;i++)
	{
		if (ext_csd[511-i]) //only printf non-zeoro value
			printf("-----ext_csd[%03d]=0x%02X-----\n",511-i,ext_csd[511-i]);
	}
	printf("\n\n");
#endif
...
int mmc_switch_part_bootflag(int dev_num, unsigned int boot_part_num)
{
	struct mmc *mmc = find_mmc_device(dev_num);
	...
	if (7 != boot_part_num && boot_part_num > 2)
		return -1;

	mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BOOT_BUS_WIDTH,
			  1);  // 4 bit mode

	return mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONF,
			  (mmc->part_config  | (boot_part_num <<3)));
}
#endif
...
//add code by David.Chen@2013/12/04, when write block size is 1, CMD24 will not return a interrupt signal.
#if defined(CONFIG_ALI_MMC)
