...
#define __CACHE_INV_ALI(start, len)			dmac_map_area((unsigned long)(start), (const void *)((unsigned long)start + (unsigned long)len), DMA_FROM_DEVICE)
#define __CACHE_CLEAN_ALI(start, len)		dmac_map_area((unsigned long)(start), (const void *)((unsigned long)start + (unsigned long)len), DMA_TO_DEVICE)
