...
#define 	MAX_BW_GROUP 		18 //18 //20

#define 	MAX_SINGLE_IP_IDX 	20		
#define 	MAX_IP_IDX 			(MAX_SINGLE_IP_IDX+2)
...
struct test_ip_bw_cfg_info
{
     ...
     __u32 	ip_enable_flag;	//Enable the Single IP to be monitor through bit0~bit20(IP0~IP20);
     								//0, not be monitored; 1, be monitored
};
...
struct test_ip_get_bw
{
	...
	__u16	ip_bw_cnt;  //Count the valid number of IP BW
						
	__u32 	ip_idx_flag;	//Enable the Single IP to get BW value through bit0~bit20(IP0~IP20);
     						//0, disabled to get bw; 1, enabled to get bw

	struct 	test_ip_bw_info bw_info[MAX_IP_IDX];	//Buffer to Save the related info about BW:
							      					//bw_info[0]: Save the info about Total BW
							      					//bw_info[1]~bw_info[21]: Save the Bw info about single IP from IP_0 to IP_20
};
