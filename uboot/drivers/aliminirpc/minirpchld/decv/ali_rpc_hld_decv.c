...
RET_CODE vdec_set_output(struct vdec_device *dev, enum VDecOutputMode eMode,struct VDecPIPInfo *pInitInfo, 	struct MPSource_CallBack *pMPCallBack, struct PIPSource_CallBack *pPIPCallBack)
{
    ...
    mps_cb.handler = (UINT32)pMPCallBack->handler;
    mps_cb.RequestCallback = (UINT32)pMPCallBack->RequestCallback;
    mps_cb.ReleaseCallback = (UINT32)pMPCallBack->ReleaseCallback;
    mps_cb.vblanking_callback = (UINT32)pMPCallBack->vblanking_callback;

    pips_cb.RequestCallback = (UINT32)pPIPCallBack->RequestCallback;
    pips_cb.ReleaseCallback = (UINT32)pPIPCallBack->ReleaseCallback;
