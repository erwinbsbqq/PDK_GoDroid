...
#define RPC_BASE_ADDR 					__VMEMALI(__G_MM_SHARED_MEM_START_ADDR)
...
void *ali_rpc_malloc_shared_mm(unsigned long size)
{
	...
	printf("ali_rpc_malloc_shared_mm, prev:0x%x, ptr:0x%x, flen:0x%x\n", prev, ptr, ptr[0]&0xffff);     
	while(1){
		head = ptr[0];
		flen  = head&0xffff; 
		if(flen == size)
...
int ali_rpc_init_mutex(void)
{
	...
	printf("__G_MM_SHARED_MEM_START_ADDR:0x%08x, ALLOC_SM_ADDR:0x%08x ALLOC_SM_LEN:0x%08x\n", __G_MM_SHARED_MEM_START_ADDR, ALLOC_SM_ADDR
		, ALLOC_SM_LEN);	
#if 0
