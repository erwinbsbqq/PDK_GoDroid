...
#if 1 //defined(__ALI_TDS__)
static Ulong ntohl(Ulong n)
{
    return g_param.g_BigEndian ? n : BigLittleSwap32(n);

}

static Ulong htonl(Ulong h)
{
    return g_param.g_BigEndian ? h : BigLittleSwap32(h);
...
void XDR_Create(XDR *xdrs, Char *addr, Uint32 size, enum XDR_OP op)
{
    ...
    Log(LOG_DEBUG,"[XDR_Create] xdrs->x_handy:0x%x\n",xdrs->x_handy);
}
