...
#ifdef RPC_SAMPLE_TEST_OPAQUE
   		RPC_PARAM_UPDATE(p1, PARAM_OUT, PARAM_OPAQUE, 128, opaquebuf);
...
#ifdef RPC_SAMPLE_TEST_OPAQUE
		//Log(LOG_INFO,"RpcCallCompletion, [peer opaque return result] buf[0]:%c, buf[55]:%c, buf[127]:%c",opaquebuf[0],opaquebuf[55],opaquebuf[127]);
#endif
...
int rpc_debuglevel_procfile_read(char*buffer, char**buffer_localation, off_t offset,int buffer_length,int* eof, void *data )
{
	...
	if(offset>0){
		*eof = 1;
		...
		if(buffer_length>0){
			buffer[0] = g_param.g_dbglevel+0x30;
			ret = 1;
		}
		else{
			ret = 0;
...
int rpc_debuglevel_procfile_write(struct file *filp, const char *buffer,unsigned long count,void *data)
{
	...
	if(itemp>=LOG_EMERG && itemp<=LOG_DEBUG)
 		g_param.g_dbglevel = itemp;
	/*Call remote debug level setting*/
	RPC_PARAM_CREATE(p1, PARAM_IN, PARAM_INT32, sizeof(Int32), &g_param.g_dbglevel);
...
{
	Log(LOG_DEBUG,"MAIN, MiniRpcInit enered!\n");
	Log(LOG_ERR,"++++++MAIN Will not startup the see!\n");
	///g_see_boot();
	PR_uSleep(100000);
	Log(LOG_ERR,"++++++MAIN will not startup see completed!\n");
