...
void ArpRequest(void)
{
	...
	ALI_PRINTF ("<ArpRequest> ARP broadcast %d @ get_timer(0)=0x%x,ARP_TIMEOUT=0x%x,CONFIG_SYS_HZ=%d\n", NetArpWaitTry,get_timer(0),ARP_TIMEOUT,CONFIG_SYS_HZ);
...
restart:
	...
	ALI_PRINTF("NetLoop: protocol = %d,bd->bi_ip_addr=0x%x\n", protocol,bd->bi_ip_addr);
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	if (memcmp(ether, NetEtherNullAddr, 6) == 0) {
		...
		ALI_PRINTF("sending ARP for %08lx\n", dest);
	...
	ALI_PRINTF("sending UDP to %08x/%pM\n", dest, ether);
...
{
	eth_halt();
	ALI_PRINTF ("Ping Timeout...timeStart=0x%x,get_timer(0)=0x%x\n",timeStart,get_timer(0));
	    ...
	    unsigned len)
{
	if (sip != NetPingIP)
      {
        	ALI_PRINTF ("PingHandler...,NetPingIP=0x%x,sip=0x%x,t=0x%x,NetArpWaitTimerStart=0x%x,NetArpWaitPacketIP=0x%x,NetArpWaitPacketMAC=0x%x\n",NetPingIP,sip,get_timer(0),NetArpWaitTimerStart,NetArpWaitPacketIP,NetArpWaitPacketMAC);
		return;
      }
...
static void receive_icmp(IP_t *ip, int len, IPaddr_t src_ip, Ethernet_t *et)
{
	...
	case ICMP_ECHO_REQUEST:
		ALI_PRINTF("Got ICMP ECHO REQUEST, return %d bytes\n",
			ETHER_HDR_SIZE + len);
...
NetReceive(volatile uchar *inpkt, int len)
{
	...
	if (x < 1514) {
		...
		ALI_PRINTF("VLAN packet received\n");
	...
	ALI_PRINTF("Receive from protocol 0x%x\n", x);
	...
	case PROT_ARP:
		...
		case ARPOP_REQUEST:
			...
			ALI_PRINTF("Got ARP REQUEST, return our IP\n");
			pkt = (uchar *)et;
		...
		case ARPOP_REPLY:		/* arp reply */
			...
			ALI_PRINTF("Got ARP REPLY, set server/gtwy eth addr (%pM)\n",
				arp->ar_data);
			...
			if (tmp == NetArpWaitReplyIP) {
				...
				ALI_PRINTF("<%s>Line:%d: NetArpWaitReplyIP=0x%x,NetArpWaitPacketMAC=%02x:%02x:%02x:%02x:%02x:%02x====------.\n",__FUNCTION__,__LINE__,NetArpWaitReplyIP,
			...
			return;
		default:
			ALI_PRINTF("Unexpected ARP opcode 0x%x\n",
			      ntohs(arp->ar_op));
			return;
		}
		break;
	...
	case PROT_RARP:
		...
		if ((ntohs(arp->ar_op) != RARPOP_REPLY) ||
			...
			ALI_PRINTF("<%s>Line:%d: CALL packetHandler()====------.\n",__FUNCTION__,__LINE__);
			(*packetHandler)(0, 0, 0, 0, 0);
		}
		break;
#endif
