...
int get_part_by_name(const char *partName,loff_t *start, size_t *size)
{
	...
	*size = part->size;
	...
	profile_dbg("get_part_by_name %s, start:0x%x, size:0x%x\n",partName, part->offset,  part->size);
	return pnum;
}
...
int load_part_sector(const char *partName, u_char * LoadAddr, int bufLen)
{
	...
	profile_dbg("load_part_sector %s, blockNum:0x%x\n",partName, blockNum);
...
/*
	...
	*/
	...
	entry = (void *)(info->memmap_info.see_start |0xa0000000)  - 64;//64byte ubo header size
	...
	//while(1);
//	info->memmap_info.kernel_start = 0x80100000;
	entry = (void *)(info->memmap_info.kernel_start |0xa0000000) - 64;//64byte ubo header size
