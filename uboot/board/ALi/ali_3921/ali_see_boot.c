...
void install_memory(void)
{
	...
	unsigned long __G_MM_SHARED_MEM_START_ADDR = (unsigned long)(__MM_SEE_MAIN_SHARED_MEM_START);
	unsigned long __G_RPC_MM_LEN = (unsigned long)(__MM_SEE_MAIN_SHARED_MEM_END - __MM_SEE_MAIN_SHARED_MEM_START);

    unsigned long __G_MM_MCAPI_MEM_START_ADDR = (unsigned long)0x857fe000;	
	unsigned long __G_MM_MCAPI_MEM_LEN = (unsigned long)(0x85800000 -0x857fe000);

    priv_mem_base_addr = (unsigned long)0x84000000; 
    priv_mem_len = (unsigned long)(0x857fd000 - 0x84000000);  

	share_mem_base_addr = (unsigned long)0x857fd000;  
    share_mem_len = (unsigned long)(0x857fe000 - 0x857fd000); 
...
#define SEE_LOAD_ADDR   0x18000200
#define SEE_LOOP_ADDR   0x18000280
#define SEE_CFG_ADDR    0x18000220
#define SEE_RTIG_BIT    1

static int set_see_parameter(unsigned long run_addr, unsigned long code_addr,unsigned long len)
{
	*(volatile unsigned long *)(SEEROM_SEE_REAL_RUN_ADDR) = run_addr ;
	*(volatile unsigned long *)(SEEROM_SEE_SIGN_LOCATION_ADDR) = run_addr+0x40000;
	*(volatile unsigned long *)(SEEROM_SEE_CODE_LEN_ADDR) = len ;
	*(volatile unsigned long *)(SEEROM_SEE_CODE_LOCATION_ADDR) = code_addr;
	return 0;
}
...
void  g_see_boot(void)
{
    ...
    if(IS_SEEROM_ENABLE)
    {
		printf("SEEROM is enable boot flow, 0x%x\n",addr);
        *(volatile unsigned long  *)(SEE_CFG_ADDR) &= (~(1<<SEE_RTIG_BIT));
        set_see_parameter(addr, addr,MIN_SEE_LEN);
		for(i=0;i<10000;i++)
		{;}		
        *(volatile unsigned long  *)(SEE_CFG_ADDR) |= (1<<SEE_RTIG_BIT);        
        *(volatile unsigned long  *)(SEE_LOAD_ADDR) = addr;
    }else{
    	printf("SEEROM is not enable boot flow, 0x%x\n",addr);
		*(volatile unsigned long *)(SEE_CFG_ADDR) &= (~(1<<SEE_RTIG_BIT));
		*(volatile unsigned long *)(SEE_LOAD_ADDR) = addr;
		for(i=0;i<10000;i++)
		{;}
...
void  g_see_stop(void)
{
    u32 i=0;
	*(volatile unsigned long *)(SEE_LOAD_ADDR) = SEE_LOOP_ADDR;
	*(volatile unsigned long *)(SEE_CFG_ADDR) &= (~(1<<SEE_RTIG_BIT));
	//mdelay(1);
	for(i=0;i<10000;i++)
	{;}
...
void wait_see_minirpc_prepared()
{
  ...
  priv_mem_base_addr = (unsigned long)__G_MM_PRIVATE_AREA_START_ADDR; 
  arg_base = (priv_mem_base_addr+0x200 -16);
