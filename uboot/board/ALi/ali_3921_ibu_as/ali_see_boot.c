...
void install_memory(void)
{
    ...
    void *pbuf;
	unsigned long minirpc_share_mem_base;
    ...
    priv_mem_base_addr = (unsigned long)0x84000200; 
    ...
    arg_base = (priv_mem_base_addr -16);

#ifdef CONFIG_ALI_MINIRPC
    /*Added by Kinson for minirpc share mem*/
	__G_MINIRPC_SHARED_MEM_START_ADDR = (unsigned long)0x85ffd000;
	SET_DWORD((arg_base-12), (__G_MINIRPC_SHARED_MEM_START_ADDR&0x1fffffff)); //uboot notify the minirpc share mm addr to see
#endif	
	...
	printf("__G_MINIRPC_SHARED_MEM_START_ADDR:0x%x\n",__G_MINIRPC_SHARED_MEM_START_ADDR);
    printf("/***************************************************/\n\n");
...
{
    u32 i=0;
	u32 addr = 0x85000200; //SEE_RUN_ADDR;
...
void  ali_see_boot(unsigned long addr)
{
    *(volatile unsigned long *)0x18040038 =0 ; // disable see interrupt
    *(volatile unsigned long *)0x1804003C =0 ; // disable see interrupt
    udelay(10);
    *(volatile unsigned long *)0x18000220 &=~(0x1 <<1) ;// dis see 
    ...
    if(addr > 0x80000000)
    {
	    ...
	    *(volatile unsigned long *)(0x18000200) = addr; 
        ...
        printf("ali_see_boot failed\n");
    }

//	install_memory();
    udelay(10);
    *(volatile unsigned long*)0x1800020C |= 1 ; // cold boot up flag 
    *(volatile unsigned long*)0x18000220 |= 0x1<<1 ;
    mdelay(10);                        // wait some time for usee boot
}
...
/*SEE status register*/
#define SEEROM_SEE_RUN_STATUS_ADDR      (SYS_SEE_NB_BASE_ADDR+0x2C8)
/*
...
void wait_see_minirpc_prepared()
{
  unsigned long arg_base;
  unsigned long priv_mem_base_addr;
  volatile unsigned int *ack_flag = (volatile unsigned int *)SEEROM_SEE_RUN_STATUS_ADDR;
  ...
  printf("now main cpu wait to see, because see may not has prepare for minirpc\n");
   ...
   printf("GET_DWORD(0x180402c8)!\n");
  while(1)
  {
    //printf("ack_flag: 0x%x\n", (*ack_flag));
     if(0x30317a8b == (*ack_flag)) //tds will write 0x30317a8b to this addr
     {
